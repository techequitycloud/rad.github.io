...
the **Grafana on GKE Autopilot** module on Google Cloud: deploy it, access and
...
Grafana product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Grafana_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Grafana (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   ...
   `grafana-data` GCS bucket, builds the container image, and starts the pods.
   Grafana auto-migrates its schema on first startup — no separate
   database-initialisation job is required. First deploys take roughly
...
2. Grafana does not auto-generate an admin password. If you injected one via
   ...
   it immediately after first login. Sign in at `http://${EXTERNAL_IP}`.
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   ...
   Monitoring → Uptime checks (targeting `/api/health`) and Alerting → Policies.
...
- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. Grafana migrates
  its schema on first boot — allow up to ~150 seconds before declaring a startup
  failure.
  ```bash
...
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret materialised into the namespace, and `GF_DATABASE_TYPE=postgres`
  is present in the pod environment (it is injected automatically).
...
- **PVC permission errors (StatefulSet):** confirm `stateful_fs_group = 472` is set
  so Grafana (UID/GID 472) can write to the mounted volume.
  ```bash
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (PostgreSQL 15), GCS bucket, secrets; Grafana auto-migrates its schema |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes at `/api/health`; retrieve admin credential and sign in |
| 3 — Operate | Manual | Inspect workload, scale, update version, manage secrets/storage, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose pod, database, admin-password, PVC, scheduling, and image-pull issues |
