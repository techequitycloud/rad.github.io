...
*   **Service Mesh (GKE):** The `configure_service_mesh` variable (§8.C Service Mesh in App GKE) enables Anthos Service Mesh (ASM) integration. When the module owns the GKE cluster (`prereq_needs_gke = true`), it registers the cluster with GKE Fleet Hub and enables `MANAGEMENT_AUTOMATIC` ASM, which handles sidecar injection and control-plane upgrades automatically. On externally-provisioned clusters (Services_GCP-managed), the namespace label `"istio.io/rev" = "asm-managed"` is applied to enable sidecar injection, providing mutual TLS (mTLS) between services and fine-grained traffic management (retries, circuit breaking, fault injection) without application code changes.
*   **Multi-Cluster Service (GKE):** The `enable_multi_cluster_service` variable (§8.D Multi-Cluster Services in App GKE) exports the Kubernetes Service to other clusters in the same GKE Fleet using a stable DNS name (`SERVICE.NAMESPACE.svc.clusterset.local`), enabling cross-cluster service discovery without public load balancers. All participating clusters must be Fleet-registered — handled automatically when `configure_service_mesh = true`.
...
**In the RAD UI:**
*   **Data Storage Allocation:** Utilizing `storage_buckets` (Group 9 for Cloud Run; §3.C Storage in App GKE) creates regional or multi-regional buckets for application data. Each bucket supports per-bucket configuration of storage class (`STANDARD`, `NEARLINE`, `COLDLINE`, `ARCHIVE`), versioning, lifecycle rules, uniform bucket-level access, and public access prevention — all available in the `storage_buckets` variable definition.
*   **Security and Access Management:** The platform automatically assigns minimum IAM roles (like `roles/storage.objectAdmin`) to the specific workload identities created for the applications. App_Common's `app_storage_enhanced` module optionally encrypts buckets with CMEK via `app_cmek`.
*   **Data Protection:** For App CloudRun, the `backup_schedule` and `backup_retention_days` variables (Group 12) configure automated Cloud Scheduler-triggered database backup Jobs. For App GKE, equivalent scheduling is configured via `cron_jobs` (§3.E) using the built-in `db-export` CronJob; backup bucket retention is set in `storage_buckets` lifecycle rules (§3.C).
...
*   **Workload Type (GKE):** The `workload_type` variable (§3.A Compute & Scaling in App GKE) controls whether the application is deployed as a `Deployment` (stateless, rolling updates) or a `StatefulSet` (stable pod identities, per-pod persistent storage) — a foundational architectural choice that determines failure behavior, update strategies, and storage topology.
*   **Resource Quotas (GKE):** The `enable_resource_quota` variable (§7.C Resource Quotas in App GKE) enforces namespace-level CPU, memory, pod count, and PVC ceilings for multi-tenant clusters, preventing any single application from monopolising shared cluster resources. Configurable via `quota_cpu_requests`, `quota_memory_requests`, `quota_max_pods`, and related variables.
