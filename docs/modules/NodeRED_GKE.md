...
| Credential secret | None | This module does not create a Secret-Manager-backed flow-credential key — see below |
...
- **No credential-encryption secret is created by this module.** Despite a
  `database_password_length` variable whose description references
  `NODE_RED_CREDENTIAL_SECRET`, no Secret Manager secret or matching
  environment variable is actually wired anywhere in this module's source —
  it is inert (the variable only takes effect when `database_type` is a real
  database engine, and this module fixes `database_type = "NONE"`). Node-RED
  manages its own flow-credential encryption key internally on the
  NFS-backed `/data` volume; there is nothing to retrieve from Secret Manager.
...
### D. Secret Manager — no application-specific secrets

This module generates **no** Secret Manager secrets for Node-RED. A
`database_password_length` variable exists whose description references a
`NODE_RED_CREDENTIAL_SECRET`, but it is inert — it only has any effect when
`database_type` is a real database engine, and this module fixes
`database_type = "NONE"`. There is no secret to list or retrieve for this
deployment; Node-RED's own flow-credential encryption key lives on the
NFS-backed `/data` volume, not in Secret Manager.

See [App_GKE](App_GKE.md) for the Secret Store CSI integration used by
modules that do generate application secrets.
...
- **Flow credential encryption is not RAD-managed.** No Secret-Manager-backed
  key is injected by this module (the `database_password_length` variable's
  description mentions `NODE_RED_CREDENTIAL_SECRET`, but it is inert — it only
  applies when `database_type` is a real engine, and this module fixes it to
  `"NONE"`). Node-RED manages its own encryption key for `flows_cred.json`
  internally on the NFS-backed `/data` share.
...
| Variable | Default | Description |
|---|---|---|
| `database_password_length` | `32` | **Inert** — its description references `NODE_RED_CREDENTIAL_SECRET`, but it only takes effect when `database_type` is a real database engine, and this module fixes `database_type = "NONE"`. No such secret is actually created. |
| `database_type` | `NONE` | Node-RED requires no database. Do not change. |
| `enable_auto_password_rotation` | `false` | **Inert for the same reason** — automated rotation only applies when `database_type` is not `"NONE"`. |
...
| Flow credential encryption | not RAD-managed | Medium | This module creates no Secret-Manager-backed key — `database_password_length`'s description mentions `NODE_RED_CREDENTIAL_SECRET`, but it's inert (only applies when `database_type` isn't `"NONE"`). Node-RED's own key lives on the NFS-backed `/data` volume; losing that volume's contents loses the ability to decrypt existing flow credentials. |
| `enable_auto_password_rotation` | `false` (only value that has any effect) | Low | Inert for this module — automated rotation only applies when `database_type` is not `"NONE"`, and this module fixes it to `"NONE"`. |
