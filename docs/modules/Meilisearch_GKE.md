...
| Ingress | Cloud Load Balancing (optional) | LoadBalancer Service by default; optional external Gateway + managed certificate |
...
- **LoadBalancer by default.** `service_type = "LoadBalancer"` exposes the search
  API externally. Set `service_type = "ClusterIP"` to keep it inside the cluster
  instead, or use the Gateway (`enable_custom_domain`, also on by default) for a
  custom domain with a managed certificate.
...
By default the workload is exposed as a LoadBalancer Service, reachable externally.
Set `service_type = "ClusterIP"` to restrict it to inside the cluster instead. A
custom domain with a Google-managed certificate can be enabled through the Gateway
API (`enable_custom_domain`, on by default), and a static IP is reserved by default
