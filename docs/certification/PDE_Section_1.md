...
*   **GKE Cluster Selection Modes for Multi-Cluster Deployments:** App GKE's `gke_cluster_selection_mode` variable (§3.D Networking) controls which cluster or clusters receive a workload deployment: `primary` targets the single cluster named by `cluster_name`; `explicit` deploys to all clusters listed in `cluster_names` (for deterministic multi-region deployment); and `round-robin` cycles deployments across all clusters registered in the GKE Fleet. In a multi-environment DevOps workflow, cluster selection mode is a key variable that changes per environment: non-production environments typically use `primary` to reduce cost; production environments use `explicit` or `round-robin` to deploy across multiple regional clusters for high availability. Pair cluster selection modes with Multi-Cluster Services (`enable_multi_cluster_service`, §8.D) to enable cross-cluster service discovery.
