...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Audiobookshelf (GKE)**
   ...
   Set `service_type = "ClusterIP"` only if you deliberately want it cluster-internal. Click **Deploy Module**,
   review the estimated cost in the confirmation dialog (if credits are enabled) and click **Confirm**, which opens the deployment
