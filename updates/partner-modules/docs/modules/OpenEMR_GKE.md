...
| **OE_PASS secret** | An OpenEMR admin password is auto-generated by `OpenEMR_Common` and stored in Secret Manager. The plaintext value is passed to `App_GKE` via `explicit_secret_values`, which creates a Kubernetes Secret and injects it as the `OE_PASS` environment variable. OpenEMR uses this on first boot to set the administrator account password. |
| **MYSQL_PASS secret** | The MySQL database password generated by `App_GKE` is automatically injected as the `MYSQL_PASS` environment variable via the `db_password_env_var_name = "MYSQL_PASS"` parameter passed to `App_GKE`. Do not define this manually in `secret_environment_variables`. |
