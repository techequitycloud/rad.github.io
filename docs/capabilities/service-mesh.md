...
The observability add-ons are installed in the same `null_resource`: Prometheus, Grafana, Jaeger, Kiali. See [observability](observability).
...
`modules/AKS_GKE/modules/attached-install-mesh/` and the EKS equivalent install ASM on a non-GCP cluster. Not invoked automatically by the parent module; callers opt in. See [multicloud](multicloud).
