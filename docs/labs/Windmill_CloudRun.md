---
title: "Windmill on Cloud Run \u2014 Lab Guide"
description: "Hands-on lab: deploy Windmill on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
of the **Windmill on Cloud Run** module on Google Cloud: deploy it, access and verify
...
not on Windmill product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Windmill (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL 16) database
   with its Secret Manager secrets, a GCS `data` bucket, builds the container
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~windmill" --format="value(metadata.name)" --limit=1)
...
1. Confirm the service is healthy and connected to its database:
   ...
   ```bash
   curl -s "$SERVICE_URL/api/version"   # expect a JSON response with the Windmill version
   ```

2. Open `$SERVICE_URL` in a browser. The Windmill login page appears. The first account
   you create is automatically granted the `super-admin` role — there is no
   pre-provisioned admin password. Create your admin account directly in the Windmill
   UI.
...
2. **Monitoring** — open the Cloud Run dashboard for the service and review request
   ...
   / memory utilisation. The module can also provision an **uptime check** targeting
   `/api/version` (when enabled via `uptime_check_config`, which defaults to disabled);
...
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret exists, and the initialisation job completed. Windmill requires
  PostgreSQL 16 — verify the instance engine version.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL (PostgreSQL 16), secrets, GCS bucket, and runs DB init |
| 2 — Access & verify | Manual | Health check passes at `/api/version`; first user becomes super-admin |
