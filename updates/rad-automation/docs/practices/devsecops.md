...
## IAM and developer role model

Three developer personas are defined in
[`iam_permissions.tf`](../../rad-ui/automation/terraform/infrastructure/iam_permissions.tf),
each scoped to the minimum permissions required for their role:

| Persona | Access level |
| --- | --- |
| Frontend developer | Read-only GCP access; Firebase and Firestore read |
| Backend developer | Cloud Run invoker; Firestore read/write; Secret Manager accessor |
| Infrastructure engineer | Terraform plan/apply; full IaC stack; service account management |

**Known gap:** one IAM binding in `iam_permissions.tf` grants
`storage.objectAdmin` at the project level rather than on a specific
bucket. This is marked as a TODO and should be narrowed to the target
bucket before it is used in a multi-tenant context.
...
**Known limitation:** secrets are currently configured with user-managed,
single-region replication. Switching to automatic or multi-region
replication is a tracked TODO that would improve availability in the event
...
PRs on high/critical findings. See [CI/CD](./cicd.md) for the full
gate breakdown including which steps are blocking vs. informational.

**SAST / DAST gap:** automated static analysis (e.g., Semgrep, CodeQL)
and dynamic application security testing are not currently integrated into
the pipeline. `npm audit` is the only automated vulnerability scan at
present. Adding a SAST step to `webapp-pr-check.yml` is the recommended
next control.
