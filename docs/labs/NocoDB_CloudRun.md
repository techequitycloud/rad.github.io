...
description: "Hands-on lab: deploy NocoDB on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
operational lifecycle of the **NocoDB on Cloud Run** module on Google Cloud: deploy
...
not on NocoDB product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **NocoDB (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
   inputs. Configure only what you need — the
   [Configuration Guide](https://docs.radmodules.dev/docs/modules/NocoDB_CloudRun)
   documents every input by group, with defaults. Review the estimated cost (if credits are enabled) and click **Deploy**, which opens the deployment status page with real-time logs.

2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL) database
   with its Secret Manager secrets, a GCS uploads bucket, builds the container image,
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~nocodb" --format="value(metadata.name)" --limit=1)
...
2. Navigate to `$SERVICE_URL` in a browser. On the first visit NocoDB displays a
   sign-up form — create the initial admin account here. NocoDB manages its own
   admin credentials; there is no admin password stored in Secret Manager.
...
4. **Manage secrets, backups, and jobs:**
   ...
   gcloud run jobs list --project="$PROJECT" --region="$REGION"   # db-init and scheduled backup jobs
   ```
...
2. **Monitoring** — open the Cloud Run dashboard for the service and review request
   ...
   `/api/v1/health`; confirm it is green under Monitoring → Uptime checks, and review
   Alerting → Policies.
...
- **Session invalidation across instances:** if multiple instances are running without
  Redis, users may be logged out when requests route to a different instance. Enable
  `enable_redis` and supply a `redis_host` before scaling above one instance.
...
| 2 — Access & verify | Manual | Health check passes; create the initial admin account in the browser |
| 3 — Operate | Manual | Inspect revisions, scale, update version, manage secrets/backups, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose revision, database, init-job, build, IAM, and multi-instance session issues |
