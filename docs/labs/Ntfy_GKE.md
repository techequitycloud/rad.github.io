...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Ntfy (GKE)** from
   ...
   `tenant_id` internally, so it can coexist with `Ntfy_CloudRun` on the
