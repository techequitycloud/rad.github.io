...
2. The platform provisions the **backend** as the main Deployment/Service, the
   ...
   **reserved static external LoadBalancer IP**, a Cloud SQL (PostgreSQL 15)
   ...
   provisioning dominate). The frontend image is prebuilt; the backend image is built
   by Cloud Build from `SparkyFitness_Common/scripts/Dockerfile` (a thin SSL-patched
   wrapper).
...
deployment). Delete removes everything the module created — both Deployments/Services,
