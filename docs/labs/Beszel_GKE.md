...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Beszel (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
1. Confirm the pod is running and find how the Service is exposed. Beszel defaults
   to `LoadBalancer`, so an external IP is provisioned out of the box (it can take a
   minute or two to be assigned); a custom domain is an additional, opt-in path:
