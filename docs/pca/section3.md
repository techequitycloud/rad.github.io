...
📌 **Case study connection — EHR Healthcare:** Section 3 topics (IAM, data encryption, VPC Service Controls, compliance) map most directly to the EHR Healthcare case study. EHR Healthcare manages electronic health records under strict HIPAA, data sovereignty, and audit requirements — meaning every design decision must consider access control, encryption at rest and in transit, network boundary enforcement, and audit trail completeness. As you explore this section, consider how you would configure the GCP Services `enable_vpc_sc`, `enable_cmek`, and `enable_security_command_center` variables to meet EHR's compliance posture.
