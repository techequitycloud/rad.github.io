...
> **This module has been retired and is no longer in the RAD catalogue.** Meilisearch keeps its index in LMDB, which holds an exclusive lock for the life of the process; Cloud Run keeps the outgoing revision warm after an update, so every later revision fails its startup probe. Use the [Meilisearch on GKE lab](https://docs.radmodules.dev/docs/labs/Meilisearch_GKE) instead. The steps below are kept for reference only.
