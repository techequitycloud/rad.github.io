---
title: "Mattermost on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Mattermost on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Mattermost on GKE Autopilot — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Mattermost_GKE)**

## Overview

**Estimated time:** 45–90 minutes

Mattermost is an open-source, self-hosted team messaging and collaboration platform.
This lab takes you through the full operational lifecycle of the **Mattermost on GKE
...
Mattermost product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Mattermost_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Mattermost (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL) database with its Secret Manager secrets, optional GCS FUSE
   storage and Redis, builds the container image, and runs a one-shot
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep mattermost | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}/api/v4/system/ping"   # expect {"status":"OK",...}
   ```

2. Open `http://${EXTERNAL_IP}` in a browser. Mattermost prompts you to create the
   first team and set up an admin account on first visit. The database password used
   by the application is stored in Secret Manager and can be inspected if needed:
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. mattermostdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^mattermost" --limit=1)
...
platform-level diagnostics and do not change with Mattermost releases.
...
- **WebSocket disconnections:** Mattermost maintains persistent WebSocket connections;
  ensure `min_instance_count` is at least `1` and Redis is enabled if running
  multiple replicas.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (PostgreSQL), secrets, GCS storage, and runs DB init |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; Mattermost setup wizard accessible |
| 3 — Operate | Manual | Inspect workload, scale, update version, manage secrets/storage/jobs, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose pod, database, init-job, scheduling, WebSocket, and image-pull issues |
