...
*   **Security**: Four application-level secrets are auto-generated by `AnythingLLM Common` — `JWT_SECRET`, `AUTH_TOKEN`, `SIG_KEY`, and `SIG_SALT` — stored in Secret Manager and accessed via Workload Identity. Inherits Cloud Armor, IAP, Binary Authorization, and VPC Service Controls from `App GKE`.
