...
| Auto-generated secrets | Generates and stores `CRAWL4AI_API_TOKEN` (**required** — without it the image binds Gunicorn to loopback only, so probes/traffic never reach it) and `SECRET_KEY` (pins JWT signing across restarts) in Secret Manager | `secret_ids` output; inject LLM provider keys separately via `secret_environment_variables` |
...
| `CRAWL4AI_API_TOKEN` | auto-generated by `Crawl4AI_Common` | Bearer token gating the API and required for the server to bind publicly at all — retrieve it from Secret Manager rather than setting your own |
| `SECRET_KEY` | auto-generated by `Crawl4AI_Common` | JWT signing secret, pinned so issued tokens survive restarts — no longer needs to be set manually via `secret_environment_variables` |
