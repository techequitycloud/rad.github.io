# PCD Certification Preparation Guide: Section 3 — Deploying applications (~24% of the exam)
...
*   **Traffic Ingress:** The `enable_cloud_armor` variable (Group 16 for App CloudRun; §4.A Cloud Armor WAF for App GKE) provisions a Global External Application Load Balancer with a global anycast IP, Cloud Armor WAF rules, and CDN capability — without it, the service is accessible only via the regional Cloud Run service URL or the internal GKE cluster Gateway.
...
**In the RAD UI:**
*   **Autoscaling:** The `enable_vertical_pod_autoscaling` variable (§3.A Compute & Scaling in App GKE) enables **Vertical Pod Autoscaler (VPA)**, which analyses historical CPU and memory usage and automatically adjusts the pod's resource requests over time — ensuring pods have sufficient resources without manual right-sizing. Separately, `min_instance_count` and `max_instance_count` (§3.A) configure the **Horizontal Pod Autoscaler (HPA)**, which scales the number of pod replicas up or down based on CPU/memory utilisation metrics. VPA and HPA address different scaling dimensions: VPA adjusts *what each pod gets*; HPA adjusts *how many pods run*.
*   **Health Checks:** The `health_check_config` variable (Group 5 for App CloudRun; §3.A Compute & Scaling for App GKE) automatically injects Kubernetes **Readiness** and **Liveness** probes into the Deployment manifest. Understanding the difference is critical for the PCD exam:
    ...
    - **Startup probe:** An optional third probe type that delays liveness/readiness checking until the application has successfully started. Use this for applications with slow initialisation (e.g., Java services) to prevent the liveness probe from killing the container before it has finished starting. In App CloudRun, the startup probe is configured separately via `startup_probe_config` (Group 5) rather than `health_check_config` — this separation is intentional because startup probes require different timing parameters (higher `initial_delay_seconds`, more `failure_threshold` attempts) than the steady-state liveness and readiness probes. Mixing startup and liveness probe configuration in a single variable leads to either premature container restarts (liveness timeout too short) or delayed failure detection (liveness timeout too long). In App GKE, all three probe types are configured via `health_check_config` (§3.A) with separate `startup`, `liveness`, and `readiness` sub-fields.
...
*   **Pod Disruption Budgets (PDB):** App GKE provisions PDBs via `pdb_min_available` (§7.A Reliability & Scheduling), which accepts an integer (e.g., `"1"` — keep at least one pod alive) or a percentage (e.g., `"50%"` — keep at least half the fleet alive) during voluntary disruptions such as node upgrades, cluster autoscaler scale-down, or `kubectl drain`. Without a PDB, GKE Autopilot can evict all pods from a node simultaneously during an upgrade, causing a complete service outage. PDBs are a required component of any high-availability GKE deployment. Navigate to `kubectl get poddisruptionbudgets -n [namespace]` in Cloud Shell to inspect the active PDB for a deployed workload.
*   **Topology Spread Constraints:** App GKE supports topology spread via `enable_topology_spread` and `topology_spread_strict` (§7.B Reliability & Scheduling). When enabled, the module injects a Kubernetes `topologySpreadConstraint` into the pod spec requiring replicas to be distributed across zones (`topologyKey: topology.kubernetes.io/zone`). `topology_spread_strict` = `true` uses `whenUnsatisfiable: DoNotSchedule` (hard constraint — pods are not scheduled if they cannot be spread correctly) versus `false` which uses `whenUnsatisfiable: ScheduleAnyway` (soft constraint — the scheduler attempts to spread but will schedule even if it cannot). In GKE Autopilot, zone-level topology spread ensures that a single zone failure does not take down all replicas simultaneously.
*   **Resource Quotas:** App GKE provisions Kubernetes ResourceQuota objects per namespace via `enable_resource_quota` (§7.C Reliability & Scheduling). The quota variables (`quota_cpu_requests`, `quota_cpu_limits`, `quota_memory_requests`, `quota_memory_limits`, `quota_max_pods`, `quota_max_pvcs`) define aggregate ceilings for all pods in the namespace. ResourceQuotas are a multi-tenant governance control: they prevent a single application from consuming all cluster capacity, and Autopilot enforces them at pod admission time. If a pod's resource request exceeds the namespace quota, it is rejected immediately — providing budget guardrails for teams sharing a cluster.
*   **Additional Services as Independent Deployments:** App GKE supports the `additional_services` variable (§3.F Core Services) to deploy multiple microservices alongside the primary application — each with its own `container_image`, `container_resources`, `min_instance_count`, `max_instance_count`, and HPA configuration. Each additional service receives its own Kubernetes Deployment, Service, and HPA object but shares the same namespace, VPC, and database credentials as the primary service. The `output_env_var_name` field injects the additional service's internal ClusterIP into the primary container's environment, enabling service discovery without hardcoded IPs. Similarly, App CloudRun's `additional_services` (Group 6) deploys independent Cloud Run services with `INGRESS_TRAFFIC_INTERNAL_ONLY` — accessible only from within the VPC via Direct VPC Egress, not from the internet.
*   **Multi-Cluster Services (MCS):** App GKE supports `enable_multi_cluster_service` (§8.D Integrations) to enable the GKE Multi-Cluster Services API (Fleet-based service discovery). When enabled, the module registers the GKE cluster with the GKE Fleet Hub and creates a `ServiceExport` object for the application service. This makes the service discoverable from other clusters in the same Fleet via a stable DNS name: `<service>.<namespace>.svc.clusterset.local`. MCS is the foundation of active-active multi-cluster architectures where applications in separate clusters (or separate regions) can call each other's services without external load balancer hops. Navigate to **Kubernetes Engine > Fleet** to view registered clusters and exported services.
*   **Cluster Selection Modes:** App GKE supports three cluster selection modes via `gke_cluster_selection_mode` (§3.D Networking): `primary` deploys to the single cluster specified by `cluster_name`; `explicit` deploys to all clusters listed in `cluster_names` (for deterministic multi-cluster deployment); and `round-robin` distributes deployments across all available clusters in the Fleet, providing automatic load distribution. These modes are used to deploy workloads to specific cluster combinations — for example, deploying a stateful service to `primary` only while deploying a stateless API to `round-robin` across all regional clusters.
*   **Cloud Run Ingress Settings:** The `ingress_settings` variable (Group 14 in App CloudRun) must be set to `internal-and-cloud-load-balancing` when `enable_cloud_armor = true` — without this, the Cloud Run service URL is accessible directly from the internet, bypassing the Cloud Armor WAF entirely. This is a common misconfiguration that creates a false security posture. The `vpc_egress_setting` (Group 14) should be set to `ALL_TRAFFIC` when all outbound traffic (including calls to external APIs) must traverse the VPC for NAT logging and firewall inspection, or `PRIVATE_RANGES_ONLY` when only RFC 1918 traffic needs VPC routing.
*   **Inline Infrastructure for Ephemeral Environments:** App GKE's Inline Infrastructure mode (§9) self-provisions VPC, GKE Autopilot cluster, Cloud Filestore NFS, and Cloud SQL within the module's own Terraform state using `prereq_needs_*` flags. This enables PR-based ephemeral environments — Cloud Build triggers deploy a complete environment for each pull request and destroy it on merge. The module manages the complete lifecycle without requiring a separate GCP Services deployment.
