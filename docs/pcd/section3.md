...
*   **Traffic Ingress:** The `enable_cloud_armor` variable (Group 16) provisions a Global External Application Load Balancer with a global anycast IP, Cloud Armor WAF, and optional CDN in front of the Cloud Run service. When `enable_cloud_armor` is `false`, the service is accessed directly via its `*.run.app` URL. Set `ingress_settings` (Group 14) to `internal-and-cloud-load-balancing` so the Cloud Run service only accepts traffic that has passed through the load balancer — preventing direct access that bypasses Cloud Armor rules.
...
**In the RAD UI:**
*   **Workload Provisioning:** The `deploy_application` variable (Group 3) triggers the deployment of the application to the GKE Autopilot cluster. The `workload_type` variable (Group 5) controls whether this creates a Kubernetes `Deployment` (default, recommended for stateless workloads) or a `StatefulSet` (required for workloads needing stable pod identities, ordered startup/shutdown, or per-pod persistent storage — for example message brokers or applications relying on leader election by hostname). For the PCD exam: use Deployments for web APIs and microservices that store state externally in Cloud SQL or GCS; use StatefulSets only when the application itself requires per-pod identity.
...
*   **Health Checks:** The `health_check_config` variable (Group 13 for GKE, Group 5 for Cloud Run) automatically injects Kubernetes **Readiness** and **Liveness** probes into the Deployment manifest. Understanding the difference is critical for the PCD exam:
...
*   **Zero-downtime deployments with PodDisruptionBudgets:** Rolling updates, node drains, and cluster upgrades can silently take down all replicas of a small Deployment simultaneously unless a `PodDisruptionBudget` (PDB) is in place. Enable `enable_pod_disruption_budget = true` (Group 14) and set `pdb_min_available = "1"` to guarantee at least one pod is always serving during any voluntary disruption. For higher-traffic services, use a percentage such as `"75%"` to maintain throughput. Navigate to **Kubernetes Engine > Config & Storage** to view the PDB object and confirm its current status, or run `kubectl describe pdb -n NAMESPACE` to see how many disruptions are currently permitted.

*   **Service Mesh integration for GKE:** Enabling `configure_service_mesh = true` (Group 5) automatically injects the Istio proxy sidecar into every pod in the application namespace. This provides mutual TLS between all services (mTLS), fine-grained traffic management (retries, circuit breakers, traffic shifting), and distributed tracing — all without application code changes. Navigate to **Kubernetes Engine > Service Mesh** to view the mesh topology and per-service traffic health. This maps directly to the PCD exam objective on securing service-to-service communication: instead of implementing mTLS in application code, the mesh sidecar handles it transparently.
