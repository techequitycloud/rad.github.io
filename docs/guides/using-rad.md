...
- **Delete** — choose **Delete** to tear down the cloud resources, or **Purge** to remove the deployment from RAD *without* destroying the cloud resources (useful when a deployment is stuck or was changed outside RAD). A deployment that belongs to a solution can be deleted or purged only from the solution's page. A RAD-managed GCP project ("GCP Project on RAD") is torn down the same way, but because that takes the whole project with it, RAD refuses while any other deployment is still running in that project and lists the ones to delete first. Google keeps a deleted project recoverable for about 30 days.
- **Cancel** — releases a deployment that is still Queued or Waiting and has not started building, or a purge that has stalled.
...
Whether anything can be bought at all is one platform switch, **Enable Subscription**. While it is off there are no new subscriptions and no one-off top-ups on either provider: the **Subscriptions** and **Buy Credits** tabs are not shown and the public pricing page lists no plans. If you already hold a subscription, a notice on **Credit Transactions** still lets you cancel it.

### ROI

The **Calculate ROI** tab on the **Credits** page is an interactive estimator. It comes pre-filled with your recent activity and lets you adjust assumptions (such as monthly deployments, manual deployment time, engineer hourly cost, and time-savings percentage) to estimate your labour cost, platform cost, net savings, and ROI. It's an estimator only — it doesn't deploy or charge anything.
