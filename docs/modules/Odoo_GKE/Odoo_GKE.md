...
<YouTubeEmbed videoId="cAChBJgmmLI" poster="https://storage.googleapis.com/rad-public-2b65/modules/Odoo_GKE.png" />
...
| **ODOO_MASTER_PASS secret** | A 16-character alphanumeric master password is auto-generated by `Odoo_Common` and stored in Secret Manager under the name `app{application_name}{tenant_deployment_id}{random_hex}-master-password` (where `random_hex` is an internally-generated deployment suffix, not the user-supplied `deployment_id`). The plaintext value is passed to `App_GKE` via `explicit_secret_values`, which writes it into a Kubernetes Secret and injects it as the `ODOO_MASTER_PASS` environment variable. |
...
The backup import variables behave identically to those in App_GKE §8.B:
