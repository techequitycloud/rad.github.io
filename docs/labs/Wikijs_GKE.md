---
title: "Wiki.js on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Wiki.js on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Wiki.js on GKE Autopilot — Lab Guide
...
Wiki.js is an open-source wiki platform that delivers Git-backed version control and a
clean writing experience for teams. This lab takes you through the full operational
lifecycle of the **Wiki.js on GKE Autopilot** module on Google Cloud: deploy it,
...
Wiki.js product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Wikijs_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Wiki.js (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL 15) database with the `pg_trgm` extension and its Secret
   Manager secrets, optional NFS/GCS storage, builds the container image, and runs a
   one-shot database-initialisation job. First deploys take roughly **20–30 minutes**
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep wikijs | head -1 | cut -d/ -f2)
...
2. Check the health endpoint (Wiki.js binds on port 3000 and exposes `/healthz`):

   ```bash
   curl -s -o /dev/null -w "%{http_code}\n" "http://${EXTERNAL_IP}/healthz"   # expect 200
   ```

3. Open `http://${EXTERNAL_IP}` in a browser. On first run Wiki.js presents a **setup
   wizard** — enter a site title, admin email, and admin password to complete
   installation. On subsequent visits the login page is shown directly.
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. wikijsdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^wikijs" --limit=1)
...
platform-level diagnostics and do not change with Wiki.js releases.

- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. Wiki.js has a
  60-second startup probe initial delay to allow the first-boot PostgreSQL schema
  migration to complete — premature probe failures are usually caused by reducing
  this delay.
  ```bash
...
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret materialised into the namespace, the `pg_trgm` extension was
  installed, and the init job completed.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (PostgreSQL 15 + pg_trgm), secrets, and runs DB init |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes at `/healthz`; complete setup wizard or sign in |
