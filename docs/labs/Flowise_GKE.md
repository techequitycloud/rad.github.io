...
description: "Hands-on lab: deploy Flowise on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
Flowise is an open-source visual AI workflow builder that lets users assemble
LangChain and LlamaIndex AI pipelines through a drag-and-drop interface. This lab
takes you through the full operational lifecycle of the **Flowise on GKE Autopilot**
...
Flowise product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Flowise_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Flowise (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL 15) database with its Secret Manager secrets, a GCS uploads
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep flowise | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}:3000/api/v1/ping"   # expect {"pong":true}
   ```
...
2. Retrieve the admin password from Secret Manager and sign in at
   ...
   The admin username defaults to the value of `flowise_username` set at deploy time
   (default: `admin`).
...
platform-level diagnostics and do not change with Flowise releases.
...
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; retrieve admin password and sign in |
