...
*   **Security**: `APP_SECRET` (exposed as `UMAMI_APP_SECRET`) auto-generated by `Umami Common` and stored in Secret Manager. Inherits Cloud Armor WAF, IAP, Binary Authorization, and VPC Service Controls from `App CloudRun`.
...
**Wrapper architecture:** `Umami CloudRun` calls `Umami Common` to build an `application_config` object containing Umami-specific environment variables, probe configuration, and the `db-init` job definition. `module_secret_env_vars` carries `UMAMI_APP_SECRET` from `Umami Common`. The `DATABASE_URL` is constructed at runtime from the DB_* platform variables injected by `App CloudRun`. `scripts_dir` is resolved to `abspath("${module.umami_app.path}/scripts")` at apply time.
...
`Umami_CloudRun` delegates all IAM provisioning to `App_CloudRun`. The Cloud Run SA, Cloud Build SA, IAP service agent, and password rotation role sets are identical to those in [App_CloudRun](App_CloudRun.md).

**Auto-generated application secret:** `Umami Common` generates `APP_SECRET` at apply time using `random_password` (32 characters, no special characters). It is written to Secret Manager and injected into the Cloud Run container as the `UMAMI_APP_SECRET` environment variable. Plaintext is never stored in Terraform state after initial apply.
...
For the complete role tables and IAP, password rotation, and public access details, see [App_CloudRun](App_CloudRun.md).
...
| `cpu_always_allocated` | 4 | `true` | When `true`, CPU is allocated at all times. Set `false` to throttle between requests for low-traffic deployments. |
...
| `enable_image_mirroring` | `false` | `true` | Mirrors from GitHub Container Registry to avoid rate limits. |
...
See [App_CloudRun](App_CloudRun.md) for full architecture details.
...
| `uptime_check_config` | 14 | `{ enabled=true, path="/api/heartbeat" }` | Cloud Monitoring uptime check. Alerts notify `support_users` if unreachable. |
...
| Variable | Group | Default | Description |
|---|---|---|---|
| `uptime_check_config` | 14 | `{ enabled=true, path="/api/heartbeat" }` | Uptime check configuration: `enabled`, `path`, `check_interval`, `timeout`. |
...
**Secret Manager:**
Navigate to **Security → Secret Manager**. Find secrets named `<application_name>-<deployment_id>-app-secret` (or similar). Click a secret to view:
...
Navigate to **Monitoring → Uptime checks**. Find the check for this Umami deployment targeting `/api/heartbeat`. View check results, response time, and failure history across GCP regions.
...
| **APP_SECRET auto-generated** | `random_password` in `Umami Common` | 32-character alphanumeric secret, stored in Secret Manager, injected as `UMAMI_APP_SECRET`. |
...
| **No Redis required** | `enable_redis = false` default | Unlike Ghost or Directus, Umami uses only PostgreSQL for all data storage. Redis is not required. |
...
**Inline infrastructure** (when no `Services_GCP` stack is present) is identical to `App_CloudRun` §9 — `App_CloudRun` provisions an inline VPC, Cloud NAT, Cloud SQL instance, service accounts, and GCP APIs as required. See [App_CloudRun](App_CloudRun.md) for the full inline resource inventory and teardown notes.
...
| `timeout_seconds` | 4 | `300` | Max request duration. |
| `cpu_always_allocated` | 4 | `true` | Allocate CPU at all times (vs throttle between requests). |
...
| `health_check_config` | 14 | `{ enabled=true, path="/api/heartbeat" }` | Cloud Run liveness probe (passed directly to `App CloudRun`). |
| `uptime_check_config` | 14 | `{ enabled=true, path="/api/heartbeat" }` | Cloud Monitoring uptime check. |
| `alert_policies` | 14 | `[]` | Cloud Monitoring metric alert policies. |
