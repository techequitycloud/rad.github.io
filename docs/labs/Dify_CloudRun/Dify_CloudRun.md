...
**Expected result:** Service status shows `Ready` with the container image, resource limits, environment variables, and VPC connector details.
...
3. Click **Setup**.
4. Dify redirects to the login page.

**Expected result:** You are redirected to the Dify login page. Log in with the credentials you just created.

### Step 3.3 — Verify SECRET_KEY Secret

The `SECRET_KEY` is auto-generated by the module and stored in Secret Manager:
...
**Expected result:** The secret has 64 characters. This key is injected into the container as the `SECRET_KEY` environment variable.

---

## Phase 4 — Configure LLM Providers [MANUAL]

### Step 4.1 — Add an OpenAI API Key
...
**Expected result:** The OpenAI provider appears in the model list with status `Active`.

### Step 4.2 — Verify pgvector Integration

Dify uses pgvector (the `vector` PostgreSQL extension) as the default vector store. Verify the extension is enabled:
...
**Expected result:** The query returns `vector` with a version number (e.g., `0.7.0`).
...
3. Enter a name (e.g., "My Assistant").
4. Click **Create**.
5. In the application editor, select a model (e.g., `gpt-4o-mini`) from the model dropdown.
6. Enter a system prompt and click **Publish**.

**Expected result:** The chatbot is published. Click **Run** to open the chatbot interface in your browser.

### Step 5.2 — Create a Knowledge Base (RAG)

1. Navigate to **Knowledge** in the left sidebar.
2. Click **Create Knowledge**.
3. Upload a text document or enter a URL.
4. Select **Automatic** chunking mode and click **Save & Process**.

**Expected result:** Dify processes the document, chunks it, generates embeddings, and stores them in the pgvector store. The knowledge base shows `Available` status.

### Step 5.3 — Create a Workflow

1. Navigate to **Studio** > **Create** > **Workflow**.
2. Add nodes: **Start** → **LLM** → **End**.
3. Configure the LLM node with a prompt template.
4. Click **Run** to test the workflow.

**Expected result:** The workflow executes and returns LLM output in the preview panel.
...
**Expected result:** Dify startup logs appear, including Flask-Migrate migration output, Celery worker startup, and Redis connection confirmation.
...
**Expected result:** Under normal operation, only informational logs appear. Redis connection warnings may appear briefly during cold starts.
...
> **Warning:** This permanently deletes all resources including the database, vector store data, and GCS file storage. Export your Dify applications and knowledge bases before undeploying: Dify Admin > Settings > Data Backup.
...
| Explore workflows | 5 | No |
| Review Cloud Logging | 6 | No |
| Examine revisions and services | 7 | No |
