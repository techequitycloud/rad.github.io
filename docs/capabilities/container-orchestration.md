...
Three of five platform modules deploy GKE; two register a non-GCP Kubernetes cluster into a GCP fleet. This page covers the Kubernetes-specific configuration common to all of them.
...
GKE clusters use IP alias ranges via `ip_allocation_policy` in `gke.tf`, with secondary ranges defined in `network.tf` for pods and services. See [zero-trust](zero-trust).
