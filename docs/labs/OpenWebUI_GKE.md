---
title: "OpenWebUI on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy OpenWebUI on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# OpenWebUI on GKE Autopilot — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/OpenWebUI_GKE)**

## Overview

**Estimated time:** 45–90 minutes

Open WebUI is an open-source, self-hosted AI interface that provides a polished
ChatGPT-style frontend for Ollama, OpenAI-compatible APIs, and dozens of other LLM
providers. This lab takes you through the full operational lifecycle of the
**Open WebUI on GKE Autopilot** module on Google Cloud: deploy it, access and verify
...
Open WebUI product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/OpenWebUI_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Open WebUI (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL 15) database with its Secret Manager secrets, an optional
   NFS volume, a GCS data bucket, builds the container image, and runs a one-shot
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep openwebui | head -1 | cut -d/ -f2)
...
2. Navigate to `http://${EXTERNAL_IP}` in a browser. Open WebUI presents a sign-up
   page on first visit — the first account registered automatically becomes the admin.
   There is no pre-set admin password; the platform stores only the auto-generated
   `WEBUI_SECRET_KEY` (a session-signing key) in Secret Manager. You can inspect it
   with:

   ```bash
   SECRET=$(gcloud secrets list --project="$PROJECT" \
     --filter="name~openwebui AND name~secret-key" --format="value(name)" --limit=1)
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. openwebuidemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^openwebui" --limit=1)
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   ...
   **uptime check** targeting `/health` (when enabled); review Monitoring → Uptime
...
platform-level diagnostics and do not change with Open WebUI releases.
...
and namespace, Cloud SQL database, Secret Manager secrets, GCS data bucket, and
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (PostgreSQL 15), secrets, GCS bucket, and runs DB init |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; first user registers as admin |
