...
> This guide documents variables that are **unique to `N8N CloudRun`** or that have
...
- **Redis** enabled by default (`enable_redis = true`) for n8n queue mode. When
  ...
  is resolved by App CloudRun at container startup, not pre-computed at Terraform apply time.
...
- **`N8N_ENCRYPTION_KEY`** and **`N8N_SMTP_PASS`** auto-generated by `N8N Common` and stored
...
> **`N8N CloudRun` vs `N8N AI CloudRun`:** This module is the standard n8n workflow
> automation deployment. It does not include Qdrant vector database or Ollama LLM server.
> Use `N8N AI CloudRun` when you need the full AI stack (RAG pipelines, local LLMs).
