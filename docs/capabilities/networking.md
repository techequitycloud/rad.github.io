...
- **Inline VPC CIDR derivation** — `modules/App_GKE/prerequisites.tf` derives unique CIDRs from `sha256(prereq_suffix)`. Override variables (`prereq_subnet_cidr_override`, `prereq_gke_pod_cidr_override`, `prereq_gke_service_cidr_override`) pin existing CIDRs. Multi-tenancy implications in [multitenancy-saas](multitenancy-saas).
...
This pattern is the foundation for HA / DR (see [disaster-recovery](disaster-recovery)) and multicloud extension (see [multicloud](multicloud)).
...
- **Observability integration** — automatic L7 metrics (request rate, latency, error rate) and distributed tracing for enrolled workloads without application instrumentation. Detailed in [observability](observability) §7.
- Multi-primary mesh runs an Istio control plane per cluster for resilience.

### 8. Edge security (cross-ref)

IAP, VPC-SC, Workload Identity — the identity-layer view of edge controls. Canonical in [practices/devsecops.md](../practices/devsecops.md). Binary Authorization (image attestation enforcement at deploy time) is also documented there and cross-referenced from [serverless](serverless) §5.
...
- [multitenancy-saas](multitenancy-saas) — inline CIDR derivation and per-tenant perimeter strategy
