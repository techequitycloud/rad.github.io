# Professional Cloud Architect Certification Exploration Guide

This document provides a hands-on exploration guide for configuring the variables defined in `modules/App_CloudRun/variables.tf` and `modules/App_GKE/variables.tf`. It is designed to help candidates preparing for the Google Cloud Professional Cloud Architect (PCA) certification experience the core concepts outlined in the exam guide by interacting directly with the provisioned resources.
...
*   **Customization Suggestion**: Try modifying the Cloud Armor rules in `security.tf` to block a specific IP address block and attempt to access the application to see the 403 response.
...
*   **Customization Suggestion**: Deploy two instances of the application: one with `min_instance_count = 1` and another with `0`. Use the GCP Pricing Calculator to compare estimated monthly costs.
...
*   **Customization Suggestion**: Introduce an artificial error in the application (if possible) and observe the alert triggering in the Console and arriving via email.
...
*   **Customization Suggestion**: Add multiple regions to a Cloud Run deployment and attach them to the same backend service to observe global traffic routing.
...
*   **Customization Suggestion**: Use a load testing tool (like Apache Bench or hey) to generate traffic and watch the GKE pods or Cloud Run instances scale out in real-time on the monitoring dashboards.
...
*   **Customization Suggestion**: Manually trigger a backup job from the Console and verify the resulting `.sql` or `.dump` file appears in the designated Cloud Storage bucket.
...
*   **Customization Suggestion**: Attempt to ping a database from an unauthorized GKE namespace to verify the Dataplane V2 network policies drop the traffic.
...
*   **Customization Suggestion**: Mount the NFS Filestore instance to multiple pods in GKE and write a file from one pod to verify it is immediately readable by the other.
...
*   **Customization Suggestion**: Try deploying a stateful application requiring persistent volume claims (PVCs) and evaluate why GKE is a better fit than Cloud Run.
...
*   **Customization Suggestion**: Enable Cloud CDN on the load balancer backend and observe cache hit ratios in **Network Services > Cloud CDN** during a load test.
...
*   **Customization Suggestion**: Set a lifecycle rule to transition objects to `COLDLINE` storage after 30 days. Upload a test object and verify the metadata reflects the upcoming transition.
...
*   **Customization Suggestion**: Intentionally delete a managed resource (like a Cloud Scheduler job) via the Console, then run `terraform apply` again to see IaC drift detection and remediation in action.
...
*   **Customization Suggestion**: Provision a GKE node pool with GPUs and deploy an application that leverages those accelerators for inference tasks.
...
*   **Customization Suggestion**: Update the secret payload in Secret Manager and restart the Cloud Run revision or GKE pod to verify it picks up the new credentials without changing the application code.
...
*   **Customization Suggestion**: Attempt to deploy an unsigned container image when Binary Authorization is enabled to observe the deployment blockage in the Console logs.
...
*   **Customization Suggestion**: Commit a small code change to trigger the pipeline and monitor its progress across staging and production environments through the Cloud Deploy console.
...
*   **Customization Suggestion**: Export the current Terraform state and analyze the JSON structure to understand how Terraform maps high-level variable configurations to low-level GCP API properties.
...
*   **Customization Suggestion**: Create a custom Log-based Metric for a specific application log payload, and then build a custom alert policy on top of that new metric in the Console.
