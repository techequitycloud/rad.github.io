---
title: "Strapi GKE Module — Configuration Guide"
...
| Storage — GCS | [App_GKE §3.C Storage (NFS / GCS / GCS Fuse)](../App_GKE/App_GKE.md#c-storage-nfs--gcs--gcs-fuse) | A default `data` bucket is provisioned. Refer to App GKE module documentation for bucket configuration. |
...
|---|---|---|---|
| `APP_KEYS` (auto-generated) | Auto-generated four-key set stored in Secret Manager | **Critical** | Signs all Strapi session cookies. Changing after first deploy immediately invalidates all active admin and user sessions. All users are logged out and client apps using session-based auth fail. |
| `ADMIN_JWT_SECRET` (auto-generated) | Auto-generated and stored in Secret Manager | **Critical** | Signs all admin panel JWT tokens. Changing it invalidates all admin tokens — every admin user is immediately logged out and all in-flight admin API calls fail with 401. |
| `JWT_SECRET` (auto-generated) | Auto-generated and stored in Secret Manager | **Critical** | Signs all API user JWT tokens. Changing it after first deploy immediately breaks every issued API token — all user sessions terminate and all client apps fail with 401. |
| `API_TOKEN_SALT` (auto-generated) | Auto-generated and stored in Secret Manager | **Critical** | Hashes all API tokens. Changing it permanently breaks every existing API token — they cannot be validated and must be regenerated by all API consumers. |
| `TRANSFER_TOKEN_SALT` (auto-generated) | Auto-generated and stored in Secret Manager | **High** | Hashes data transfer tokens. Changing it invalidates all existing transfer tokens, breaking scheduled or in-progress data migration operations. |
| `application_name` | `"strapi"` | **Critical** | Immutable after first deploy. Changing it renames all GCP and Kubernetes resources, triggering full recreation and complete data loss. |
| `db_name` | `"strapi"` | **Critical** | Immutable after first deploy. Changing it causes Strapi to connect to a new empty database, losing all content types, content data, users, and API configurations. |
| `enable_redis` | `false` | **Medium** | Only enable Redis if using plugins that explicitly require it. Enabling without a valid `redis_host` causes a startup failure — the Redis connection string will be empty. Validated by the Strapi GKE validation guard: "When enable_redis is true, either redis_host must be set or enable_nfs must be true." |
| `redis_host` | `null` (uses NFS server IP when `enable_nfs = true`) | **High** | When `enable_redis = true`, `redis_host` is null/empty, and `enable_nfs = false`, the GKE validation guard fires a blocking error at plan time. |
| `memory_limit` | `"512Mi"` | **High** | Strapi is a Node.js application. Values below `512Mi` cause OOM kills during admin panel operations. Increase to `1Gi` for deployments with custom plugins or complex content types. Under Kubernetes, OOM-killed pods trigger CrashLoopBackOff if limits are consistently too low. |
| `min_instance_count` | `1` | **High** | GKE does not support true scale-to-zero without KEDA. Setting to `0` may leave the HPA in an inconsistent state. The validation guard rejects `min > max`. |
| `workload_type` | `null` (defaults to `Deployment`) | **High** | Setting `stateful_pvc_enabled = true` without an explicit `workload_type` automatically resolves to `StatefulSet`. Setting `workload_type = "Deployment"` alongside `stateful_pvc_enabled = true` fails at plan time. |
| `stateful_pvc_size` | `"10Gi"` | **Medium** | PVC size is immutable after creation in most Kubernetes storage classes. Plan capacity in advance — resizing requires manual volume expansion or data migration. |
| `quota_memory_requests` / `quota_memory_limits` | `""` (not enforced) | **High** | Must use binary suffixes (`"512Mi"`, `"1Gi"`). Bare integers are treated as bytes by Kubernetes and block all pod scheduling in the namespace. |
| `enable_cloudsql_volume` | `true` | **High** | Required for PostgreSQL connectivity in GKE. Setting to `false` while `database_type != "NONE"` is blocked at plan time by the GKE validation guard. |
| `enable_nfs` | `true` | **High** | Strapi stores uploaded media files in its `/uploads` directory. Without NFS or GCS, all uploaded assets are stored on ephemeral pod storage and lost on pod restart or rescheduling. |
...
| `session_affinity` | `"ClientIP"` | **Medium** | Strapi's admin panel uses persistent WebSocket connections for real-time content updates. Disabling affinity (`"None"`) can cause admin panel disconnections and "unsaved changes" warnings. |
| `enable_pod_disruption_budget` | `true` | **Medium** | Disabling PDB allows GKE to evict all Strapi pods during node maintenance, causing a complete outage. In-flight content saves may be lost. |
| `termination_grace_period_seconds` | `30` | **Medium** | Node.js needs time to finish handling in-flight HTTP requests before shutdown. Values below `15` may cause request aborts and incomplete content saves from the admin panel. |
| `enable_topology_spread` | `false` | **Low** | With `max_instance_count > 1`, enabling topology spread prevents all Strapi pods from landing on the same node. Recommended for multi-replica production deployments. |
