...
description: "Hands-on lab: deploy Strapi on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
operational lifecycle of the **Strapi on Cloud Run** module on Google Cloud:
...
not on Strapi product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Strapi (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL) database
   with its Secret Manager secrets, a Filestore NFS share for media uploads, builds
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~strapi" --format="value(metadata.name)" --limit=1)
...
1. Confirm the service is healthy and connected to its database:
   ...
   ```bash
   curl -s "${SERVICE_URL}/_health"   # expect HTTP 200
   ```

   The `/_health` endpoint returns 200 only when Strapi and its database connection
   are both ready.

2. Open `${SERVICE_URL}/admin` in a browser. On a fresh deployment Strapi's
   first-run wizard prompts you to create the initial administrator account. Fill in
   the form and save your credentials securely — there is no pre-set admin password
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. strapidemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^strapi" --limit=1)
...
2. **Monitoring** — open the Cloud Run dashboard for the service and review request
   ...
   enabled — `uptime_check_config` defaults to `enabled = false`); if enabled,
...
- **Revision unhealthy / service won't serve:** inspect the latest revision and its
  ...
  probe targets `/_health` and allows up to ~300 seconds for first-boot
  initialisation; a slow Cloud SQL connection is the most common cause of a slow
  start.
  ```bash
  gcloud run revisions list --service="$SERVICE" --project="$PROJECT" --region="$REGION"
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL, NFS, secrets, and runs DB init |
| 2 — Access & verify | Manual | Health check passes; first-run wizard creates admin account |
