...
Cal.diy is the MIT-licensed, self-hostable fork of Cal.com — the scheduling platform used by millions globally to eliminate the back-and-forth of meeting coordination. `Cal CloudRun` is a **wrapper module** built on top of `App CloudRun`. It uses `App CloudRun` for all GCP infrastructure provisioning and injects Cal.diy-specific application configuration, database initialisation, and secret management via `Cal Common`.
...
*   **Security**: `NEXTAUTH_SECRET` and `CALENDSO_ENCRYPTION_KEY` auto-generated by `Cal Common` and stored in Secret Manager. Inherits Cloud Armor WAF, IAP, Binary Authorization, and VPC Service Controls from `App CloudRun`.
...
*   **Reliability**: Health probes target `/api/health`. Cal.diy runs Prisma migrations and seed operations on first boot — probe timings are configured generously by `Cal Common`.
...
| `display_name` | 3 | `string` | `'Cal.com Scheduling'` | Human-readable name shown in the GCP Console. Maps to `application_display_name` in `App CloudRun`. |
| `description` | 3 | `string` | `'Cal.com - Open-source scheduling platform'` | Cloud Run service description. Passed to `Cal Common`. |
...
**30-second secret propagation delay:** `Cal Common` inserts a `time_sleep` of 30 seconds after creating secrets. `Cal CloudRun` allows overriding this via `secret_propagation_delay` (passed to `App CloudRun`), which defaults to 30 seconds.
...
Cal.diy is a Next.js 14 application that runs Prisma database migrations, a seed operation for the app store, and a URL-replacement script (`replace-placeholder.sh`) on every first boot. These operations take 4–5 minutes. `Cal CloudRun` exposes `cpu_limit` and `memory_limit` as dedicated top-level variables.
...
The module uses `db_name` and `db_user` as top-level variables. Passwords are auto-generated by `App CloudRun` and stored in Secret Manager.
...
A default GCS `data` bucket is provisioned by `Cal Common` via `module_storage_buckets`. Additional buckets can be defined via `storage_buckets`.
...
A `db-init` Cloud Run Job is automatically provisioned by `Cal Common` when `initialization_jobs` is left as the default empty list (`[]`). It uses the `postgres:15-alpine` image and executes `Cal_Common/scripts/db-init.sh`, which performs the following idempotent operations:
...
Override `initialization_jobs` with a non-empty list to replace this default with custom jobs. When `initialization_jobs` is non-empty, `Cal Common` does not inject the default `db-init` job.

| Variable | Group | Default | Description |
|---|---|---|---|
| `initialization_jobs` | 13 | `[]` | One-shot Cloud Run Jobs. Leave empty for `Cal Common` to supply the default `db-init` job. |
...
`Cal Common` auto-generates `NEXTAUTH_SECRET` and `CALENDSO_ENCRYPTION_KEY` at apply time.
...
Total first-boot time is typically 4–5 minutes. The startup probe defaults in `Cal Common` allow a 6-minute window (`initial_delay_seconds = 180`, `failure_threshold = 18`, `period_seconds = 10`).
...
**Cal.diy performs database migrations and app store seeding on first boot.** `Cal Common` sets generous probe defaults — the startup probe allows 6 minutes of total startup tolerance.

| Variable | Group | Default (from `Cal Common`) | Description |
|---|---|---|---|
...
**Note:** The `startup_probe` and `liveness_probe` variables declared in `Cal_CloudRun/variables.tf` have shorter defaults than those baked into `Cal Common`. `Cal Common`'s defaults take precedence because probe config is passed directly from `Cal Common.config` to the Foundation Module — the `Cal CloudRun` variable values are only used to override `Cal Common`'s defaults when explicitly set.
...
| `backup_uri` | 7 | `""` | Full GCS URI (e.g., `'gs://my-bucket/calcom-2024-01.sql'`) or Google Drive file ID. Maps to `backup_file` in `App CloudRun`. |
...
The following behaviours are applied automatically by `Cal CloudRun` regardless of variable values. They cannot be overridden via `tfvars`.

| Behaviour | Implementation | Detail |
|---|---|---|
| **PostgreSQL 15 required** | `database_type = "POSTGRES_15"` default | Cal.diy uses Prisma ORM exclusively targeting PostgreSQL. |
| **NEXTAUTH_SECRET auto-generated** | `random_password` in `Cal Common` | 32-character alphanumeric secret, stored in Secret Manager as `<app>-<id>-nextauth-secret`. |
| **CALENDSO_ENCRYPTION_KEY auto-generated** | `random_password` in `Cal Common` | 32-character alphanumeric key, stored in Secret Manager as `<app>-<id>-encryption-key`. |
...
| **Default db-init job** | Supplied by `Cal Common` when `initialization_jobs = []` | PostgreSQL database and user created automatically. Override with a non-empty list to replace. |
...
| **30-second secret wait** | `time_sleep` in `Cal Common` | After writing `NEXTAUTH_SECRET` and `CALENDSO_ENCRYPTION_KEY` to Secret Manager, `Cal Common` waits 30 seconds before the module outputs are available. This prevents `Permission denied` errors when the Cloud Run service first reads these secrets. |

---

## 10. Variable Reference

All user-configurable variables exposed by `Cal CloudRun`, sorted by UI group then order.
...
| `display_name` | 3 | `'Cal.com Scheduling'` | Human-readable name. Maps to `application_display_name` in `App CloudRun`. |
| `description` | 3 | `'Cal.com - Open-source scheduling platform'` | Service description. Passed to `Cal Common`. |
...
| `startup_probe` | 14 | `{ path="/api/health", initial_delay_seconds=60, failure_threshold=12, ... }` | Startup probe. `Cal Common`'s defaults (initial_delay=180, failure_threshold=18) take effect when this is left at the variable default. |
...
| `startup_probe.initial_delay_seconds` | `180` (via `Cal Common`) | **High** | Cal.com's startup runs `replace-placeholder.sh` (rewrites Next.js static chunks, ~2.5 min) then Prisma migrations (~60 s) and seed-app-store (~30 s). The `Cal Common` defaults are tuned for this. The `Cal CloudRun` variable default of `60s` is insufficient — always rely on `Cal Common`'s longer defaults or leave the probe configuration at `Cal Common` values. |
| `startup_probe.failure_threshold` | `18` (via `Cal Common`) | **High** | With `period_seconds = 10`, a `failure_threshold` of `18` gives Cal.com up to 6 minutes to complete startup. Reducing this below `12` causes Cloud Run to kill and restart the container before initialization completes, resulting in an infinite restart loop. |
