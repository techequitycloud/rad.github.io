---
title: "Jellystat on Cloud Run \u2014 Lab Guide"
description: "Hands-on lab: deploy Jellystat on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Jellystat on Cloud Run — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Jellystat_CloudRun)**
...
the **Jellystat on Cloud Run** module on Google Cloud: deploy it, access and
verify it, run it day-to-day, observe it, diagnose common problems, and tear
it down.

The lab focuses on operating the **Cloud Run module and the Google Cloud
platform**, not on Jellystat product features. For the complete list of
provisioned services and every configuration input (organised by group), see
the [Configuration Guide](https://docs.radmodules.dev/docs/modules/Jellystat_CloudRun) —
...
- (Optional, for the pairing step in Task 2) A running **Jellyfin** server — deploy
  one first with the **Jellyfin (Cloud Run)** or **Jellyfin (GKE)** module if you
...
1. In the RAD platform, open **Jellystat (Cloud Run)**, set `project_id`, and
...
2. The platform provisions the Cloud Run service (the official
   `cyfershepard/jellystat` prebuilt image — no build step), a Cloud SQL
   (PostgreSQL 15) database with its Secret Manager secrets (`JWT_SECRET` and
   the database password), and runs a one-shot database-initialisation job.
   First deploys take roughly **15–25 minutes** (Cloud SQL creation
   dominates).
...
1. Confirm the service is healthy and connected to its database. Jellystat
   exposes a public, unauthenticated endpoint that responds only when the
   server is up:
...
2. Open `$SERVICE_URL` in a browser. On first visit Jellystat prompts you to
   create the initial administrator account — no pre-seeded admin credential
   exists in Secret Manager.

3. **Pair with a Jellyfin server (manual, cannot be automated).** After
   logging in:
   ...
   There is no environment variable or Terraform input for this pairing —
   it is entirely UI-driven by design of the upstream application.
...
3. **Update the application version tag** by changing the version input in
   ...
   pointing at the updated `cyfershepard/jellystat:<tag>` image.
...
2. **Monitoring** — open the Cloud Run dashboard for the service and review
   ...
   `uptime_check_config.enabled = true` — it defaults to `false`); if
...
- **Revision unhealthy / service won't serve:** inspect the latest revision
  ...
  The startup probe targets `/auth/isConfigured`.
  ```bash
  gcloud run revisions list --service="$SERVICE" --project="$PROJECT" --region="$REGION"
...
- **Database connection errors:** confirm the Cloud SQL instance is
  ...
  completed successfully. If Jellystat logs show it can't reach Postgres,
  double-check the deployed revision actually has `POSTGRES_IP`/`POSTGRES_USER`/
  `POSTGRES_DATABASE`/`POSTGRES_PASSWORD` set (not just the generic `DB_*`
  names) — `gcloud run revisions describe <revision> --format=json` shows the
  full injected env var list.
- **Initialisation job failed:** list executions and read the failed one's
  ...
  ```
- **Jellystat shows no data even though the service is healthy:** this is
  almost always the Jellyfin pairing step (Task 2, step 3) not having been
  completed yet — it is not automated by this module.
...
| 2 — Access & verify | Manual | Health check passes; create the admin account; pair with a Jellyfin server |
