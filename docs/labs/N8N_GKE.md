...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **N8N (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. n8ndemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^n8n" --limit=1)
