...
description: "Hands-on lab: deploy Metabase on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
Metabase is an open-source business intelligence platform that lets non-technical
users query and visualize data without writing SQL. This lab takes you through the
full operational lifecycle of the **Metabase on GKE Autopilot** module on Google
...
Metabase product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Metabase_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Metabase (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL) database with its Secret Manager secrets, builds the
   container image, and runs a one-shot database-initialisation job that creates the
   Metabase schema before first boot. First deploys take roughly **20–35 minutes**
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep metabase | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}:3000/api/health"   # expect {"status":"ok"}
   ```

2. Open `http://${EXTERNAL_IP}:3000` in a browser. Metabase's JVM takes 60–120
   seconds to fully start after the pod becomes ready. On first boot Metabase presents
   a **setup wizard** — create your admin account, set your language, and optionally
   add a data source. Metabase manages its own admin credentials internally; no admin
   password is stored in Secret Manager by this module.
...
4. **Manage secrets, storage, and jobs:**
   ...
   gcloud secrets list --project="$PROJECT" --filter="name~metabase"
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. metabasedemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^metabase" --limit=1)
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   ...
   **uptime check** targeting `/api/health` (when enabled); review Monitoring → Uptime
...
platform-level diagnostics and do not change with Metabase releases.

- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. Metabase's JVM
  requires up to 120 seconds to initialise; if the startup probe threshold is too
  tight the pod restarts prematurely.
  ```bash
...
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; complete the first-boot setup wizard |
| 3 — Operate | Manual | Inspect workload, scale, update version, manage secrets/storage, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose pod, JVM startup, database, init-job, scheduling, and image-pull issues |
