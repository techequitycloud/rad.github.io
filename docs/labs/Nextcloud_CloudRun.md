...
description: "Hands-on lab: deploy Nextcloud on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
takes you through the full operational lifecycle of the **Nextcloud on Cloud Run** module
...
on Nextcloud product features. For the complete list of provisioned services and every
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Nextcloud (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (MySQL 8.0) database
   with its Secret Manager secrets, a Filestore NFS share for user file data, an
   optional Redis endpoint, builds the container image, and runs a one-shot
   database-initialisation job. First deploys take roughly **20–35 minutes** (Cloud SQL
   creation dominates). Note that Nextcloud also runs `occ maintenance:install`
   synchronously on the very first container start — the startup probe allows up to
   10 minutes for this to complete before traffic is routed.
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~nextcloud" --format="value(metadata.name)" --limit=1)
...
1. Confirm the service is healthy and Nextcloud is installed:

   ```bash
   curl -s "$SERVICE_URL/status.php"   # expect {"installed":true,"maintenance":false,...}
   ```

   If `"installed": false`, Nextcloud is still running its first-boot setup — wait
   60 seconds and retry.

2. Retrieve the admin password from Secret Manager and sign in at `${SERVICE_URL}`:
...
2. **Scale** by changing the min/max instance inputs and clicking **Update** on the deployment details page —
   ...
   Nextcloud requires Redis and NFS when `max_instance_count > 1` to prevent
   file-locking errors across concurrent instances.

3. **Update the application version** by changing the `application_version` input in
   the RAD platform and applying it via **Update**; a new image builds and a new revision rolls out.
...
- **First-boot timeout / `occ maintenance:install` still running:** the startup probe
  allows up to 10 minutes; check the revision logs for progress. If the Cloud SQL
  instance was just created, the first install can take several minutes on a cold
  instance — wait and retry rather than applying it via **Update** immediately.
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret exists and was injected, and the db-init job completed
  successfully.
- **Initialisation job failed:** list executions and read the failed one's logs:
  ...
  ```
- **"File is locked" HTTP 503 errors:** this occurs when multiple instances share
...
Cloud SQL database, Filestore NFS share, Secret Manager secrets, GCS buckets, and
...
| 2 — Access & verify | Manual | Health check at `/status.php` passes; sign in with admin credentials from Secret Manager |
| 3 — Operate | Manual | Inspect revisions, scale, update version, manage secrets/backups, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose revision, first-boot, database, file-locking, init-job, build, and IAM issues |
