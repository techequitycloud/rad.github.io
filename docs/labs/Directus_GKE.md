...
1. Click **Modules** in the RAD platform top navigation, open **Directus (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}/server/health"   # expect {"status":"ok"}
   ```
