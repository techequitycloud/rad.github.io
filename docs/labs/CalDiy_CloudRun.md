---
title: "CalDiy on Cloud Run \u2014 Lab Guide"
description: "Hands-on lab: deploy CalDiy on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# CalDiy on Cloud Run — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/CalDiy_CloudRun)**
...
through the full operational lifecycle of the **Cal.diy on Cloud Run** module on Google
...
on Cal.diy product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/CalDiy_CloudRun) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Cal.diy (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL) database
   with its Secret Manager secrets (`NEXTAUTH_SECRET` and `CALENDSO_ENCRYPTION_KEY`),
   builds the custom wrapper container image, and runs three one-shot
   database-initialisation jobs (`db-init`, `db-migrate`, `seed-app-store`). First
   deploys take roughly **20–35 minutes** (Cloud SQL creation and image build dominate).
...
1. Confirm the service is healthy and its NextAuth session layer is ready:
   ...
   > **Note on first boot:** Cal.diy runs a URL-rewrite step and Prisma migrations
   > before it can serve requests. The startup probe allows up to 6 minutes. If you
   > see a non-200 response, wait 2–3 minutes and retry.

2. Open `$SERVICE_URL` in a browser. Cal.diy presents an account-creation form on
   first visit — create your admin account there. There is no pre-provisioned admin
   password secret; the account credentials are set by the first user who signs up.
...
- **Revision unhealthy / service won't serve:** inspect the latest revision and its
  ...
  ```
- **Startup loop / 503 on `/api/auth/session`:** Cal.diy runs a URL-rewrite pass,
  Prisma migrations, and an app-store seed before it can serve requests. The startup
  probe allows 6 minutes; if the instance is killed before that window, check that
  `startup_probe.failure_threshold` has not been reduced below the default. Also
  confirm `memory_limit` is at least `2Gi` — OOM kills before the app is ready.
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret exists, and all three initialisation jobs completed successfully.
- **Initialisation jobs failed:** list executions and read the failed one's logs:
  ```bash
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL, secrets, and runs DB init/migrate/seed |
| 2 — Access & verify | Manual | Health check passes; first-visit account creation; secrets confirmed |
| 3 — Operate | Manual | Inspect revisions, scale, update version, manage secrets/backups, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose revision, startup, database, init-job, build, and IAM issues |
