---
title: "WordPress on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy WordPress on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# WordPress on GKE Autopilot — Lab Guide
...
takes you through the full operational lifecycle of the **WordPress on GKE Autopilot**
...
WordPress product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Wordpress_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **WordPress (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   ...
   share for shared `wp-content` storage, an optional Redis object cache, builds the
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep wordpress | head -1 | cut -d/ -f2)
...
2. Check that Apache is responding:

   ```bash
   curl -s -o /dev/null -w "%{http_code}" "http://${EXTERNAL_IP}/wp-admin/install.php"
   # expect 200 (whether WordPress is freshly installed or already configured)
   ```

3. Open `http://${EXTERNAL_IP}` in a browser. If the WordPress 5-minute install wizard
   appears, complete it to create an admin account. If WordPress has already been
   initialised, proceed to `http://${EXTERNAL_IP}/wp-admin` and sign in.

4. To retrieve the database password (useful for CLI database access), find its secret
   in Secret Manager:
...
2. **Scale** by changing the min/max instance inputs and clicking **Update** on the deployment details page —
   ...
   manual `kubectl scale` (a manual edit would be reverted on the next apply). NFS must
   be enabled when running multiple replicas so all pods share the same `wp-content`
   directory and PHP session affinity (`ClientIP`) is preserved.
...
4. **Manage secrets, storage, and jobs:**
   ...
   kubectl get jobs,cronjobs -n "$NS"          # DB-init and any scheduled backup jobs
   ```
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. wordpressdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^wordpress" --limit=1)
...
- **WordPress not persisting plugins, themes, or uploads:** confirm NFS is enabled
  (`enable_nfs = true`) and the Filestore instance is reachable. Without shared NFS,
  each pod has an isolated `wp-content` directory that is lost on restart.
...
and namespace, Cloud SQL database, Filestore NFS share, GCS buckets, Secret Manager
...
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; complete the install wizard and sign in to wp-admin |
| 3 — Operate | Manual | Inspect workload, scale, update version, manage secrets/storage, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose pod, database, init-job, NFS persistence, scheduling, and image-pull issues |
