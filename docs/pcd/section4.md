...
The deployments abstract the complexity of Cloud SQL connections by utilizing the Cloud SQL Auth Proxy (via `cloudsql_volume_mount_path` for GKE or native integration for Cloud Run).

### Exploration
 (App_GKE) / Cloud SQL connections (App_CloudRun)
...
### Exploration
 (implicitly manages service accounts)
*   **Description:** The platform automatically provision dedicated service accounts for the workload and bind necessary roles (like Cloud SQL Client or Secret Manager Secret Accessor).
...
### Implementation Context
By deploying via these deployments, Cloud Logging and Monitoring are natively integrated. Applications can be configured to emit traces by setting specific environment variables.

### Exploration
, `environment_variables` (App_GKE / App_CloudRun)
