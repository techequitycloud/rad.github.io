---
title: "Django on Google Cloud Run"
...
1. Detects the connection mode: if `DB_SSL=false` and `DB_HOST` is not a Unix socket path, forces `DB_HOST=127.0.0.1` to route through the Cloud SQL Auth Proxy sidecar. Otherwise uses `DB_IP` (injected by `App CloudRun`) or falls back to `DB_HOST`.
