...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **GoAlert (GKE)** from
   ...
   `tenant_id = "gke"` (and `"cr"` on the Cloud Run deployment) so the
   two variants don't collide on shared resource names. Click **Deploy Module**, review
...
deployment). This removes everything the module created — the Kubernetes workload and
