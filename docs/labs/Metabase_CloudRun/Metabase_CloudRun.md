...
2. Select **PostgreSQL**.
3. Configure:
   - **Host**: Cloud SQL internal IP (from the `Services GCP` stack) or the Auth Proxy socket path
   - **Port**: `5432`
...
**Expected result:** `MB_JETTY_PORT = "3000"` and `JAVA_TIMEZONE = "UTC"` are present — injected automatically by `Metabase Common`.
