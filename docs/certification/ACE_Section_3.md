...
*   **Cloud Run functions (2nd gen):** Deploy new function versions using `gcloud functions deploy <name> --gen2 --source=.` or via the **Cloud Functions** console. Each deploy creates a new immutable version. Because Cloud Run functions 2nd gen runs on Cloud Run infrastructure, the same CI/CD principles apply — a Cloud Build trigger can include a `gcloud functions deploy` step alongside container build steps, enabling functions and services to be versioned and deployed through the same pipeline.

**Console Exploration:**
In the GCP Console, navigate to **Cloud Build > Triggers** to see the configured integration with source control. Then, navigate to **Cloud Deploy > Delivery pipelines** to visualise the progression of a release and understand how approvals or automated promotions move the application through its environments. For Cloud Functions, navigate to **Cloud Functions > &lt;function&gt;** and click the **Versions** tab to see the full version history and which version is currently receiving traffic.
...
*   **Traffic Splitting (Cloud Run functions 2nd gen):** Cloud Run functions 2nd gen supports traffic splitting between function versions using the same model as Cloud Run services. Deploy a new version without routing traffic using `gcloud functions deploy <name> --gen2 --no-traffic`, then shift traffic progressively with `gcloud functions set-traffic <name> --gen2 --traffic=<version>=<percentage>`.
...
**Console Exploration:**
Navigate to **Cloud Run** in the GCP Console, select the deployed service, and click on the **Revisions** tab. Observe the "Traffic" column to see traffic weighting. For Cloud Functions, navigate to **Cloud Functions > &lt;function&gt; > Traffic** to view and adjust the active traffic split across function versions. For GKE, navigate to **Cloud Deploy > Delivery pipelines** and review the "Rollout" details to observe multi-stage deployments.
...
*   **Autoscaling (Cloud Run functions 2nd gen):** Configure minimum and maximum instances via `--min-instances` and `--max-instances` flags on `gcloud functions deploy`, or adjust them under **Cloud Functions > &lt;function&gt; > Edit** in the console. Setting `--min-instances=1` eliminates cold starts for latency-sensitive functions; `--max-instances` caps concurrent instances and controls maximum spend under a traffic spike. These settings use the same underlying autoscaling model as Cloud Run services.
*   **Startup Probes vs. Liveness Probes:** For applications with slow initialisation (e.g., Java services loading large datasets, Python services importing heavy ML libraries), the `startup_probe_config` variable (Group 5 in App CloudRun) configures a separate startup probe distinct from the steady-state `health_check_config`. This separation is important because startup probes require a much longer `initial_delay_seconds` and more `failure_threshold` retries than liveness probes — a Java Spring Boot service may take 45 seconds to fully initialise, but the liveness probe should detect a deadlock within 10 seconds once the service is running. Using a single `health_check_config` for both startup and liveness forces a trade-off that degrades either startup reliability or deadlock detection. In App GKE, startup, liveness, and readiness probes are each configured as separate sub-fields within `health_check_config` (§3.A).

**Console Exploration:**
Still in the **Revisions** tab of your Cloud Run service in the Console, inspect the autoscaling and concurrency settings. For Cloud Functions, navigate to **Cloud Functions > &lt;function&gt; > Edit** and expand the **Runtime, build, connections and security settings** section to view min/max instance configuration. For GKE, navigate to **Kubernetes Engine > Workloads**, select the deployment, and view the **Autoscaling** tab to see current CPU/Memory targets driving the HPA.
...
*   **GKE Namespace Resource Quotas:** App GKE supports namespace-scoped `ResourceQuota` objects via `enable_resource_quota` (§7.C Reliability & Scheduling). The `quota_cpu_requests`, `quota_cpu_limits`, `quota_memory_requests`, `quota_memory_limits`, `quota_max_pods`, and `quota_max_pvcs` variables define the aggregate ceiling for all workloads in the namespace. For the ACE exam, understand that ResourceQuotas are enforced at pod admission time — if a new pod's request would exceed the namespace quota, the pod is immediately rejected. Inspect active quotas with `kubectl describe quota -n [namespace]` and check the `Used` vs `Hard` columns to see current consumption versus limits.
*   **Additional Services as Independent Kubernetes Deployments:** App GKE's `additional_services` variable (§3.F Core Services) deploys sidecar or helper microservices alongside the primary application — each as a fully independent Kubernetes Deployment with its own HPA, resource requests, and image. The `output_env_var_name` field for each additional service injects its ClusterIP into the primary container's environment as an environment variable, enabling simple service discovery by name. This pattern is useful for sidecar services such as cache warmers, event consumers, or background processors that share the same namespace and database credentials as the primary service but scale independently.
*   **GKE Cluster Selection Modes:** App GKE's `gke_cluster_selection_mode` variable (§3.D Networking) controls which cluster or clusters receive a deployment: `primary` targets the cluster named by `cluster_name`; `explicit` deploys to all clusters listed in `cluster_names`; and `round-robin` cycles deployments across all clusters registered in the Fleet. This enables multi-cluster deployment patterns — for example, deploying a global API service to every regional cluster simultaneously using `round-robin`, while deploying a stateful session service to `primary` only.
