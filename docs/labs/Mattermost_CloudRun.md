...
📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Mattermost_CloudRun)**
...
This lab takes you through the full operational lifecycle of the **Mattermost on Cloud
...
not on Mattermost product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Mattermost (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL) database
   with its Secret Manager secrets, optional GCS FUSE storage and Redis, builds the
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~mattermost" --format="value(metadata.name)" --limit=1)
...
1. Confirm the service is healthy and connected to its database:
   ...
   ```bash
   curl -s "$SERVICE_URL/api/v4/system/ping"   # expect {"status":"OK",...}
   ```

2. Open `$SERVICE_URL` in a browser. Mattermost prompts you to create the first team
   and set up an admin account on first visit. The database password used by the
   application is stored in Secret Manager and can be inspected if needed:
...
2. **Monitoring** — open the Cloud Run dashboard for the service and review request
   ...
   / memory utilisation. The module also provisions an **uptime check** (when
...
- **WebSocket disconnections:** Mattermost maintains persistent WebSocket connections
  for real-time messaging; ensure `min_instance_count` is at least `1` and consider
  raising `timeout_seconds` to `3600` in the RAD platform.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL (PostgreSQL), secrets, GCS storage, and runs DB init |
| 2 — Access & verify | Manual | Health check passes; Mattermost setup wizard accessible |
| 3 — Operate | Manual | Inspect revisions, scale, update version, manage secrets/backups/jobs, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose revision, database, init-job, build, WebSocket, and IAM issues |
