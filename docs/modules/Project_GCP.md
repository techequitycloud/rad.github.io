...
| **Deployed-by (human)** | `deployed_by_email` | The human who requested the deployment. Receives a **tier-dependent** role bundle: read-only in `sandbox` and `lab`; read-only plus operator roles in `production`; read-only plus build-and-deploy roles in `development`. |
...
| `folder_id` | The tier folder the platform injects — there is no default to fall back on |  **Critical** | A project placed in a folder that hasn't had `02-setup-ui.sh` step 10 run against it inherits **no** folder-level org policy guardrails (external-IP denial, SA key restrictions, the API allowlist) — the project is created but effectively unguarded at the org-policy layer, and this module cannot detect or warn about that from inside a single project's state. |
