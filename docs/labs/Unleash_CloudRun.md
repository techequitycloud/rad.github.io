---
title: "Unleash on Cloud Run \u2014 Lab Guide"
description: "Hands-on lab: deploy Unleash on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
lab takes you through the full operational lifecycle of the **Unleash on Cloud Run**
module on Google Cloud: deploy it, access and verify it, create and evaluate a feature
flag, run it day-to-day, observe it, diagnose common problems, and tear it down.

The lab focuses on operating the **Cloud Run module and the Google Cloud platform**,
not on Unleash product features. For the complete list of provisioned services and
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL 15)
   ...
   database password), builds the container image, and runs a one-shot
   database-initialisation job that creates the `unleash` database and user. First
...
1. Confirm the service is healthy and connected to its database. Unleash exposes a
   public health endpoint that returns 200 only when the server is fully initialised
...
2. Open `$SERVICE_URL` in a browser. Log in to the admin UI with the well-known
...
2. **Scale** by changing the min/max instance inputs and clicking **Update** on the deployment details page —
   ...
   manual `gcloud` edit (a manual edit would be reverted on the next apply). Unleash is
   stateless, so any instance can serve any request — scaling out needs no Redis or
   session affinity.
...
3. **Update the application version** by changing the version input in the RAD platform
   ...
   Unleash applies any schema migrations on startup.
...
2. **Monitoring** — open the Cloud Run dashboard for the service and review request
   ...
   CPU / memory utilisation. The module also provisions an **uptime check** against
...
- **Revision unhealthy / service won't serve:** inspect the latest revision and its
  ...
  probe targets `/health` and allows generous headroom for first-boot migrations.
  ```bash
  gcloud run revisions list --service="$SERVICE" --project="$PROJECT" --region="$REGION"
  gcloud run services logs read "$SERVICE" --project="$PROJECT" --region="$REGION" --limit=100
  ```
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret exists, and the initialisation job completed successfully. Check
  the injected `DATABASE_URL`/`DB_*` on the running revision:
  ```bash
...
| 2 — Access & verify | Manual | Health check passes; log in to the admin UI as `admin` / `unleash4all` |
...
| 6 — Troubleshoot | Manual | Diagnose revision, database, init-job, build, and IAM issues |
