---
title: "Directus on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Directus on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
Directus is an open-source data platform that exposes any SQL database through a REST
and GraphQL API and a no-code admin Studio. This lab takes you through the full
operational lifecycle of the **Directus on GKE Autopilot** module on Google Cloud:
...
Directus product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Directus_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Directus (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL) database with its Secret Manager secrets, optional
   NFS/Redis, builds the container image, and runs a one-shot database-initialisation
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep directus | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}/server/ping"   # expect "pong" (200) — /server/health needs an authenticated admin session and 403s here
   ```

2. Retrieve the admin password from Secret Manager and sign in to the Studio at
   `http://${EXTERNAL_IP}/admin`:

   ```bash
   ADMIN_SECRET=$(gcloud secrets list --project="$PROJECT" \
     --filter="name~directus-admin-password" --format="value(name)" --limit=1)
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. directusdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^directus" --limit=1)
...
platform-level diagnostics and do not change with Directus releases.
...
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; sign in to the Studio |
