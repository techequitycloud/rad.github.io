...
- **Least-privilege node SAs** — node pool service accounts hold only the four roles required for logging/monitoring plus `artifactregistry.reader`. See [container-orchestration](container-orchestration).
