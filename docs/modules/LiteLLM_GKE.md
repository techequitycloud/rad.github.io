...
- [Hands-on lab: LiteLLM on GKE Autopilot](../labs/LiteLLM_GKE.md) — deploy it step by step, with the console screens and commands at each stage.
- [LiteLLM on Google Cloud Run](LiteLLM_CloudRun.md) — the same application on Cloud Run, for when you need the other deployment target.
