...
| `module_dependency` | `['GCP_Project']` | List of module names | Declares which platform modules must be deployed before this one. The platform uses this to enforce deployment ordering. `GCP_Project` must exist before `GCP Services` can provision resources. Metadata only — changing this does not alter any GCP resource. |
