...
description: "Hands-on lab: deploy Nextcloud on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Nextcloud (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   ...
   share for user file data, an optional Redis endpoint, builds the
