---
title: "NocoDB on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy NocoDB on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# NocoDB on GKE Autopilot — Lab Guide
...
NocoDB is an open-source no-code database platform — an Airtable alternative — that
transforms any SQL database into a collaborative spreadsheet with a no-code interface,
REST and GraphQL APIs, and built-in automations. This lab takes you through the full
operational lifecycle of the **NocoDB on GKE Autopilot** module on Google Cloud:
...
NocoDB product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/NocoDB_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **NocoDB (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL) database with its Secret Manager secrets, a GCS uploads
   bucket, builds the container image, and runs a one-shot database-initialisation
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep nocodb | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}/api/v1/health"   # expect {"status":"ok"}
   ```

2. Navigate to `http://${EXTERNAL_IP}` in a browser. On the first visit NocoDB
   displays a sign-up form — create the initial admin account here. NocoDB manages
   its own admin credentials; there is no admin password stored in Secret Manager.
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. nocodbdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^nocodb" --limit=1)
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   ...
   **uptime check** targeting `/api/v1/health` (when enabled); review
...
platform-level diagnostics and do not change with NocoDB releases.
...
- **Session invalidation across pods:** if multiple replicas are running without
  Redis, users may be logged out when requests route to a different pod. Enable
  `enable_redis` and supply a `redis_host` before scaling above one replica.
...
and namespace, Cloud SQL database, Secret Manager secrets, GCS uploads bucket, and
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL, GCS bucket, secrets, and runs DB init |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; create the initial admin account in the browser |
| 3 — Operate | Manual | Inspect workload, scale, update version, manage secrets/storage, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose pod, database, init-job, scheduling, image-pull, and multi-replica session issues |
