...
takes you through the full operational lifecycle of the **Superset on GKE Autopilot**
...
Superset product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Superset_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Superset (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL 15) database with its Secret Manager secrets (including the
   auto-generated `SUPERSET_SECRET_KEY`), builds the container image with the required
   database drivers, and runs a two-phase initialisation pipeline: a `db-init` job that
   creates the database and user, followed by an `app-init` job that runs schema
   migrations and creates the admin account. First deploys take roughly **25–45 minutes**
...
2. Retrieve the `SUPERSET_SECRET_KEY` from Secret Manager. The `app-init` job uses this
   ...
   Sign in to Superset at `http://${EXTERNAL_IP}` using username `admin` and the
   retrieved secret key value as the password. Change the admin password immediately
   after first login.

   > **Note:** `SUPERSET_SECRET_KEY` signs Flask sessions and encrypts stored database
   > credentials. Do not rotate it after the first deploy. The Service is configured
   > with `ClientIP` session affinity to keep browser sessions routed consistently to
   > the same pod.
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   ...
   **uptime check** against `/health` (when enabled); review Monitoring → Uptime checks
...
- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. Superset requires up
  to 180 seconds to start (60 s initial probe delay, 12 retries at 10 s intervals):
  ```bash
...
- **Image pull errors:** confirm the image exists in Artifact Registry and the node
  service account can pull it. The custom build is required to add the psycopg2
  PostgreSQL driver to the base image.
...
namespace, Cloud SQL database, Secret Manager secrets (including `SUPERSET_SECRET_KEY`),
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL, secrets, and runs db-init + app-init |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes; retrieve secret key and sign in as admin |
