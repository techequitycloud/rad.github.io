...
*   **Application-Level Alert Policies (Cloud Run):** The `alert_policies` variable (Group 5 for Cloud Run) configures Cloud Monitoring alert policies for application-specific metrics — request latency, error rate, CPU utilisation, and memory utilisation — with customisable thresholds, durations, and notification channels per metric.
*   **Application-Level Alert Policies (App GKE):** App GKE exposes monitoring configuration through `support_users` (Group 1) and the shared `App_Common/modules/app_monitoring` sub-module, which provisions GKE-specific alert policies (pod CrashLoopBackOff, unschedulable pods, container CPU and memory saturation, Gateway latency) and a custom Cloud Monitoring dashboard. Additional policies can be added via the `custom_alert_policies` map in the `app_monitoring` module.
*   **Synthetic Monitoring (Cloud Run):** The `uptime_check_config` variable (Group 5 for Cloud Run) configures Cloud Monitoring uptime checks that probe the application from multiple global locations, validating end-to-end reachability including DNS, load balancers, and Cloud Armor. Uptime checks target the external Load Balancer IP, verifying that the entire stack is operational. For App GKE, synthetic uptime checks for the GKE Gateway endpoint are provisioned by the `app_monitoring` module when `support_users` is non-empty.
