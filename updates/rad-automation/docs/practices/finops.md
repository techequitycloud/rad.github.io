...
functions. Credit functions are allocated 256 Mi–512 Mi of memory,
chosen based on the cost analysis to minimise invocation spend while
avoiding OOM failures under peak load. The phased daily schedule that
drives them lives in
[`scheduler.tf`](../../rad-ui/automation/terraform/infrastructure/scheduler.tf).

### Scheduler phasing

The daily cron jobs are deliberately staggered rather than run in a single
burst. Phasing prevents concurrent GCP Billing API calls from hitting rate
limits, smooths Cloud Function cold-start costs across the day, and
ensures that upstream data (e.g., BigQuery billing exports) has settled
before downstream reconciliation functions consume it.

## Resource labelling

All Terraform-managed resources carry two standard labels:

| Label | Value |
| --- | --- |
| `cost_center` | `rad-automation` |
| `managed_by` | `terraform` |

These labels propagate to Cloud Run, Cloud Functions, Cloud Storage
buckets, and Pub/Sub topics, enabling cost attribution by `cost_center`
in GCP Billing reports without manual tagging. Teams adding new resources
must include these labels in their Terraform definitions.
