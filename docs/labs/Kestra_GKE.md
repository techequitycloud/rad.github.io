...
description: "Hands-on lab: deploy Kestra on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
the **Kestra on GKE Autopilot** module on Google Cloud: deploy it, access and verify it, run
it day-to-day, observe it, diagnose common problems, and tear it down.

The lab focuses on operating the **GKE module and the Google Cloud platform**, not on Kestra
product features. For the complete list of provisioned services and every configuration input
(organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Kestra_GKE) — this lab
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Kestra (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a Cloud SQL
   (PostgreSQL 15) database with its Secret Manager secrets, a dedicated GCS storage bucket,
   builds the container image, and runs a one-shot database-initialisation job. First deploys
   take roughly **20–35 minutes** (Cloud SQL creation dominates; Kestra's JVM startup adds
   additional time on the first boot while database migrations are applied).
...
1. Confirm the workload is running and find its external address:
   ...
   > Kestra uses a Java JVM with a slow first boot. If the pod is not yet `Running`, it
   > may still be completing its database migration. The startup probe allows up to ~14
   > minutes — wait a few minutes and retry.
...
2. **Scale** by changing the min/max instance inputs and clicking **Update** on the deployment details page — the
   ...
   `kubectl scale` (a manual edit would be reverted on the next apply). Note that Kestra's
   standalone mode requires `max_instance_count = 1` to prevent queue-lock conflicts.

3. **Update the application version** by changing the version input via **Update** on the deployment details page; a new image builds and a rolling update replaces the pods with schema
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   utilisation, restart counts, and HPA status. The module can provision an optional
   **uptime check** targeting the `/health` endpoint; review Monitoring → Uptime checks
   and Alerting → Policies if enabled.
...
- **Pod not Ready / CrashLoopBackOff:** inspect events and logs:
  ...
  ```
- **Slow startup / pod not Ready on first boot:** Kestra performs Flyway database migrations
  on startup. The startup probe allows up to ~14 minutes. Check the logs for migration
...
| 2 — Access & verify | Manual | Connect to the cluster; health check passes (`{"status":"UP"}`); sign in to the Kestra UI |
