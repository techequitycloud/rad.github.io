...
description: "Configuration reference for deploying App on GKE Autopilot with the RAD module — variables, architecture, networking, and operations."
---

# App GKE Module — Configuration Guide

<img src="https://storage.googleapis.com/rad-public-2b65/modules/App_GKE.png" alt="App GKE Module — Configuration Guide" style={{maxWidth: "100%", borderRadius: "8px"}} />
...
2. **Services GCP module** (provides shared clusters, shared Cloud SQL, shared NFS, and shared Artifact Registry). You do not need to deploy this yourself first — the platform auto-provisions it before your deployment if it doesn't already exist in the target project. App GKE can also operate in fully inline self-contained mode without Services GCP (`require_services_gcp_module = false`), but every deployment then provisions its own VPC, GKE cluster, NFS VM, and Cloud SQL instance.
...
| `resource_creator_identity` | `string` | `"rad-module-creator@YOUR_PLATFORM_PROJECT.iam.gserviceaccount.com"` | Service account used by Terraform to create and manage resources. Override with a project-specific service account for production deployments. |
...
| `requires_services` | `object` | `{ create_postgres = true, create_network_filesystem = true, create_google_kubernetes_engine = true, … }` | Explicit map of the Services GCP-provisioned resources this module needs. The platform reads this map — not `module_services`, which is a human-readable list for the deployment-confirmation UI — to decide which Services GCP `create_*` toggles must be enabled when auto-provisioning or updating the shared Services GCP deployment for the destination project. Keys mirror Services GCP's own boolean variable names 1:1: `create_postgres`, `create_mysql`, `create_redis`, `create_network_filesystem`, `create_filestore_nfs`, `create_google_kubernetes_engine`, `create_firestore`, `enable_alloydb`. `create_redis` and `create_filestore_nfs` default to `false` because the automated deployment path uses the Compute Engine NFS+Redis VM (`create_network_filesystem`); managed Memorystore/Filestore is a manual post-deployment optimisation. |
...
| Variable | Type | Default | Description |
|---|---|---|---|
| `tenant_id` | `string` | `"demo"` | Short identifier appended to resource names (e.g. Kubernetes namespace, secrets, SQL instance) to distinguish this deployment from others in the same project. Use `prod`, `staging`, `dev`, or a tenant identifier. **Never change after initial deployment** — it is baked into every resource name. |
...
| `namespace_name` | `string` | `""` | Kubernetes namespace in which to deploy the application resources. Leave empty to auto-generate from `application_name` and `tenant_id`. |
...
| `extra_service_ports` | `list(object)` | `[]` | Additional ports to expose on the Kubernetes Service, for a workload that speaks more than one protocol on the same pod (for example ClickHouse, which serves HTTP on `8123` while its native protocol is on `9000`). Each entry defines `name`, `port`, optional `target_port` (defaults to `port`), and optional `protocol` (defaults to `TCP`). Names must be unique — Kubernetes requires a distinct name for every port in a multi-port Service, and this is enforced at plan time. Purely additive: the empty default renders exactly the same Service as before. |
...
| `prereq_gke_master_cidr_override` | `string` | `""` | Override for the inline GKE cluster's control-plane private `/28`. When empty, the foundation derives a unique `/28` per deployment from the random deployment ID inside `172.16.0.0/12` — a different RFC1918 block from the subnet/pod/service ranges above, so it can never collide with them. Pin to the previously-applied value on existing deployments to avoid GKE cluster replacement. |

#### Inline GKE clusters always run private nodes

The inline GKE Autopilot cluster is created with `private_cluster_config { enable_private_nodes = true }`, which is **not** optional and has no variable to disable it. RAD-managed projects enforce a `constraints/compute.vmExternalIpAccess` DENY organization policy; without private nodes, GKE attempts to assign external IPs to its nodes, node creation fails with `Constraint constraints/compute.vmExternalIpAccess violated`, and the cluster is left stuck in `ERROR`.

Only the **nodes** are private. `enable_private_endpoint` is deliberately left at its default (`false`), so the control plane keeps its public endpoint — Cloud Build and other out-of-VPC tooling connect to it directly. The control plane's own private range comes from `prereq_gke_master_cidr_override` (or the derived `/28` above).
...
| `health_check_config` | `object` | `{ enabled = true, path = "/healthz" }` | Kubernetes liveness **and** readiness probes — a single configuration renders both, on Deployment **and** StatefulSet workloads. (StatefulSets originally rendered no readinessProbe and so hit the Gateway `GET /` fallback described below; `statefulset.tf` now emits an identical readinessProbe on both of its container specs, so that fallback no longer applies. Note this does not retroactively change an already-created Backend Service's health check.) The liveness probe periodically checks whether the running container is healthy; if it fails `failure_threshold` consecutive times, Kubernetes restarts the container. The readiness probe additionally determines whether the pod receives traffic, and GKE's Gateway API (Container-Native Load Balancing) derives its GCP-level Backend Service health check from it — with no readiness probe, the Gateway silently falls back to an unconfigurable `GET /` expecting `200`, which permanently marks the backend `UNHEALTHY` for any application whose root path is not a `200`. Sub-fields mirror `startup_probe_config` (`initial_delay_seconds` default: 15, `timeout_seconds` default: 5, `period_seconds` default: 30, `failure_threshold` default: 3). The health endpoint must respond quickly and not perform expensive operations. |
...
**Google Cloud Console:**
- **Health probes:** Navigate to **Kubernetes Engine → Workloads → *your deployment* → YAML** and look for the `livenessProbe`, `readinessProbe`, and `startupProbe` fields in the container spec.
...
| `additional_services` | `list(object)` | `[]` | Supplementary Kubernetes Deployments deployed alongside the main application in the same namespace. Use for sidecar-style patterns: dedicated workers, proxy services, background queue consumers. Each creates its own Deployment, Service, and optional HPA. Key sub-fields: `name`, `image`, `port`, `command`, `args`, `env_vars`, `secret_env_vars` (maps an environment variable name to a key in the materialised app Secret, so a sidecar can read the same secrets as the main container), `cpu_limit`, `memory_limit`, `ephemeral_storage_limit`, `ephemeral_storage_request`, `min_instance_count`, `max_instance_count`, `ingress`, `service_port`, `loadbalancer_ip` (pins a `LoadBalancer` additional service to a reserved static external IP, giving a stable plan-time-known URL), `extra_ports` (further Service ports for a container listening on more than one port under a single Service name), `output_env_var_name` (auto-injects the service URL into the main app as an environment variable), `volume_mounts`, `persistent_volume` (a dedicated block-storage PVC — `size`, `mount_path`, `storage_class`, default `standard-rwo` — for a stateful sidecar; `ReadWriteOnce`, so the service must stay at a single replica, and the PVC is **not** deleted with the Deployment), `wait_for_rollout` (default `true`; set `false` only when the service cannot become healthy until a later step runs, which would otherwise deadlock the apply — it converts an apply-time failure into a silent one), `startup_probe`, `liveness_probe`. |
...
| `tenant_id` | Match environment: `"prod"`, `"staging"`, `"dev"` | **Critical** | Embedded in all resource names alongside `application_name`. **Never change after first deployment** — same consequence as changing `application_name`: full resource recreation, data loss, and a new empty deployment running beside the orphaned old one. |
