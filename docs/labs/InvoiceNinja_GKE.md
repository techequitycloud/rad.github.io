---
title: "InvoiceNinja on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy InvoiceNinja on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# InvoiceNinja on GKE Autopilot — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/InvoiceNinja_GKE)**

## Overview

**Estimated time:** 45–90 minutes

Invoice Ninja is an open-source invoicing and billing platform for managing clients,
creating invoices, tracking expenses, and accepting payments. This lab takes you
through the full operational lifecycle of the **Invoice Ninja on GKE Autopilot**
...
Invoice Ninja product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/InvoiceNinja_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Invoice Ninja (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (MySQL 8.0) database with its Secret Manager secrets, a GCS bucket for
   document storage (mounted via GCS Fuse), builds the container image, and runs two
   one-shot initialisation jobs (`db-init` and `artisan-migrate`). First deploys take
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep invoiceninja | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   # expect 200 or 302 (redirect to setup wizard on first visit)
   ```

2. Open `http://${EXTERNAL_IP}` in a browser. Invoice Ninja redirects to its **setup
   wizard** on first visit. Enter your admin email address and a password of your
   choice, then click **Submit**. The wizard completes the final schema bootstrap and
   redirects to the login page.

   > Note: Invoice Ninja does not store the admin password in Secret Manager. The only
   > secret the module provisions is the application encryption key (`APP_KEY`). You
   > can inspect it if needed:
   >
   > ```bash
   > APP_KEY_SECRET=$(gcloud secrets list --project="$PROJECT" \
   >   --filter="name~invoiceninja-app-key" --format="value(name)" --limit=1)
   > gcloud secrets versions access latest --secret="$APP_KEY_SECRET" --project="$PROJECT"
   > ```
...
4. **Manage secrets, storage, and jobs:**
   ...
   gcloud secrets list --project="$PROJECT" --filter="name~invoiceninja"
   kubectl get jobs -n "$NS"          # db-init and artisan-migrate jobs
   ```
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. invoiceninjademo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^invoiceninja" --limit=1)
...
platform-level diagnostics and do not change with Invoice Ninja releases.

- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. Invoice Ninja uses
  HTTP probes on `/` against port 80 (nginx+PHP-FPM); a slow PHP-FPM start or a
  missing `APP_KEY` secret often causes the startup probe to fail.
  ```bash
  kubectl describe pod -n "$NS" <pod>          # Events section shows scheduling/probe/mount errors
  kubectl logs -n "$NS" <pod> --previous       # logs from the crashed container
  ```
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret materialised into the namespace, and both initialisation jobs
...
and namespace, Cloud SQL database, Secret Manager secrets, GCS document bucket, and
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (MySQL 8.0), secrets, GCS bucket, and runs DB init + migration |
| 2 — Access & verify | Manual | Connect to the cluster; service responds; setup wizard sets admin credentials on first visit |
