...
*   **App GKE** (`support_users` variable, plus custom alert policies configured through the shared App_Common monitoring module): Provisions GKE-specific alert policies for pod CrashLoopBackOff restart loops, unschedulable pods, container CPU and memory saturation, and Gateway latency, plus a custom dashboard for pod counts and replica status. Navigate to **App GKE → §3.A Compute** to configure autoscaling bounds (`min_instance_count` / `max_instance_count`) whose breach thresholds drive these alerts.
