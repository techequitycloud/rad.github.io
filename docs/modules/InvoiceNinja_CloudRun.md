...
**Key Capabilities:**
*   **Compute**: Cloud Run v2 (Gen2), Laravel PHP + nginx container, 2 vCPU / 2 Gi by default. Configurable instance scaling with `min_instance_count = 1` by default (no scale-to-zero) to avoid cold-start latency on invoice processing.
...
**Application secret:** `InvoiceNinja Common` auto-generates the `APP_KEY` secret (Laravel application encryption key — a base64-encoded 32-byte random value, stored as `base64:<value>`) and injects it directly into the application configuration via `secret_environment_variables`. Unlike most modules where secrets flow through `module.app.secret_ids`, the `APP_KEY` is wired within `InvoiceNinja Common` itself.
...
**Scale-to-zero is disabled by default** (`min_instance_count = 1`). Invoice Ninja's cold start involves PHP-FPM initialisation, Laravel bootstrap, and optional database migration — this can take 20–40 seconds. Set `min_instance_count = 0` only for development or low-traffic environments where cold-start latency is acceptable.
...
| `min_instance_count` | 4 | `1` | Minimum running instances. Default `1` avoids cold-start latency on invoice processing. |
...
| `min_instance_count` | `0` | `1` | Invoice Ninja benefits from warm instances for responsive invoice delivery. |
...
`min_instance_count = 1` by default — Invoice Ninja maintains at least one warm instance at all times. `max_instance_count = 3` provides horizontal scale during peak invoice processing or batch export periods. Both values are user-configurable.
...
| **min_instance_count = 1** | Default | Invoice Ninja is kept warm by default to avoid cold-start latency on client invoice delivery. |
...
| `container_image` | 4 | `""` | Override image URI. Leave empty for the default Invoice Ninja image. |
| `min_instance_count` | 4 | `1` | Minimum instances. Default `1` keeps Invoice Ninja warm. |
...
| `min_instance_count` | `1` | **Medium** | Scale-to-zero (`0`) causes cold starts. Invoice Ninja's PHP-FPM initialisation + database migration check takes 10–30 seconds. Clients accessing invoice links during cold start receive timeout errors. |
