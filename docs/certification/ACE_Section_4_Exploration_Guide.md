...
**In the RAD UI:**
*   **Secret Manager Integration:** The application modules use Secret Manager to store sensitive configurations like database passwords. In the RAD UI, the `enable_auto_password_rotation` variable (Group 11 for App CloudRun; §7.D Auto Password Rotation in App GKE) configures automated secret rotation. The secret values are fetched dynamically during deployment and mounted as environment variables or volumes, meaning the plaintext secret is never exposed. App GKE also supports `enable_secrets_store_csi_driver = true` (§4.E Advanced Security) which mounts Secret Manager secrets directly into pods via the GKE Secrets Store CSI Driver, keeping secret values entirely out of Terraform state.
...
**In the RAD UI:**
*   **Zero-Trust Access:** Both modules support configuring IAP via the `enable_iap` variable (Group 15 for App CloudRun; §4.B Identity-Aware Proxy in App GKE). You then provide specific users or groups via `iap_authorized_users` and `iap_authorized_groups` (same section as `enable_iap` in each module). App GKE also requires `iap_oauth_client_id` and `iap_oauth_client_secret` because IAP on the GKE Gateway backend is configured via a `GCPBackendPolicy` CRD, which requires a pre-created OAuth 2.0 client. IAP evaluates policies at the edge before traffic ever reaches the backend Cloud Run service or GKE Gateway, restricting access to authenticated Google users. Because authentication happens at the Google Front End — before the request reaches your application — IAP protects against unauthenticated access even if the application itself has a vulnerability.
...
**In the RAD UI:**
*   **Network Policies (GKE):** The `App GKE` module defines strict internal access controls using the `enable_network_segmentation` variable (§3.D Networking & Network Policies). This ensures that even if a pod within the namespace is compromised, lateral movement is strictly restricted (e.g., denying ingress from all namespaces except the Gateway API or specific whitelisted pods). The implementation uses GKE Dataplane V2 (Cilium) `NetworkPolicy` resources — namespace-level isolation is enforced, and egress to Private Google Access endpoints (`restricted.googleapis.com`, `private.googleapis.com`) is explicitly permitted for VPC Service Controls compatibility.
...
*   **Cloud KMS (Key Management Service):** Practice creating a Key Ring and a symmetric Key under **Security > Key Management**. Then, practice encrypting a Cloud Storage bucket or a Compute Engine disk using that Customer-Managed Encryption Key (CMEK) instead of the default Google-managed keys. CMEK gives you control over the key lifecycle — you can rotate, disable, or destroy the key, which effectively revokes GCP's ability to decrypt the protected data even for its own operations.
    ...
    **In the RAD UI:** App CloudRun (Group 9 `manage_storage_kms_iam = true`) and App GKE (via the shared `App_Common/modules/app_cmek` sub-module) both support CMEK-encrypted Cloud Storage buckets. The `app_cmek` module creates a KMS keyring named `${project_id}-cmek-keyring` and a `storage-key` CryptoKey in your region. Set `manage_storage_kms_iam = true` in App CloudRun Group 9 on a second deployment (after the KMS key exists) to grant the Cloud Run service account `roles/cloudkms.cryptoKeyEncrypterDecrypter` and activate CMEK encryption on all provisioned buckets. Navigate to **Security > Key Management** to observe the keyring and key created by the module, and to **Cloud Storage > Buckets > *your bucket* > Configuration** to confirm the encryption type shows `Customer-managed key`.
