...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Rallly (GKE)** from
   ...
   `smtp_password` now if you want email login working from the first boot. Click
   **Deploy Module**, review the estimated cost in the confirmation dialog (if credits are enabled) and click **Confirm**, which opens the
