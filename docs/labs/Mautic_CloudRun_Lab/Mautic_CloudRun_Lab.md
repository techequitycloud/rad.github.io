...
sidebar_label: "Mautic CloudRun Lab"
---
...
2. The `Services_GCP` module deployed in the same project (provides VPC, Cloud SQL MySQL instance, and NFS server).
...
1. Cloud SQL instance provisioning (if not shared with Services_GCP).
2. Cloud Build custom image build (`mautic/mautic:5-apache` extended with `Mautic_Common`'s Dockerfile).
