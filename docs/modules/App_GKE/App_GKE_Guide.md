...
The `App GKE` module deploys onto **GKE Autopilot**, where Google manages the node OS, node pool configuration, system pods, and OS security patching — significantly narrowing the customer's compliance scope compared to GKE Standard. The module implements a layered, defence-in-depth security posture on top of these Autopilot-managed foundations. Enable controls progressively based on the sensitivity of the workload.
...
|---|---|---|---|
| `module_description` | `"App GKE: A production-ready module…"` | Any string | Human-readable description displayed in the platform catalogue. Change only when forking or white-labelling the module. |
...
| `module_dependency` | `["Services_GCP"]` | List of module names | Declares which platform modules the platform catalogue will associate with this one for dependency tracking and display purposes. All required GCP prerequisites (APIs, networking, IAM) are provisioned automatically by this module if not already present. Optionally, deploying `GCP Services` first is recommended when multiple deployments need to share a common set of platform resources — for example a shared Cloud SQL instance, NFS/Filestore server, or VPC network — as `GCP Services` provisions these shared resources centrally. |
...
| `application_display_name` | `"App GKE Application"` | Any string | A human-readable name shown in the platform UI and monitoring dashboards. Unlike `application_name`, this can be updated freely at any time without affecting resource names. Use a descriptive title that helps operators identify the workload at a glance, e.g. `Customer Portal`, `Payment Processing API`. |
| `application_description` | `"App GKE Custom Application…"` | Any string | A brief description of the application's purpose. Populated into Kubernetes resource annotations and used in platform documentation. Update this to accurately describe your application — it is particularly useful for audit and governance purposes when multiple workloads exist in the same cluster or project. |
...
| `gke_cluster_selection_mode` | `"primary"` | `explicit` / `round-robin` / `primary` | The strategy used to select the target GKE cluster when `gke_cluster_name` is not set. **`primary`** (default): targets the cluster tagged as the primary GCP Services-managed cluster in the project — the simplest option for single-cluster deployments. **`explicit`**: requires `gke_cluster_name` to be set; fails if it is empty. Use this to guarantee a specific cluster is always targeted regardless of what other clusters exist in the project. **`round-robin`**: distributes successive deployments across all available GCP Services-managed clusters in the project in rotation — useful for spreading workloads across multiple clusters for load distribution or isolation. |
...
| `prereq_gke_subnet_cidr` | `"10.201.0.0/24"` | CIDR notation (e.g. `"10.x.x.x/24"`) | The IP CIDR range for the inline GKE subnet created when a GCP Services VPC exists in the project but no GKE cluster is present. The module provisions a new subnet in this CIDR range within the existing VPC in order to create the GKE cluster. **This CIDR must not overlap with any other subnet in the VPC.** Each App GKE deployment sharing the same VPC must use a distinct, non-overlapping CIDR. Consult your network administrator before changing this value. This variable has no effect when a GKE cluster already exists in the project, as the module will reuse the existing cluster. |
...
| Variable | Default | Options / Format | Description & Implications |
|---|---|---|---|
| `create_cloud_storage` | `true` | `true` / `false` | Master switch for GCS bucket provisioning. When `true`, all buckets defined in `storage_buckets` are created. Set to `false` when buckets are managed externally, already exist, or when this deployment should share buckets provisioned by another module (e.g. a shared `GCP Services` deployment). When `false`, the `storage_buckets` variable is ignored but `gcs_volumes` can still reference externally managed buckets by name. |
...
> **Kubernetes Probe Model — Three Distinct Roles:** Kubernetes defines three probe types, each serving a different purpose:
> - **Startup probe** (`startup_probe_config`): Runs *only during pod initialisation*. Kubernetes holds the pod out of the Service load-balancing pool and does not start the liveness probe until the startup probe succeeds. Use this for slow-starting containers (e.g. those running DB migrations at startup) — it prevents Kubernetes from prematurely killing a pod that is legitimately still initialising.
> - **Liveness probe** (`health_check_config`): Runs *continuously* after the startup probe succeeds. If it fails `failure_threshold` consecutive times, Kubernetes restarts the container. Use this to detect deadlocks or hung processes that cannot recover on their own.
> - **Readiness probe**: *Not directly exposed as a separate variable in this module.* In standard Kubernetes, the readiness probe controls whether a pod is included in the Service's load-balancing pool — a pod that fails its readiness probe is temporarily removed from traffic rotation without being restarted. This module uses the startup probe result to gate the initial traffic inclusion; once a pod is running and healthy (liveness probe passing), it is considered ready. For applications that need to temporarily signal "not ready" during runtime (e.g. while draining a task queue or undergoing a configuration reload), a separate readiness probe can be added to the pod spec via the `service_annotations` variable or by extending the module.
>
> Understanding the distinction between these three probe types — particularly that liveness restarts the pod while readiness only removes it from traffic — is a commonly tested topic on the PDE exam.
...
This section summarises every external dependency for deploying `App GKE`. Dependencies are grouped by failure mode to help you identify what must be in place before deploying, what will silently not work, and what requires post-deployment manual action. GKE-specific dependencies that differ from the equivalent `App CloudRun` behaviour are highlighted.
...
| **Multiple inline deployments — subnet CIDR conflict** | `prereq_gke_subnet_cidr` | When `GCP Services` has not been deployed and more than one `App GKE` deployment is created in the same VPC, each deployment that provisions its own inline GKE cluster must use a distinct, non-overlapping `prereq_gke_subnet_cidr`. The default value (`10.201.0.0/24`) will cause an address conflict if used by more than one deployment in the same project. This constraint is eliminated entirely by using a `GCP Services`-managed shared cluster. |
...
| **Multi-Cluster Service** | `enable_multi_cluster_service = true` | The variable is declared but **no Kubernetes or GCP resources are created** in the current version of the module. Enabling it has no effect. | Use `GCP Services` with `configure_cloud_service_mesh = true` for Fleet-based multi-cluster connectivity. Multi-Cluster Service support within `App GKE` is planned for a future release. |
| **Redis cache** | `enable_redis = true` + explicit `redis_host` | `REDIS_HOST` and `REDIS_PORT` environment variables are injected into pods, but applications cannot connect if no Redis service exists at the specified address. There is no Terraform error. | Provision a Cloud Memorystore instance or Redis VM before deploying, or deploy `GCP Services` which provides a shared instance that is auto-discovered when `redis_host` is left blank. |
...
| **Two-apply bootstrap for inline cluster** | *(first deployment without `GCP Services`)* | `prerequisites.tf` now polls the GKE cluster status via `gcloud container clusters describe` (rather than attempting a direct TCP connection to the Kubernetes API server endpoint) and waits up to 600 seconds for the cluster to reach `RUNNING` before proceeding with workload deployment. This eliminates the two-apply bootstrap — a single `terraform apply` creates the cluster and deploys all workloads in the same run. It is also eliminated by using a `GCP Services`-managed cluster. |

---

### Dependency on `GCP Services` for Shared Resources

`GCP Services` is declared as a module dependency (`module_dependency = ["Services_GCP"]`) but is **not required**. The module self-provisions all necessary infrastructure inline, including the GKE Autopilot cluster itself. However, deploying `GCP Services` first is strongly recommended when multiple application modules share the same GCP project — it centralises shared infrastructure and removes per-deployment subnet CIDR management.

| Resource | Without `GCP Services` | With `GCP Services` |
|---|---|---|
| **GKE Autopilot cluster** | Module auto-provisions an inline GKE Autopilot cluster with its own subnet, secondary pod and service CIDR ranges, Cloud NAT, and Cloud Router. A single `terraform apply` creates the cluster and deploys all workloads — the module polls cluster status via `gcloud` and waits up to 600 s for `RUNNING` before proceeding. Each `App GKE` deployment using inline cluster creation in the same VPC must use a distinct `prereq_gke_subnet_cidr`. | Module discovers and deploys workloads to the shared GKE cluster in a single apply. No subnet CIDR coordination required. |
...
| **Anthos Service Mesh** | `gke-mesh.tf` provisions Fleet Hub membership and ASM for the inline cluster when `configure_service_mesh = true`. | `GCP Services` (with `configure_cloud_service_mesh = true`) provisions ASM with full fleet-wide configuration, multi-cluster ingress, and mesh observability dashboards. Recommended for production mesh deployments. |
...
| **Binary Authorization** | `app_security` self-provisions the attestor, KMS key, and policy automatically (see above). | `GCP Services` (with `enable_binary_authorization = true`) provisions these centrally with configurable enforcement mode. Recommended for production. |
| **CMEK encryption** | `app_cmek` self-provisions the `${project_id}-cmek-keyring` and `storage-key` automatically (see above). | `GCP Services` (with `enable_cmek = true`) provisions the keyring with configurable rotation and applies CMEK across all shared resources simultaneously. |
