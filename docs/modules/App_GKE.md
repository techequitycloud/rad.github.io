...
| `extra_service_ports` | `list(object)` | `[]` | Additional ports to expose on the Kubernetes Service, for a workload that speaks more than one protocol on the same pod (e.g. ClickHouse's HTTP port 8123 vs its native protocol port 9000). Purely additive — the default empty list renders exactly the Service the module rendered before. Each entry: `name` (string, required, must be unique across entries), `port` (number, 1-65535), `target_port` (optional, defaults to `port`), `protocol` (optional, defaults to `"TCP"`). Example: `[{ name = "native", port = 9000 }]`. |
