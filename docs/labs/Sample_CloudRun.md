...
description: "Hands-on lab: deploy Sample on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Sample on Cloud Run — Lab Guide
...
the full feature set of the App_CloudRun foundation. This lab takes you through the full
operational lifecycle of the **Sample on Cloud Run** module on Google Cloud: deploy it,
...
on the Sample application itself. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Sample_CloudRun) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Sample (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL 15) database
   with its Secret Manager secrets, an optional NFS share and GCS bucket, builds the
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~sample" --format="value(metadata.name)" --limit=1)
...
1. Confirm the service is healthy and connected to its database:
   ...
   ```bash
   curl -s "$SERVICE_URL/healthz"   # expect {"status":"healthy"}
   ```

2. Verify end-to-end database connectivity via the diagnostics endpoint:

   ```bash
   curl -s "$SERVICE_URL/db"   # expect the PostgreSQL version string
   ```

3. Confirm the root route is reachable (it increments a visitor counter in PostgreSQL):
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. cloudrunappdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^cloudrunapp" --limit=1)
...
- **Initialisation job failed:** list executions and read the failed one's logs:
  ...
  ```
- **Health probe failing (`/healthz` not reachable):** verify `container_port = 8080`
  and `execution_environment = gen2` (required when NFS is enabled).
...
| 2 — Access & verify | Manual | Health check passes; database and root endpoints respond |
| 3 — Operate | Manual | Inspect revisions, scale, update version, manage secrets/backups, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose revision, database, init-job, probe, build, and IAM issues |
