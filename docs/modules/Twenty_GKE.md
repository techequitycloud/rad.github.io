...
- [Hands-on lab: Twenty on GKE Autopilot](../labs/Twenty_GKE.md) — deploy it step by step, with the console screens and commands at each stage.
- [Twenty CRM on Google Cloud Run](Twenty_CloudRun.md) — the same application on Cloud Run, for when you need the other deployment target.
