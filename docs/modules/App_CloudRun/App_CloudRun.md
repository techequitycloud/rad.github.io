...
|---|---|---|---|
| `module_description` | `"App CloudRun: A production-ready module…"` | Any string | Human-readable description displayed in the platform catalogue. Change only when forking or white-labelling the module. |
...
| `module_dependency` | `["Services GCP"]` | List of module names | Declares which platform modules the platform catalogue will associate with this one for dependency tracking and display purposes. All required GCP prerequisites (APIs, networking, IAM) are provisioned automatically by this module if not already present. Optionally, deploying `Services GCP` first is recommended when multiple deployments need to share a common set of platform resources — for example a shared Cloud SQL instance, NFS server, or VPC network — as `Services GCP` provisions these shared resources centrally. |
...
`Services GCP` is declared as a module dependency (`module_dependency = ["Services GCP"]`) but is **not required** for a standalone deployment. The module self-provisions all necessary infrastructure inline when `Services GCP` has not been deployed. However, deploying `Services GCP` first is strongly recommended when multiple application modules share the same GCP project — it centralises shared infrastructure, reduces per-deployment cost, and simplifies ongoing management.
...
|---|---|---|---|
| `application_name` | Short, lowercase, hyphen-safe (e.g. `"myapp"`, `"payments-api"`) | **Critical** | Embedded in every GCP resource name (Cloud Run service, secrets, SQL instance, GCS buckets). **Never change after first deploy** — Terraform destroys and recreates all named resources, causing complete data loss and a new empty database. |
...
| `enable_backup_import` | `false` (default) — set `true` only on the single apply where you want to restore | **Critical** | Leaving `enable_backup_import = true` after a successful restore: the import job re-runs on every subsequent `terraform apply`, overwriting the live database with the stale backup file. **Always set back to `false` immediately after a successful restore.** |
...
| `storage_buckets[].force_destroy` | `false` for production buckets containing user data | **Critical** | `true` on a production bucket: `tofu destroy` permanently deletes all bucket contents with no recovery. `false` on a temporary bucket that needs destruction: `tofu destroy` fails until the bucket is manually emptied. |
