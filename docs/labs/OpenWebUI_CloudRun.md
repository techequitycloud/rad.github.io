---
title: "OpenWebUI on Cloud Run \u2014 Lab Guide"
description: "Hands-on lab: deploy OpenWebUI on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# OpenWebUI on Cloud Run — Lab Guide
...
**Open WebUI on Cloud Run** module on Google Cloud: deploy it, access and verify it,
...
not on Open WebUI product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Open WebUI (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL 15) database
   with its Secret Manager secrets, an optional NFS volume, a GCS data bucket, builds
   the container image, and runs a one-shot database-initialisation job. First deploys
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~openwebui" --format="value(metadata.name)" --limit=1)
...
2. Navigate to `$SERVICE_URL` in a browser. Open WebUI presents a sign-up page on
   first visit — the first account registered automatically becomes the admin. There
   is no pre-set admin password; the platform stores only the auto-generated
   ...
   Open WebUI's own documentation covers the chat interface, model connections, and
...
2. **Monitoring** — open the Cloud Run dashboard for the service and review request
   ...
   `/health` (when enabled); confirm it is green under Monitoring → Uptime checks,
...
Cloud SQL database, Secret Manager secrets, GCS data bucket, and Artifact Registry
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL (PostgreSQL 15), secrets, GCS bucket, and runs DB init |
| 2 — Access & verify | Manual | Health check passes; first user registers as admin |
