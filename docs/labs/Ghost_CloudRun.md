...
description: "Hands-on lab: deploy Ghost on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
Ghost is a modern open-source publishing platform for blogs, newsletters, and membership sites. This lab takes you through the full operational lifecycle of the **Ghost on Cloud Run** module on Google Cloud: deploy it, access and verify it, run it day-to-day, observe it, diagnose common problems, and tear it down.

The lab focuses on operating the **Cloud Run module and the Google Cloud platform**, not on Ghost product features. For the complete list of provisioned services and every configuration input (organised by group), see the [Configuration Guide](https://docs.radmodules.dev/docs/modules/Ghost_CloudRun) — this lab deliberately does not duplicate that detail so it stays accurate over time.
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Ghost (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
   inputs. Configure only what you need — the
   [Configuration Guide](https://docs.radmodules.dev/docs/modules/Ghost_CloudRun)
   documents every input by group, with defaults. Review the estimated cost (if credits are enabled) and click **Deploy**, which opens the deployment status page with real-time logs.

2. The platform provisions the Cloud Run service, a Cloud SQL (MySQL 8.0) database
   with its Secret Manager secrets, a Filestore NFS share for shared content, a
   dedicated `ghost-content` GCS bucket, builds the container image, and runs a
...
1. Confirm the service is healthy. Ghost's health path is `/`, which returns HTTP 200
   once Ghost has finished running database migrations and compiling themes on first
   boot (allow up to 90 seconds on a fresh deploy):

   ```bash
   curl -s -o /dev/null -w "%{http_code}\n" "$SERVICE_URL/"
   ```

2. Open `${SERVICE_URL}/ghost` in a browser to reach the Ghost admin panel. On first
   boot Ghost presents an interactive setup wizard — enter your site title, admin
   name, email, and password to complete setup. The database password (the only
...
- **Revision unhealthy / service won't serve:** Ghost has a 90-second startup probe
  initial delay to allow database migrations and theme compilation on first boot.
...
- **Initialisation job failed:** list executions and read the failed one's logs:
  ...
  ```
- **NFS mount / content not shared:** verify `enable_nfs = true` and that the
  execution environment is `gen2` (required for Filestore mounts in Cloud Run).
- **Image build failed:** review Cloud Build history for the failed build's log.
  The `container_image_source` must be `custom` — the upstream Ghost image lacks
  the entrypoint that maps DB credentials and detects the service URL.
...
Cloud SQL database, Secret Manager secrets, GCS buckets (including the `ghost-content`
...
| 2 — Access & verify | Manual | Health check passes; complete the Ghost admin setup wizard |
| 3 — Operate | Manual | Inspect revisions, scale, update version, manage secrets/backups/storage, DB access |
| 4 — Observe | Manual | Query Cloud Logging; review Cloud Monitoring metrics and uptime check |
| 5 — Troubleshoot | Manual | Diagnose revision, database, init-job, NFS, build, and IAM issues |
