...
2. **Services GCP module** deployed (strongly recommended for shared Cloud SQL, shared NFS, shared Artifact Registry, and shared VPC). App CloudRun can operate in fully inline self-contained mode without Services GCP, but every deployment then provisions its own VPC, NFS VM, and Cloud SQL instance.
...
| `tenant_deployment_id` | Match environment: `"prod"`, `"staging"`, `"dev"` | **Critical** | Embedded in all resource names alongside `application_name`. **Never change after first deploy** — same consequence as changing `application_name`: full resource recreation and data loss. |
