...
1. In the RAD platform, open **Seerr (Cloud Run)**, set `project_id`, and
   ...
   Configuration Guide's Pitfalls section). Click **Deploy Module**, review the
   estimated cost in the confirmation dialog (if credits are enabled) and click **Confirm**, which opens the deployment
