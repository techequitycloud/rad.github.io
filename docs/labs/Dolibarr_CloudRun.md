---
title: "Dolibarr on Cloud Run \u2014 Lab Guide"
description: "Hands-on lab: deploy Dolibarr on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Dolibarr on Cloud Run — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Dolibarr_CloudRun)**

## Overview

**Estimated time:** 45–90 minutes

An open-source ERP and CRM platform for businesses, non-profits, and freelancers. This
lab takes you through the full operational lifecycle of the **Dolibarr on Cloud Run**
...
not on Dolibarr product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Dolibarr (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (MySQL 8.0) database
   ...
   database-initialisation job that creates the Dolibarr schema. First deploys take
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~dolibarr" --format="value(metadata.name)" --limit=1)
...
1. Confirm the service is healthy:
   ...
   Dolibarr's root path returns HTTP 200 once the application has started. If the
   database schema is still being initialised, the app may redirect to the setup
   page — wait 30 seconds and retry.

2. Open `$SERVICE_URL` in a browser. On first access Dolibarr presents a
   **post-install setup wizard** to configure the admin account, company name, and
   enabled modules. Complete the wizard to finish the initial configuration.
   Subsequent logins use the admin credentials set during this wizard.
...
- **Initialisation job failed:** list executions and read the failed one's logs:
  ...
  ```
- **Setup wizard loops or shows DB errors:** confirm the db-init job ran to
  completion and created the Dolibarr schema before the first browser visit.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL (MySQL 8.0), secrets, builds the image, and runs DB init |
| 2 — Access & verify | Manual | Health check passes; complete the Dolibarr post-install setup wizard |
