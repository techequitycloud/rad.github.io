...
description: "Hands-on lab: deploy Flowise on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
takes you through the full operational lifecycle of the **Flowise on Cloud Run**
...
not on Flowise product features. For the complete list of provisioned services and
...
1. Click **Modules** in the RAD platform top navigation, open **Flowise (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
   inputs. Configure only what you need — the
   [Configuration Guide](https://docs.radmodules.dev/docs/modules/Flowise_CloudRun)
   documents every input by group, with defaults. Review the estimated cost (if credits are enabled) and click **Deploy**, which opens the deployment status page with real-time logs.

2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL 15)
   database with its Secret Manager secrets, a GCS uploads bucket, builds the
...
2. Retrieve the admin password from Secret Manager and sign in at `${SERVICE_URL}`:
   ...
   (default: `admin`). Flowise's own documentation covers the chatflow builder and
   REST API features.
...
| 2 — Access & verify | Manual | Health check passes; retrieve admin password and sign in |
