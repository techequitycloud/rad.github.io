...
A fully configured `App CloudRun` deployment provisions and integrates the following GCP services:

- **Cloud Run v2** — Fully managed, serverless container runtime with automatic scaling, traffic splitting, and revision management
...
- **Cloud Storage (GCS)** — Application buckets with optional GCS Fuse mounts inside Cloud Run containers
- **Cloud Filestore / NFS GCE VM** — Shared persistent NFS storage accessible by all Cloud Run instances simultaneously
- **Secret Manager** — Secure storage for database passwords, GitHub tokens, and application secrets; injected into containers as environment variables
...
- **Cloud Armor** *(optional)* — WAF security policy attached to the HTTPS load balancer in front of Cloud Run
- **Identity-Aware Proxy** *(optional)* — Google-identity authentication enforced at the load balancer layer
- **Certificate Manager** *(optional)* — Google-managed SSL certificates for custom domains
- **VPC Service Controls** *(optional)* — API perimeter restricting access to GCP services to within-VPC and approved identities
- **Binary Authorization** *(optional)* — Policy enforcement requiring attested container images before deployment

---

## Prerequisites

Before deploying App CloudRun:

1. **GCP project** with billing enabled.
2. **Services GCP module** (provides shared Cloud SQL, shared NFS, shared Artifact Registry, and shared VPC). You do not need to deploy this yourself first — the platform auto-provisions it before your deployment if it doesn't already exist in the target project. App CloudRun can also operate in fully inline self-contained mode without Services GCP (`require_services_gcp_module = false`), but every deployment then provisions its own VPC, NFS VM, and Cloud SQL instance.
3. **IAM permissions**: The deploying service account requires broad project-level permissions (Project Editor or equivalent) to create Cloud Run services, Cloud SQL users, GCS buckets, and Secret Manager secrets.
...
These variables are consumed by the deployment platform rather than by the Terraform resources themselves. The `module_*` prefixed variables (`module_storage_buckets`, `module_env_vars`, `module_secret_env_vars`, `module_writable_secret_ids`, `module_explicit_secret_values`) and `scripts_dir` are populated by an Application (wrapper) module — such as `Ghost_CloudRun` — that calls `App_CloudRun` as a child module; they should be left at their empty defaults when deploying `App_CloudRun` standalone. `additional_cloudrun_sa_roles` is a real functional variable (it grants extra IAM roles) that happens to share this group.

| Variable | Default | Options / Format | Description & Implications |
|---|---|---|---|
| `module_description` | *(module blurb)* | Any string | Human-readable description of the module's purpose, shown in the platform UI. Metadata only — do not modify unless customising the module for a fork. |
...
| `requires_services` | `{ create_postgres = true, create_network_filesystem = true, … }` | Object of `create_*` booleans | Explicit map of the `Services_GCP`-provisioned resources this module needs, independent of whatever `Services_GCP`'s own variable defaults currently are. The platform reads this map — not `module_services`, which is only a human-readable list for the deployment-confirmation UI — to decide which `Services_GCP` `create_*` toggles must be enabled when it auto-provisions or updates the shared `Services_GCP` deployment for the destination project. Keys mirror `Services_GCP`'s own boolean variable names 1:1: `create_postgres`, `create_mysql`, `create_redis`, `create_network_filesystem`, `create_filestore_nfs`, `create_google_kubernetes_engine`, `create_firestore`, `enable_alloydb`. Defaults to `create_postgres = true` and `create_network_filesystem = true` with all others `false`. `create_redis` and `create_filestore_nfs` deliberately stay `false` on the automated path: the free Compute Engine NFS+Redis VM (`create_network_filesystem`) is what every automated deployment gets, and switching to managed Cloud Memorystore/Filestore is a manual post-deployment optimisation rather than something an automated dependency chain requests on its own. |
| `shared_users` | `[]` | List of email addresses | Users who can view and deploy this module regardless of the `public_access` setting. |
| `technical_support_users` | `[]` | List of email addresses | Users responsible for providing technical support for this module. The deployment portal routes support requests to these addresses. Metadata only. |
| `resource_creator_identity` | `"rad-module-creator@YOUR_PLATFORM_PROJECT.iam.gserviceaccount.com"` | Service account email | Service account used by Terraform to create and manage resources. Override with a project-specific service account for production deployments. |
| `impersonation_service_account` | `""` | Service account email | Service account to impersonate when shell scripts (discovery, image mirroring, NFS setup) call GCP APIs. Required for cross-project deployments; leave empty to use the runner's own credentials. |
| `job_execution_wait_timeout` | `900` | Integer (seconds) | Maximum seconds a deployment waits for an initialization or database setup job execution to finish before aborting. Bounds the `gcloud run jobs execute --wait` provisioners so a stuck job fails the apply quickly. |
| `module_storage_buckets` | `[]` | List of bucket definition objects | Additional GCS bucket definitions injected by a wrapper module; merged with `storage_buckets` at runtime. Leave empty when deploying standalone. |
| `module_env_vars` | `{}` | Map of `"VAR_NAME" = "value"` | Additional plain-text environment variables injected by a wrapper module; merged with `environment_variables`. Leave empty when deploying standalone. |
| `module_secret_env_vars` | `{}` | Map of `"VAR_NAME" = "secret-name"` | Additional Secret Manager environment variable references injected by a wrapper module; merged with `secret_environment_variables`. Leave empty when deploying standalone. |
| `module_writable_secret_ids` | `{}` | Map of secret key → secret ID | Secret Manager secret IDs where the workload service account needs `secretVersionAdder` (write) access. Set by app modules whose post-install hooks write values back to Secret Manager. |
| `module_explicit_secret_values` | `{}` | Map of `"VAR_NAME" = "value"` | Raw secret values provided directly by a wrapper module. Keys must match entries in `module_secret_env_vars`. When provided, the plan-time Secret Manager data source lookup is skipped for these keys, allowing the first apply to succeed before the secrets exist. |
| `scripts_dir` | `""` | Filesystem path | Directory containing initialisation and utility scripts. Defaults to the module's built-in scripts directory when left blank. Override when a wrapper module supplies custom scripts. |
| `additional_cloudrun_sa_roles` | `[]` | List of IAM role strings | Extra IAM roles granted to the Cloud Run service account in addition to the standard set. Use when an application module requires GCP permissions not included in the base role list (e.g. `roles/datastore.owner` for Firestore user management). |
...
| Variable | Default | Options / Format | Description & Implications |
|---|---|---|---|
| `project_id` | *(required)* | `[a-z][a-z0-9-]{4,28}[a-z0-9]` | The GCP project ID where all resources will be provisioned. All required GCP prerequisites (APIs, networking, IAM) are provisioned automatically if not already present. Optionally, `Services GCP` can be deployed first to provision shared platform resources (such as a shared Cloud SQL instance, NFS server, or VPC network) that multiple deployments in the same project can then reuse. **All resource names, IAM bindings, secrets, and API calls are scoped to this project.** Changing this after initial deployment will cause all resources to be recreated in the new project. |
| `region` | `"us-central1"` | GCP region string | The GCP region used when no `Services GCP` subnet mapping can be auto-discovered for the project. Override this when deploying to a project whose primary region is not `us-central1` and where `Services GCP` has not been deployed (or its region data is not yet propagated). Example: `"europe-west1"`, `"asia-northeast1"`. Has no effect when a `Services GCP`-managed subnet is successfully auto-discovered, as the region is inferred from the subnet. |
| `tenant_id` | `"demo"` | `[a-z0-9-]{1,20}` | A short label appended to resource names (e.g. Cloud Run service, secrets, SQL instance) to distinguish this deployment from others in the same project. Use values such as `prod`, `staging`, `dev`, or a customer/tenant identifier. **Do not change this after initial deployment** — it is baked into resource names and changing it will cause all resources to be recreated with new names, leaving the old ones orphaned. |
...
| `application_description` | `"App CloudRun Custom Application…"` | Any string | A brief description of the application's purpose. Populated into the Cloud Run service description field and used in platform documentation. Visible in the Cloud Run console under the service details. Update this to accurately describe your application — it is particularly useful for audit and governance purposes when multiple services exist in the same project. |
...
> **Choosing runtime & scaling.** Three decisions dominate cost and behaviour. **Image source:** `"custom"` builds from your repo via Cloud Build (needs the GitHub connection); `"prebuilt"` deploys an existing image URI directly and *requires* `container_image` (enforced at plan time). **Scaling floor:** `min_instance_count = 0` gives scale-to-zero — cheapest, but every cold request pays a start-up penalty and any in-memory/local-disk state is lost between requests; set `1+` for latency-sensitive or stateful apps. `max_instance_count` is your cost ceiling. **Execution environment:** keep the `gen2` default — `gen1` cannot mount NFS or GCS Fuse (a `gen1` + `enable_nfs`/`gcs_volumes` combination is rejected at plan time). `cpu_always_allocated` defaults to `false` (request-based billing), so scale-to-zero savings are realised out of the box; set it `true` only when background work must run between requests (schedulers, queue workers, WebSockets).
...
| `cpu_always_allocated` | `false` | `true` / `false` | **`true`:** CPU is allocated to the container at all times, even when it is not processing a request. This enables background processing, scheduled tasks within the container, and WebSocket connections. Costs are incurred continuously per instance. **`false` (default):** CPU is only allocated during request processing; it is throttled to near-zero between requests. Reduces cost for low-traffic workloads but prevents any background computation. Set `true` for applications that run background threads or maintain persistent connections (e.g. message queue consumers). |
...
| `max_revisions_to_retain` | `7` | Integer `0`–`100` | The maximum number of Cloud Run revisions to keep after each deployment. After each deployment, old revisions are automatically pruned beyond this limit (sorted oldest-first). Revisions actively serving traffic are never deleted. Set to `0` to disable automatic pruning entirely and retain all revisions indefinitely. Reducing this value limits API quota consumption and keeps the revision list manageable. Only applies to non-Cloud Deploy deployments; Cloud Deploy–managed services are handled separately per stage. Must be between `0` and `100`. |
...
| Variable | Default | Options / Format | Description & Implications |
|---|---|---|---|
| `environment_variables` | `{}` | Map of `"VAR_NAME" = "value"` | Plain-text environment variables injected into every container instance at startup. Use for non-sensitive configuration such as feature flags, log levels, API base URLs, or application mode settings. Examples: `{ LOG_LEVEL = "info", FEATURE_NEW_UI = "true", API_BASE_URL = "https://api.example.com" }`. **Do not store passwords, tokens, API keys, or any sensitive values here** — they will be visible in the Cloud Run revision configuration and in the platform's configuration. Use `secret_environment_variables` for sensitive values instead. Changes to this map trigger a new Cloud Run revision. |
...
| `uptime_check_config` | `{ enabled = false, path = "/" }` | Object | Configures a **Google Cloud Monitoring uptime check** that sends periodic HTTP requests to the application from multiple global locations (typically 6 Google points of presence worldwide). If the application becomes unreachable from a majority of locations, an alert is triggered and sent to `support_users`. Disabled by default — set `enabled = true` to provision the check; it is only created when the endpoint is publicly reachable. Sub-fields: **`enabled`** (`true`/`false`). **`path`** — the HTTP path to probe from the outside, e.g. `/healthz` or `/`. **`check_interval`** — how frequently to probe, in seconds with an `s` suffix (default: `"60s"`; minimum `"60s"`). **`timeout`** — maximum response time before the check is marked as failed (default: `"10s"`; must be less than `check_interval`). Unlike the startup and liveness probes — which are internal container-level checks — the uptime check validates end-to-end reachability from the public internet. This means it also validates DNS, load balancers, and Cloud Armor rules where applicable. |
...
| `additional_containers` | `[]` | List of objects | **In-pod sidecar** containers added to the **same** Cloud Run service as the main app (multi-container revision), sharing `localhost` and the pod lifecycle — distinct from `additional_services`, which deploy as separate services. Use this when the app or a dependency must be reached over a non-HTTP/loopback protocol that Cloud Run service-to-service networking cannot carry, e.g. a MongoDB the app connects to at `127.0.0.1:27017`. The main (ingress) container starts only after every sidecar whose `startup_tcp_port` is set has itself started. Key sub-fields: **`name`** — sidecar container name. **`image`** — container image URI. **`command`** / **`args`** — entrypoint override. **`env_vars`** — plain-text environment variables for the sidecar. **`cpu_limit`** / **`memory_limit`** — resource limits. **`mount_nfs`** — mounts the shared Filestore volume (requires `enable_nfs = true`) at `nfs_mount_path`, e.g. for a database data directory. **`startup_tcp_port`** — TCP port Cloud Run polls to determine the sidecar is ready before starting the main container. Example: `[{ name = "mongo", image = "mongo:7", args = ["--dbpath", "/data/db"], mount_nfs = true, nfs_mount_path = "/data/db", startup_tcp_port = 27017 }]`. |
...
> **Choosing a storage model (Groups 8 & 9).** Match the storage type to the access pattern. **NFS (this group)** gives a POSIX filesystem shared read-write across all instances simultaneously — the right choice when the app or its sidecars need a true shared mountable filesystem (uploaded media, a database data-dir on a sidecar). **GCS object storage (Group 9)** is for app-managed objects accessed via the API/SDK — cheaper, infinitely scalable, no mount. **GCS Fuse (Group 9)** bridges the two: a bucket surfaced as a filesystem path, good for read-mostly assets but with filesystem-semantics caveats (no POSIX locking, higher latency). Both NFS and GCS Fuse require `execution_environment = "gen2"`. With a `Services_GCP` foundation present, `enable_nfs` auto-discovers the shared, managed Filestore; standalone, it provisions a single inline NFS VM (a single point of failure) — prefer the foundation for anything production.
...
| Variable | Default | Options / Format | Description & Implications |
|---|---|---|---|
| `enable_nfs` | `true` | `true` / `false` | When `true`, an NFS volume is mounted into the Cloud Run service at the path defined by `nfs_mount_path`. The module will use an existing Filestore instance if one is discovered in the project (either named via `nfs_instance_name` or auto-discovered from a `Services GCP` deployment). An inline NFS GCE VM is created **only when no `Services GCP` deployment exists in the project at all** — if `Services GCP` is present but was deployed without NFS (`create_network_filesystem = false`), the plan fails with an error directing you to redeploy `Services GCP` with NFS enabled, rather than creating a colliding inline VM. **NFS provides shared persistent storage** — files written by one instance are immediately visible to all other instances. This is essential for applications that handle file uploads, shared configuration, or any data that must persist beyond the lifetime of a single container. Set to `false` if your application is entirely stateless or uses GCS/Cloud SQL for all persistence. |
...
| `nfs_volume_name` | `"nfs-data-volume"` | String | The Kubernetes/Cloud Run volume name used for the NFS mount. Change this when a deployment requires a second NFS share with a distinct volume name — for example, separating a media volume (`nfs-media-volume`) from a data volume. Jobs that mount this NFS share reference the volume by this name in their `mount_nfs` configuration. Only relevant when `enable_nfs` is `true`. |
| `nfs_instance_name` | `""` *(auto-discover)* | String | The name of a specific existing NFS server (GCE VM) to connect to. When set, the module targets this instance directly and skips auto-discovery. Leave blank to allow the module to auto-discover a `Services GCP`-managed NFS instance in the project, or to create a new inline NFS VM when no `Services GCP` deployment exists in the project at all. Use this when you have multiple NFS servers in the project and need to explicitly control which one this deployment connects to, or when the auto-discovery would select the wrong instance. |
...
| Variable | Default | Options / Format | Description & Implications |
|---|---|---|---|
| `create_cloud_storage` | `true` | `true` / `false` | Master switch for GCS bucket provisioning. When `true`, all buckets defined in `storage_buckets` are created. Set to `false` when buckets are managed externally, already exist, or when this deployment should share buckets provisioned by another module or deployment (e.g. a shared `Services GCP` deployment). When `false`, the `storage_buckets` variable is ignored but `gcs_volumes` can still reference externally managed buckets. |
...
| `manage_storage_kms_iam` | `false` | `true` / `false` | Controls whether the module manages the IAM binding that grants the Cloud Run service account `roles/cloudkms.cryptoKeyEncrypterDecrypter` on the Cloud KMS key used to encrypt storage buckets. It is now safe to set this to `true` on the first deployment — the module's `app_cmek` sub-module automatically creates the `${project_id}-cmek-keyring` KMS keyring and `storage-key` CryptoKey if they do not already exist before applying the IAM binding. If `Services GCP` was deployed with `enable_cmek = true`, the same well-known keyring name is shared and no duplicate resources are created. |
...
| `max_images_to_retain` | `7` | Integer `0`–`100` | The maximum number of the most recent container images to keep in the **inline-created** Artifact Registry repository for this deployment. This is a KEEP policy that acts as a safety net — it prevents the DELETE policies below from removing the N newest images even if they are old or untagged. Only applies to the inline repository created by this module (not a shared `Services GCP` repository) and is scoped to this deployment's images by `package_name_prefix`. Set to `0` to disable the retention guard entirely. |
...
- **Artifact Registry cleanup policies:** Navigate to **Artifact Registry → Repositories → *your repository* → Cleanup Policies** to confirm the KEEP and DELETE policies are configured correctly for this deployment.
...
> **Choosing the database backend.** First decide *whether* you need one: `database_type = "NONE"` skips all DB provisioning (and disables the dependent features — backup import, custom SQL, auto-rotation, the Cloud SQL volume — each of which is rejected at plan time if left on with `NONE`). If you need a database, pick the engine your app requires (`POSTGRES`/`MYSQL`, or a version-pinned variant like `POSTGRES_15`). **Pin the version for production** — `database_type` is effectively immutable: changing it after first deploy *replaces the Cloud SQL instance and destroys its data*. Where the instance lives is automatic: with a `Services_GCP` foundation the module creates only a database + user inside the shared instance (no per-deploy instance cost); standalone, it provisions a dedicated instance. Engine-family toggles (`enable_postgres_extensions`, `enable_mysql_plugins`) must match the chosen engine — a mismatch is caught at plan time.
...
| `enable_postgres_extensions` | `false` | `true` / `false` | When `true`, enables installation of PostgreSQL extensions in the application database after provisioning. Only applies when `database_type` is a PostgreSQL variant. Set to `false` if no extensions are required or if extensions are managed by the application at startup. |
| `postgres_extensions` | `[]` | List of extension name strings | The PostgreSQL extensions to install in the application database. Requires `enable_postgres_extensions = true` and a PostgreSQL `database_type`. Common extensions: `postgis` (geospatial data), `uuid-ossp` (UUID generation), `pg_trgm` (trigram text search), `pgcrypto` (cryptographic functions), `hstore` (key-value storage), `pg_stat_statements` (query performance tracking). Ensure the extension is supported by the Cloud SQL PostgreSQL version in use. |
| `enable_mysql_plugins` | `false` | `true` / `false` | When `true`, enables installation of MySQL plugins in the application database after provisioning. Only applies when `database_type` is a MySQL variant. Set to `false` if no plugins are required or if plugins are managed by the application at startup. |
...
| `db_host_env_var_name` | `""` *(disabled)* | Environment variable name string | An additional environment variable name under which the database host is also exposed alongside the standard `DB_HOST`. Intended for wrapper modules wrapping applications that expect a non-standard host variable name (e.g. Nextcloud's `NC_DB_HOST`). Leave empty to inject the host only as `DB_HOST`. |
| `db_user_env_var_name` | `""` *(disabled)* | Environment variable name string | An additional environment variable name under which the database user is also exposed alongside the standard `DB_USER`. Leave empty to inject the user only as `DB_USER`. |
| `db_name_env_var_name` | `""` *(disabled)* | Environment variable name string | An additional environment variable name under which the database name is also exposed alongside the standard `DB_NAME`. Leave empty to inject the name only as `DB_NAME`. |
| `db_port_env_var_name` | `""` *(disabled)* | Environment variable name string | An additional environment variable name under which the database port is also exposed alongside the standard `DB_PORT`. Leave empty to inject the port only as `DB_PORT`. |
| `service_url_env_var_name` | `""` *(disabled)* | Environment variable name string | An additional environment variable name under which the Cloud Run service URL is also exposed alongside the standard `CLOUDRUN_SERVICE_URL`. Useful for applications that build absolute URLs from a specific variable (e.g. `NC_PUBLIC_URL`, Outline's `URL`). Leave empty to inject the URL only as `CLOUDRUN_SERVICE_URL`. |
...
> **Choosing access & networking.** Two independent axes. **Ingress** controls who can reach the service: `"all"` for a public app; `"internal"` for VPC-only backends; `"internal-and-cloud-load-balancing"` whenever a load balancer fronts it — and this is mandatory pairing with `enable_cloud_armor` (Group 16), since `"all"` leaves the `*.run.app` URL publicly reachable and bypasses the WAF entirely. **Egress** controls outbound: keep `"PRIVATE_RANGES_ONLY"` if the app only talks to private VPC resources (Cloud SQL, Memorystore, NFS); switch to `"ALL_TRAFFIC"` if it calls external APIs/SaaS/webhooks, or those calls silently time out. Leave `network_name` blank to auto-discover the single `Services_GCP` VPC; set it only when the project has more than one.
...
| `network_name` | `""` *(auto-discovered)* | VPC network name string | The name of the VPC network to attach the Cloud Run service to for egress routing and Direct VPC Egress. Leave empty to allow the module to auto-discover the single Services GCP-managed network in the project. **Specify a value** when more than one Services GCP-managed network exists in the project, or when you want to attach the service to a specific network. The network must exist in the same project. Changing this value will trigger a new Cloud Run revision. |
| `prereq_subnet_cidr_override` | `""` | CIDR string | Override for the inline VPC primary subnet CIDR created when no `Services GCP` network exists. When empty (default), the foundation derives a unique `/24` per deployment from the random deployment ID. Set to the previously-applied value (e.g. `"10.200.0.0/24"`) on existing standalone deployments to avoid resource replacement. Has no effect when a `Services GCP`-managed network is in use. |
...
> **Note:** Roll out VPC-SC in **dry-run mode first** (`vpc_sc_dry_run = true`, the default). The perimeter is created and violations are logged, but API calls are not blocked. Review the dry-run logs (see the gcloud snippet below) before flipping `vpc_sc_dry_run = false` to enforce the perimeter.

> **Note:** VPC-SC requires a GCP organization. Standalone projects (Qwiklab, personal Google accounts) have no organization and `enable_vpc_sc = true` will emit a `WARNING: VPC Service Controls skipped …` log line and create no perimeter resources. Folder-nested projects require `organization_id` to be set explicitly because it cannot be auto-discovered from the project metadata.
...
| `enable_audit_logging` | `false` | `true` / `false` | When `true`, the module enables detailed Cloud Audit Logs for the entire project: `ADMIN_READ`, `DATA_READ`, and `DATA_WRITE` for all GCP services, plus explicit per-service overrides for Secret Manager and Cloud KMS to guarantee sensitive access is always logged. Equivalent to `Services GCP enable_audit_logging = true`. **Recommended for** compliance-sensitive environments (PCI, HIPAA, SOC 2). **Trade-off:** Significantly increases Cloud Logging storage volume and associated costs — apps with high request throughput or heavy Secret Manager / KMS usage should budget for the extra log ingestion. Safe to toggle at any time; does not affect running workloads. |
...
- **Dry-run status:** On the perimeter details page, the **Dry-run** banner indicates whether the perimeter is in log-only mode. Flip `vpc_sc_dry_run = false` after reviewing logs to switch to enforcing.
- **Perimeter membership:** In the perimeter details, confirm the project is listed under **Projects** and that the four expected access levels (`vpc_access_*`, `admin_access_*`, `iap_access_*`, `cicd_access_*`) are attached.
- **VPC-SC audit logs:** Navigate to **Logging → Logs Explorer** and filter for `protoPayload.metadata.@type="type.googleapis.com/google.cloud.audit.VpcServiceControlAuditMetadata"` to identify any API calls being denied by the perimeter.
- **Project audit logging:** When `enable_audit_logging = true`, navigate to **IAM & Admin → Audit Logs** and confirm that **Admin Read**, **Data Read**, and **Data Write** are enabled for the relevant services.
...
> **Notation:** *Self-provisioned* means the module (or its `App Common` library) creates the resource automatically on first deployment — no manual pre-requisite is required.

---

### Tier 1 — Hard Prerequisites

These configurations will prevent deployment from succeeding, or will prevent the Cloud Run service from reaching a healthy state, if the listed prerequisite is not satisfied.

| Feature | Variable(s) | Requirement |
|---|---|---|
| **Secret Manager references** | `secret_environment_variables` | Every secret named in the map must exist in Secret Manager **before deploying**. Create the secret in Secret Manager first, then deploy. |
...
| **CI/CD pipeline** | `enable_cicd_trigger = true` | A GitHub repository must be accessible and either a GitHub Personal Access Token (scopes: `repo` and `admin:repo_hook`) or a GitHub App installation ID must be provided. Without valid credentials, the Cloud Build GitHub connection cannot be established and deployment will fail. |
| **Custom container build** | `container_image_source = "custom"` | Requires the same GitHub repository connection and credentials as `enable_cicd_trigger`. Cloud Build will fail during deployment if the repository is unreachable or credentials are absent. |
| **VPC Service Controls (folder-nested projects)** | `enable_vpc_sc = true` + project nested under a folder | The project's organization ID cannot be auto-discovered when the project sits under a folder rather than directly under an organization. `organization_id` must be supplied explicitly or the perimeter is skipped with a warning. For projects directly under an organization or for standalone projects this prerequisite does not apply — see §17 for the full auto-skip matrix. |
| **VPC Service Controls (admin lockout protection)** | `enable_vpc_sc = true` | `admin_ip_ranges` must contain at least one CIDR range before the perimeter is created. An empty list causes the perimeter to be skipped with a warning, to prevent an admin lockout scenario where no identity outside the VPC can reach the APIs. |

---

### Tier 2 — Silent Failures

These configurations deploy successfully but will not function correctly at runtime. There is no immediate error to indicate the problem.

| Feature | Variable(s) | Failure mode | Resolution |
|---|---|---|---|
| **Redis cache** | `enable_redis = true` + explicit `redis_host` | `REDIS_HOST` and `REDIS_PORT` environment variables are injected into the container, but the application cannot connect if no Redis service exists at the specified address. The deployment itself will succeed with no error. | Provision a Cloud Memorystore instance or Redis VM and set `redis_host` to its private IP — the module does **not** discover Memorystore. Leaving `redis_host` blank only works when a `Services GCP` NFS **GCE VM** exists (it co-hosts Redis); a managed Memorystore or Filestore instance is never discovered. |
...
|---|---|---|
| **Custom domain** | `application_domains` (with `enable_cloud_armor = true`) | After deployment, create **DNS A records** for each domain pointing to the load balancer's external IP address (shown in the deployment outputs). Google-managed SSL certificate provisioning begins automatically after DNS propagation and typically completes within 10–60 minutes. The application will not be reachable on the custom domain until the certificate reaches `ACTIVE` status. |
| **Identity-Aware Proxy** | `enable_iap = true` | The project must already have an IAP OAuth brand (consent screen). A plan-time probe (`data.external.iap_brand_probe`, backed by `scripts/check-iap-brand.sh`) fails the **plan** when the project has no brand, because enabling IAP destroys the service's `allUsers`/`run.invoker` binding in the same apply and a later failure would leave the service reachable by nobody. Create one first with `gcloud iap oauth-brands create --application_title=... --support_email=...`. The probe returns `unknown` and is allowed through when it cannot determine the answer (no gcloud, no permission, API disabled). |
...
| **CMEK keyring for storage encryption** | `manage_storage_kms_iam = true` | `App_Common/modules/app_cmek` idempotently creates the `${project_id}-cmek-keyring` KMS keyring and its `storage-key` CryptoKey before the storage IAM binding is applied. It is now safe to set `manage_storage_kms_iam = true` on the first deployment without any pre-existing KMS resources. If `Services_GCP` is deployed with `enable_cmek = true`, both modules target the same well-known keyring name — whichever runs first creates it; the second is a no-op. The GCS service agent KMS binding is re-asserted proactively by `assert_gcs_kms_binding.sh` — this breaks the chicken-and-egg where a drifted binding previously caused "Permission denied on Cloud KMS key" errors during deployment and blocked the very operation that would restore it. |
| **PSA service agent for inline Cloud SQL** | Inline Cloud SQL (`database_type != "NONE"`, no `Services GCP` VPC) | GCP auto-grants `roles/servicenetworking.serviceAgent` when `servicenetworking.googleapis.com` is enabled, but the grant is asynchronous and can lag beyond the 60 s API-enablement wait on fresh projects (including Qwiklabs). The module now explicitly provisions `google_project_service_identity.servicenetworking_sa` and grants `google_project_iam_member.servicenetworking_service_agent` before creating the PSA connection, so the Service Networking API's internal `compute.globalAddresses.list` call is never blocked by a missing IAM binding. |
| **VPC Service Controls perimeter, access policy & access levels** | `enable_vpc_sc = true` | `App_Common/modules/app_vpc_sc` auto-discovers the organization ID, reuses any existing Access Context Manager policy (or creates a new one), provisions four per-deployment access levels (VPC, admin IPs, IAP, CI/CD), and creates a `PERIMETER_TYPE_REGULAR` service perimeter covering every GCP API this module touches. Defaults to `vpc_sc_dry_run = true` so violations are logged-only until you validate and flip to enforcing. Auto-skips with a clear warning for standalone projects, folder-nested projects without `organization_id`, and deployments with no `admin_ip_ranges`. |

---

### Dependency on `Services GCP` for Shared Resources

`Services GCP` is declared as a module dependency (dependency on `Services GCP`) but is **not required** for a standalone deployment. The module self-provisions all necessary infrastructure inline when `Services GCP` has not been deployed. However, deploying `Services GCP` first is strongly recommended when multiple application modules share the same GCP project — it centralises shared infrastructure, reduces per-deployment cost, and simplifies ongoing management.

| Resource | Without `Services GCP` | With `Services GCP` |
|---|---|---|
| **VPC network** | Module auto-provisions an inline VPC, subnet, Cloud NAT, and Cloud Router. Inline Cloud SQL instances join the VPC via Private Service Access (PSA). | Module attaches to the shared centrally-managed VPC. Avoids per-project VPC and IP address quota consumption; simplifies firewall management across all deployments. No inline VPC and no inline Cloud SQL instance are created in this mode — see the note below the table. |
...
| **Binary Authorization** | `app_security` self-provisions the attestor, KMS key, and policy automatically (see above). | `Services GCP` (with `enable_binary_authorization = true`) provisions the same resources centrally with configurable `binauthz_evaluation_mode` (`ALWAYS_ALLOW`, `REQUIRE_ATTESTATION`, `ALWAYS_DENY`). Recommended for production environments requiring attestation enforcement. |
| **CMEK encryption** | `app_cmek` self-provisions the `${project_id}-cmek-keyring` and `storage-key` automatically (see above). | `Services GCP` (with `enable_cmek = true`) provisions the keyring with a configurable rotation period and applies CMEK across Cloud SQL, Artifact Registry, and other shared resources simultaneously, providing a consistent encryption baseline across all shared infrastructure. |
| **VPC Service Controls perimeter** | `app_vpc_sc` self-provisions the access policy, four access levels, and `PERIMETER_TYPE_REGULAR` perimeter scoped to this deployment (suffixed with `deployment_id`) — see above. | `Services GCP` can host a single shared perimeter across all deployments in the project, providing consistent ingress/egress rules and simpler enforcement. Recommended for organisations that already manage VPC-SC centrally; otherwise the self-provisioned per-deployment perimeter is sufficient. |
| **Project audit logging** | `enable_audit_logging = true` wires `google_project_iam_audit_config` for `allServices` plus Secret Manager and KMS overrides. Requires no additional infrastructure. | `Services GCP` (with `enable_audit_logging = true`) provisions the same audit configs once at the project level — recommended when multiple deployments share the project, to avoid duplicated IAM audit config resources. |

> **The two columns are mutually exclusive, not a per-resource fallback.** The "Without `Services GCP`" behaviour applies only when no VPC network is discovered. On the default path (`network_name = ""`) discovery matches a `Services GCP`-managed network specifically, so "no network found" and "no `Services GCP` deployment" coincide. Setting `network_name` explicitly to any existing VPC also satisfies the network check, but that does **not** give you the "With" column: the inline fallbacks are suppressed while Cloud SQL, NFS and registry discovery still find nothing (Cloud SQL and NFS filter on `managed-by=services-gcp`; registry discovery filters on `labels.module=services-gcp`, falling back to a `shared-repo-` name match with no label filter), so on the stock defaults (`database_type = "POSTGRES"`, `enable_nfs = true`) the preconditions below fail the plan. Each guard is conditional, so a deployment needing none of those resources — no database, no NFS, a prebuilt image, no CI/CD — trips none of them and applies against an arbitrary VPC. Once that network is found, this module will never create an inline Postgres instance, MySQL instance, NFS VM, or Artifact Registry repository, even if `Services GCP` was deployed without that particular resource. Each of those inline resources would share `Services GCP`'s exact tenant-prefixed name, so a silent per-resource fallback risks a later destroy-collision in which this module's apply deletes the shared resource that `Services GCP` still tracks in its own state. Instead, four preconditions fail the **plan** with an explicit message telling you to redeploy `Services GCP` with the missing resource enabled (`create_postgres`, `create_mysql`, `create_network_filesystem`, or a discoverable shared registry). Set `requires_services` (Group 0) so the platform enables the right `create_*` toggles when it auto-provisions `Services GCP` for you.
...
> **Many invalid combinations are caught at plan time.** The module carries 29 cross-variable preconditions (`validation.tf`) plus per-variable `validation` blocks, so a large class of misconfiguration fails the **plan** with a clear, named error before any resource is created. Rows below marked **🛡 plan-time** are rejected up front — you never reach the consequence described. Unmarked rows are *runtime* or *operational* hazards (a wrong port, a stale backup left enabled, a destructive rename) that the module cannot safely decide for you — they remain your responsibility. A clean plan confirms the value/combination rules passed; it does not validate that a port matches your app or that a rename is intentional.
...
| `enable_cdn` + `enable_cloud_armor` | Enable CDN only alongside Cloud Armor | **Medium** 🛡 plan-time | `enable_cdn = true` with `enable_cloud_armor = false` has no load balancer to attach CDN to — the setting was silently ignored; now rejected at plan time. |
| `cicd_enable_cloud_deploy` + `enable_cloud_deploy` | Set both together, or neither | **Medium** 🛡 plan-time | Routing CI/CD builds through Cloud Deploy without `enable_cloud_deploy = true` leaves the build with no delivery pipeline to release into — now rejected at plan time. |
| `mount_nfs` (jobs/sidecars) + `enable_nfs` | Enable NFS when any job or sidecar mounts it | **High** 🛡 plan-time | A job/sidecar with `mount_nfs = true` while `enable_nfs = false` (and no discoverable NFS server) references a volume that does not exist — now rejected at plan time. |
| `enable_postgres_extensions` / `enable_mysql_plugins` + `database_type` | Match the toggle to the engine family | **Medium** 🛡 plan-time | Enabling Postgres extensions on a MySQL engine (or vice-versa) is rejected at plan time. *(Defect fixed: the allow-lists now correctly include `POSTGRES_15` and `MYSQL_8_0`, which were previously rejected in error.)* |
| `application_name` | Short, lowercase, hyphen-safe (e.g. `"myapp"`, `"payments-api"`) | **Critical** | Embedded in every GCP resource name (Cloud Run service, secrets, SQL instance, GCS buckets). **Never change after first deploy** — all named resources are destroyed and recreated, causing complete data loss and a new empty database. |
| `tenant_id` | Match environment: `"prod"`, `"staging"`, `"dev"` | **Critical** | Embedded in all resource names alongside `application_name`. **Never change after first deploy** — same consequence as changing `application_name`: full resource recreation and data loss. |
| `enable_cloudsql_volume` | `true` (inject the Cloud SQL Auth Proxy sidecar — the secure path) | **High** | Default is `true` — correct for apps that use Cloud SQL. **Set to `false` for apps that do not use Cloud SQL** (e.g. stateless services or apps with an external DB): leaving it `true` injects an unused sidecar container, increasing cold-start time and resource consumption. Setting `false` when Cloud SQL is needed requires apps to reach the private IP directly — firewall rules must permit it and IAM authentication is lost. |
| `container_port` | Must exactly match what the application server binds to (e.g. `8080`, `3000`, `5000`) | **Critical** | Port mismatch: Cloud Run startup probe fails immediately on every instance. All requests return HTTP 502. The service never becomes healthy and continuously restarts. |
| `execution_environment` | `"gen2"` (always; required for NFS and GCS Fuse mounts) | **High** 🛡 plan-time | `"gen1"` with `enable_nfs = true` or `gcs_volumes` configured is rejected at plan time (gen1 does not support NFS/GCS Fuse mounts), rather than silently failing the volume mount at container start. |
| `ingress_settings` | `"internal-and-cloud-load-balancing"` when `enable_cloud_armor = true`; `"all"` for public services without a load balancer | **High** | `"all"` with `enable_cloud_armor = true`: the Cloud Run `*.run.app` URL remains publicly accessible, completely bypassing the load balancer and Cloud Armor WAF. Attackers can reach the app directly, making the WAF investment worthless. |
| `vpc_egress_setting` | `"PRIVATE_RANGES_ONLY"` for apps only needing private VPC resources; `"ALL_TRAFFIC"` for apps calling external APIs through Cloud NAT | **High** | `"PRIVATE_RANGES_ONLY"` blocks all outbound traffic to public IPs including external Redis, third-party APIs, and SaaS webhooks. Apps that call external services must set `"ALL_TRAFFIC"`. Symptom: silent connection timeouts to anything outside RFC 1918. |
| `enable_iap` + `iap_authorized_users` / `iap_authorized_groups` | Set `enable_iap = true` only after populating at least one user or group | **Critical** 🛡 plan-time | `enable_iap = true` with both lists empty would lock everyone out (100% HTTP 403, including you) — now rejected at plan time, so you cannot deploy an inaccessible service. Add at least your own email to `iap_authorized_users`. |
| `enable_cloud_armor` + `ingress_settings` | Always pair `enable_cloud_armor = true` with `ingress_settings = "internal-and-cloud-load-balancing"` | **High** | `enable_cloud_armor = true` with `ingress_settings = "all"`: Cloud Armor protects the load balancer path, but the `*.run.app` URL bypasses it entirely. Clients that discover the direct URL evade all WAF rules and rate limits. |
| `min_instance_count` | `1` for latency-sensitive or stateful production apps; `0` for dev/batch | **High** | `0` for stateful apps (databases, persistent sessions): on scale-to-zero, any in-memory or local-disk state is lost when the instance shuts down. On the next request, the new instance starts cold with no prior state. Risk of data loss or corrupted session state. |
| `enable_backup_import` | `false` (default) — set `true` only when you want to restore | **Critical** | Leaving `enable_backup_import = true` after a successful restore: the import job re-runs on every subsequent deployment, overwriting the live database with the stale backup file. **Always set back to `false` immediately after a successful restore.** |
| `backup_file` | Must exactly match the filename present in the backup source (including extension) | **High** | Filename mismatch: the import Cloud Run Job fails immediately with a "file not found" error. No data is imported but the deployment apply succeeds — the failure is only visible in the job execution logs. |
| `secret_rotation_period` | `"2592000s"` (30 days) — must include the `s` suffix | **High** 🛡 plan-time | Omitting the `s` suffix (e.g. `"2592000"`) is rejected at plan time by the variable's format validation, rather than being accepted and silently failing to register the rotation notification. |
| `enable_auto_password_rotation` + `rotation_propagation_delay_sec` | Enable only after validating the rotation pipeline; keep `rotation_propagation_delay_sec` ≥ `90` | **High** | `rotation_propagation_delay_sec` too short (e.g. `10`): the old DB password version is disabled before all running pods have restarted with the new credential. Connection pool exhaustion causes HTTP 500 errors until all pods complete a full restart cycle. |
| `enable_binary_authorization` + `binauthz_evaluation_mode` | Start with `"ALWAYS_ALLOW"`; promote to `"REQUIRE_ATTESTATION"` only after CI pipeline produces valid attestations | **Critical** | `"REQUIRE_ATTESTATION"` without a CI pipeline that attests images: **all new revisions fail to deploy** with `Image is not attested`. Rollbacks are also blocked. `"ALWAYS_DENY"` immediately blocks all deployments including emergency hotfixes. |
| `enable_vpc_sc` + `vpc_sc_dry_run` | Always enable with `vpc_sc_dry_run = true` first; review audit logs before enforcing | **Critical** | `vpc_sc_dry_run = false` on first enable: any principal, IP, or service not explicitly in the access level is blocked immediately. Cloud Build SA, Cloud Run SA, and your admin IP must all be in an access level before enforcing — otherwise Cloud Build, deployments, and the running app all fail simultaneously. |
| `organization_id` | Required for folder-nested projects when `enable_vpc_sc = true` | **High** | Folder-nested project without `organization_id`: VPC-SC perimeter is silently skipped with a warning. No error is raised, but no perimeter is created — the security control you intended is absent. |
| `cloud_deploy_stages` prod stage | `require_approval = true` on the production stage | **Critical** | `require_approval = false` combined with `auto_promote = true` on the prod stage: a successful staging deployment automatically promotes to production without any human review. A broken build or bad migration reaches production automatically. |
| `enable_image_mirroring` | `true` (default; strongly recommended) | **High** | `false`: Cloud Run pulls images directly from external registries (Docker Hub, GHCR). Subject to pull rate limits (HTTP 429 during deployments). In VPC-SC environments, external registry access is blocked — new revisions fail with `ImagePullBackOff` and the service cannot be updated. |
| `database_type` | Version-pinned value for production (e.g. `"POSTGRES_15"`) rather than generic `"POSTGRES"` | **Critical** | **Changing `database_type` after first deploy replaces the Cloud SQL instance** — all data in the existing database is destroyed. Never change this after the first apply without a validated backup and a tested restore plan. |
| `application_database_name` | Lowercase, tied to the app (e.g. `"myappdb"`) — never change after first deploy | **Critical** | Changing creates a new empty database with the new name; the original database with all data is orphaned on the instance. The application starts against an empty schema and fails on every DB call. |
| `storage_buckets[].force_destroy` | `false` for production buckets containing user data | **Critical** | `true` on a production bucket: removing the deployment permanently deletes all bucket contents with no recovery. `false` on a temporary bucket that needs deletion: removing the deployment fails until the bucket is manually emptied. |
| `startup_probe_config.path` | A fast, lightweight endpoint that returns HTTP 200 when the app is ready (e.g. `"/healthz"`) | **Critical** | Wrong path (returns 404 or 500): Cloud Run never considers the instance healthy and never routes traffic to it. The instance is continuously restarted. The service is offline for all users. |
| `enable_redis` | `false` for apps that do not use Redis | **Medium** | Left as the default `true` for a non-Redis app: `REDIS_HOST` defaults to the NFS server IP. If no Redis-compatible service is running there, the application logs connection errors on every startup. Set `false` explicitly for apps that do not use Redis. |
| `max_revisions_to_retain` | `7` (default) — sufficient for most rollback needs | **Low** | `0`: all old revisions pruned immediately. Rollback to any prior revision is impossible — only the currently-serving revision is kept. Too high (e.g. `100`): Cloud Run revision list API quota is consumed faster and the console is difficult to navigate. |
| `secret_propagation_delay` | `30` seconds (default); increase to `60`–`90` for multi-region deployments | **Medium** | Too short in multi-region setups: a new Cloud Run revision starts before the new secret version has replicated globally. The instance reads the old (potentially just-rotated) credential and fails to authenticate. Increase if you see intermittent secret-not-found errors on fresh deployments. |
...
| `uptime_check_names` | List of uptime check configuration names; empty when the uptime check is disabled or the endpoint is not publicly reachable |
...
| `tenant_id` | Tenant identifier derived from `tenant_id` |
