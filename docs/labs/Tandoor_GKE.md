---
title: "Tandoor on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Tandoor on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Tandoor on GKE Autopilot — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Tandoor_GKE)**
...
operational lifecycle of the **Tandoor on GKE Autopilot** module on Google
...
the [Configuration Guide](https://docs.radmodules.dev/docs/modules/Tandoor_GKE) —
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Tandoor
...
2. The platform deploys the workload into the GKE Autopilot cluster,
   ...
   secrets (`SECRET_KEY`, `DJANGO_SUPERUSER_PASSWORD`, and the database
   password), a Cloud Storage `data` bucket, and runs two one-shot jobs:
   `db-init` (creates the database and user) and `create-superuser`
   (bootstraps the initial admin account). First deploys take roughly
...
2. Confirm the service is healthy. Tandoor has no dedicated unauthenticated
   health endpoint, so check Django's public login page:

   ```bash
   curl -s -o /dev/null -w '%{http_code}\n' "http://${EXTERNAL_IP}/accounts/login/"   # expect 200
   ```

3. Retrieve the generated superuser credential — Tandoor has **no
   self-registration flow and no fixed default credential**:
...
4. Open `http://${EXTERNAL_IP}/accounts/login/` in a browser and log in with
   username `admin` (or your configured `admin_username`) and the password
   retrieved above.
...
1. **Inspect the workload** — deployment, pods, and the horizontal
   autoscaler:
...
2. **Scale** by changing the min/max instance inputs and clicking **Update**
   ...
   worker, so scaling beyond one replica needs no special coordination.

3. **Update the application version** by changing the version input in the
   RAD platform and applying it via **Update**; a rolling update replaces
   the pods. Tandoor publishes a genuine `latest` tag upstream.
...
- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. The startup
  probe targets `/accounts/login/` and requires Postgres connectivity plus
  applied migrations to return 200; a connection failure to PostgreSQL will
...
- **Database connection errors:** confirm the Cloud SQL instance is
  ...
  the `db-init` job completed.
- **Can't log in / no credential known:** retrieve
  `DJANGO_SUPERUSER_PASSWORD` from Secret Manager (Task 2, step 3) — there is
  no fixed fallback credential.
- **`create-superuser` job failed:** inspect the job and its pod logs — a
  common cause is the job racing `db-init` (GKE's `execute_on_apply` only
  gates whether Terraform waits, not whether the pod is scheduled before
  `db-init` finishes; the job retries up to twice):
  ```bash
...
- **Image pull errors:** confirm the image exists in Artifact Registry and
  ...
  this only applies if `container_image_source` was overridden to
  `custom`).
...
makes RAD forget the deployment). This removes everything the module
