...
📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Formbricks_GKE)**
...
Formbricks product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Formbricks_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Formbricks (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL 15) database with its Secret Manager secrets, an NFS volume,
   a GCS uploads bucket, Redis caching, builds the container image, and runs a
...
2. Open `http://${EXTERNAL_IP}` in a browser. On first visit Formbricks displays a
   sign-up screen — create the initial admin account and workspace directly in the UI.
   The admin credential you set here is managed by Formbricks itself; application-level
...
- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. Formbricks performs
  Prisma migrations at container start, so early failures often appear as database
  connection or migration errors in the logs.
  ```bash
...
and namespace, Cloud SQL database, Secret Manager secrets, GCS buckets, NFS resources,
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (PostgreSQL 15), NFS, GCS uploads bucket, Redis, secrets, and runs DB init |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes at `/api/v2/health`; create admin account via the UI |
