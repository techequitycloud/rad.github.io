# Credits Management System: Technical Documentation

This document provides a comprehensive technical overview of the Credits Management System. It is intended for engineers and technical staff responsible for maintaining and extending the platform.
...
### 3.4. Successful Execution vs. Failure

- **Success**: The user's credit balances are updated in Firestore. The `useUsers` React Query key is invalidated, causing the table to refetch and display the new data.
- **Failure**: An API error occurs. The UI remains in "edit" mode, and an error may be logged to the console.
...
The core logic resides in the `/api/revenue` endpoint:

1. **Fetch Data**: The API fetches all users, their deployments within the specified date range, and all their credit transactions.
2. **Calculate Total Free Credits**: For each user, the API calculates the total sum of their "awarded" credits by looking at all transactions *not* of type "PURCHASE".
3. **Process Deployments Chronologically**: The API processes each user's deployments in chronological order.
4. **Apply Free Credits**: It maintains a running total of `spentFreeCredits`. For each deployment, it "spends" the user's free credits first.
5. **Calculate Revenue**: If a deployment's cost exceeds the remaining free credits, the portion of the cost paid for with the overflow is considered revenue. This revenue is calculated as `(creditCost - freeCreditsApplied) / creditsPerUnit`.
6. **Return Data**: The API returns a list of only the deployments that generated revenue.
...
- **Admins**: Can view revenue data for any user by providing a comma-separated list of emails. If no emails are provided, they see revenue data for all users.
...
### 11.2. Dependencies & Required Permissions:
-   **BigQuery Data Access**: The function's service account needs the `BigQuery User` role to query the billing table.
-   **Cloud Billing API**: Requires the `billing.resourceAssociations.update` permission on the organization or billing account to disable/re-enable billing. This is a high-privilege permission and should be managed carefully.

---

## 12. Function Deep Dive: `low_credit`

-   **File Location**: `rad-ui/automation/terraform/infrastructure/function/low_credit/index.js`
-   **Purpose**: A simple, proactive monitoring and notification utility.
-   **Trigger**: **Cloud Scheduler** (typically runs daily).

### 12.1. Execution Flow & Logic:
1.  **Setting Retrieval**: Fetches the `lowCreditTriggerAmount` from the `settings` collection. Exits if not set.
2.  **User Iteration**: Loops through all users in the `users` collection.
3.  **Balance Check**: For each user, it compares their `creditBalance` to the `lowCreditTriggerAmount`.
4.  **Notification Throttling**: If the balance is low, it checks the `lastLowCreditNotificationSentAt` timestamp on the user's document. It will only proceed if a notification has not been sent within the last 24 hours.
5.  **Send Email**: If conditions are met, it sends a warning email via **Nodemailer** and updates the `lastLowCreditNotificationSentAt` timestamp to the current time.

---

## 13. Maintenance & Debugging

-   **Logging**: All functions are instrumented with `logger` calls. Check Google Cloud Logging for detailed execution traces, warnings, and errors.
-   **Firestore Data**: When debugging an issue with a specific user, inspect their document in the `users` collection (for balance and timestamps), their `deployments`, and the `credit_transactions` collection.
-   **BigQuery Billing Table**: If users are not being billed correctly, verify that the billing export to the BigQuery table specified by `BIGQUERY_BILLING_TABLE` is active and that the table contains recent data.
-   **IAM Permissions**: If the `project_credits` function fails to disable/re-enable billing, check the IAM permissions for its service account.
