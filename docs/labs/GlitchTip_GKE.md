...
1. In the RAD platform, open **GlitchTip (GKE)**, set `project_id`, and review the inputs.
   ...
   every input by group, with defaults. Click **Deploy Module**, review the estimated cost in
...
it, use **Purge** instead (from the same **Delete** dialog) — it removes the deployment from RAD's records **without**
