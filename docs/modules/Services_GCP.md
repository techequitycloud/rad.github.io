...
2. **Deployment identity**: the service account used by the platform to apply the module needs permission to create the shared infrastructure it manages. Prefer the least-privilege bundle that `Project GCP` grants its deploying identity (see that module's guide) over a broad primitive role; grant it in the target project, ideally time-limited and conditional. `roles/owner` will work but is far wider than required — if you grant it when onboarding an external project, scope it to the onboarding window rather than leaving it in place.
...
| `create_postgres` | `false` | Provision a Cloud SQL PostgreSQL instance in the primary region. The general-purpose choice for relational workloads — but **off by default**, so a deployment that needs Postgres must ask for it. Application modules that require it declare `create_postgres = true` in their own `requires_services` map, which the platform reads when auto-provisioning this module. |
