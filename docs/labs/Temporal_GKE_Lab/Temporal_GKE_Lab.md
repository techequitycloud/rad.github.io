...
sidebar_label: "Temporal GKE Lab"
---
...
- Creates a PostgreSQL database user and two databases (`temporal` for persistence, `temporal_visibility` for advanced visibility) inside the Services_GCP Cloud SQL instance
...
- GCP project ID and the Services_GCP outputs: `postgres_instance_name` and `postgres_instance_ip`
...
| `postgres_instance_name` | Yes | — | Cloud SQL instance name from Services_GCP |
| `postgres_instance_ip` | Yes | — | Private IP of Cloud SQL instance from Services_GCP |
...
> **Note:** The Cloud SQL PostgreSQL instance itself is managed by Services_GCP and is not affected. Resources provisioned by the `Services_GCP` module must be undeployed via their own RAD UI deployment entry.
