...
<video controls width="100%" poster="https://storage.googleapis.com/rad-public-2b65/gcp/pde_section2.png">
  <source src="https://storage.googleapis.com/rad-public-2b65/gcp/pde_section2.mp4" type="video/mp4" />
  Your browser does not support the video tag.
</video>

<br/>

[Download PDF](https://storage.googleapis.com/rad-public-2b65/gcp/pde_section2.pdf)
...
> **App_CloudRun — Two-Flag Cloud Deploy Requirement:** The App_CloudRun module (Group 7) exposes two separate flags for Cloud Deploy that must *both* be set to `true` for a fully automated pipeline: `enable_cloud_deploy = true` provisions the Cloud Deploy delivery pipeline and targets; `cicd_enable_cloud_deploy = true` configures the Cloud Build trigger to create Cloud Deploy *releases* (rather than deploying directly to Cloud Run). Setting only one flag produces a partial configuration — the pipeline is created but not triggered, or the trigger fires but no pipeline exists to receive the release. The App_GKE module requires only `enable_cloud_deploy = true` because GKE deployments always go through Cloud Deploy when the flag is enabled. Understanding this module-specific distinction is important when configuring Cloud Deploy in the RAD platform.
...
### App_CloudRun vs App_GKE — CI/CD and Deployment Differences

The two modules implement CI/CD and deployment patterns differently. Understanding these differences is important for PDE exam questions that ask you to choose the appropriate deployment strategy for a given workload.

| Feature | App_CloudRun | App_GKE |
|---|---|---|
| **Canary deployments** | Native `traffic_split` variable (Group 3) — splits traffic between named revisions without Cloud Deploy | Implemented via Cloud Deploy `cloud_deploy_stages` with separate namespace or cluster targets per stage |
| **Scale-to-zero** | `min_instance_count = 0` fully supported — instances terminate when idle, eliminating compute costs | Not natively supported in GKE Autopilot Deployments; `min_instance_count` minimum is effectively `1` |
| **Cloud Deploy flags** | Requires `enable_cloud_deploy = true` AND `cicd_enable_cloud_deploy = true` | Requires only `enable_cloud_deploy = true` |
| **Secret delivery** | Secret Manager references injected at startup — plaintext value never in Terraform state | Default: Kubernetes Secrets (stored in etcd). Secure alternative: Secrets Store CSI Driver (`enable_secrets_store_csi_driver = true`) — secrets fetched directly from Secret Manager at pod start, never in etcd or state |
| **Backup executor** | Cloud Run Job triggered by Cloud Scheduler | Kubernetes CronJob |
| **Rollback mechanism** | Cloud Run revision traffic management (instant, UI-driven) or Cloud Deploy rollback | `kubectl rollout undo` or Cloud Deploy rollback |
| **Binary Authorization** | Enforces on the Cloud Run service — deployment rejected if image lacks attestation | Enforces on the GKE cluster — pod scheduling rejected if image lacks attestation |

The right choice between App_CloudRun and App_GKE depends on workload requirements: Cloud Run is preferable for event-driven or variable-traffic workloads that benefit from scale-to-zero; GKE is preferable for workloads requiring persistent network identity, StatefulSet semantics, service mesh capabilities, or fine-grained scheduling control.
