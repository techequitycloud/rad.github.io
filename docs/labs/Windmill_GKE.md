---
title: "Windmill on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Windmill on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Windmill on GKE Autopilot — Lab Guide
...
Windmill is an open-source developer platform for building internal tools, scripts,
workflows, and automations. This lab takes you through the full operational lifecycle
of the **Windmill on GKE Autopilot** module on Google Cloud: deploy it, access and
...
Windmill product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Windmill_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Windmill (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (PostgreSQL 16) database with its Secret Manager secrets, a GCS
   `data` bucket, builds the container image, and runs a one-shot
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep windmill | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}/api/version"   # expect a JSON response with the Windmill version
   ```

2. Open `http://${EXTERNAL_IP}` in a browser. The Windmill login page appears. The
   first account you create is automatically granted the `super-admin` role — there is
   no pre-provisioned admin password. Create your admin account directly in the
   Windmill UI.
...
4. **Manage secrets, storage, and jobs:**
   ...
   gcloud storage buckets list --project="$PROJECT" --filter="name~windmill"
   ```
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. windmilldemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^windmill" --limit=1)
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   ...
   **uptime check** targeting `/api/version` (when enabled); review Monitoring →
...
platform-level diagnostics and do not change with Windmill releases.
...
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  ...
  Windmill requires PostgreSQL 16 — verify the instance engine version.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (PostgreSQL 16), secrets, GCS bucket, and runs DB init |
| 2 — Access & verify | Manual | Connect to the cluster; health check passes at `/api/version`; first user becomes super-admin |
