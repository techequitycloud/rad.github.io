...
lifecycle of the **Wiki.js on Cloud Run** module on Google Cloud: deploy it, access and
...
not on Wiki.js product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Wiki.js (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL 15) database
   with the `pg_trgm` extension and its Secret Manager secrets, optional NFS/GCS
   storage, builds the container image, and runs a one-shot database-initialisation
   job. First deploys take roughly **20–30 minutes** (Cloud SQL creation dominates).
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~wikijs" --format="value(metadata.name)" --limit=1)
...
1. Confirm the service is healthy and connected to its database (Wiki.js binds on
   port 3000 and exposes `/healthz`):

   ```bash
   curl -s -o /dev/null -w "%{http_code}\n" "${SERVICE_URL}/healthz"   # expect 200
   ```

2. Open `${SERVICE_URL}` in a browser. On first run Wiki.js presents a **setup
...
- **Revision unhealthy / service won't serve:** inspect the latest revision and its
  logs for startup errors. Wiki.js has a 60-second startup probe initial delay to
  allow the first-boot PostgreSQL schema migration to complete — premature probe
  failures are usually caused by reducing this delay.
  ```bash
  gcloud run revisions list --service="$SERVICE" --project="$PROJECT" --region="$REGION"
  gcloud run services logs read "$SERVICE" --project="$PROJECT" --region="$REGION" --limit=100
  ```
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret exists, and the `pg_trgm` extension was installed by the
  initialisation job.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL (PostgreSQL 15 + pg_trgm), secrets, and runs DB init |
| 2 — Access & verify | Manual | Health check passes at `/healthz`; complete setup wizard or sign in |
