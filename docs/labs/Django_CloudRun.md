...
description: "Hands-on lab: deploy Django on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
the **Django on Cloud Run** module on Google Cloud: deploy it, access and verify it,
...
not on Django product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Django (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
   inputs. Configure only what you need — the
   [Configuration Guide](https://docs.radmodules.dev/docs/modules/Django_CloudRun)
   documents every input by group, with defaults. Review the estimated cost (if credits are enabled) and click **Deploy**, which opens the deployment status page with real-time logs.

2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL) database
   with its Secret Manager secrets, a Filestore NFS share and Cloud Storage media
   bucket, builds the container image, and runs two one-shot jobs: `db-init` (creates
   the database, user, and PostgreSQL extensions) and `db-migrate` (runs
   `manage.py migrate` and `collectstatic`). First deploys take roughly
...
- **Revision unhealthy / service won't serve:** inspect the latest revision and its
  ...
  startup probe targets `GET /healthz` on port 8080; ensure your Django app exposes
  a lightweight `/healthz` view that returns HTTP 200 without redirects — setting
  `SECURE_SSL_REDIRECT = True` in `settings.py` will cause the probe to receive a
  301 and the service will never become healthy.
  ```bash
  gcloud run revisions list --service="$SERVICE" --project="$PROJECT" --region="$REGION"
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module provisions Cloud Run, Cloud SQL, NFS, GCS, secrets, and runs db-init + db-migrate |
