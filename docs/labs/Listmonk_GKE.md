---
title: "Listmonk on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Listmonk on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Listmonk on GKE Autopilot — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Listmonk_GKE)**
...
**Listmonk on GKE Autopilot** module on Google Cloud: deploy it, access and verify it,
...
Listmonk product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Listmonk_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Listmonk (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster (on port 9000),
   provisions a Cloud SQL (PostgreSQL 15) database with its Secret Manager secrets, a
   GCS bucket for media uploads, builds the container image, and runs a one-shot
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep listmonk | head -1 | cut -d/ -f2)
...
2. Open `http://${EXTERNAL_IP}` in a browser. On first access, Listmonk will prompt
   you to create the initial superadmin account. A pre-generated password was stored
   in Secret Manager at deploy time for reference — retrieve it with:
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. listmonkdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^listmonk" --limit=1)
...
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  ...
  Listmonk applies schema migrations automatically on startup via `--install`; confirm
  no migration errors appear in the pod logs.
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (PostgreSQL 15), GCS uploads bucket, secrets, and runs DB init |
| 2 — Access & verify | Manual | Connect to the cluster; HTTP check passes; admin account created on first login |
