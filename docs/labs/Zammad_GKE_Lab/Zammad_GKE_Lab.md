...
2. The `Services_GCP` module deployed in the same project (includes GKE Autopilot cluster).
...
Resources provisioned by `Services_GCP` (VPC, GKE cluster, Cloud SQL, NFS server) must be undeployed via their own RAD UI deployment entry.
