...
| `execution_environment` | `"gen2"` (always; required for NFS and GCS Fuse mounts) | **High** | `"gen1"` with `enable_nfs = true` or `gcs_volumes` configured: NFS/GCS Fuse mounts silently fail at container start. All instances fail with a volume-mount error. Gen1 does not support NFS. |
...
| `enable_iap` + `iap_authorized_users` / `iap_authorized_groups` | Set `enable_iap = true` only after populating at least one user or group | **Critical** | `enable_iap = true` with empty `iap_authorized_users` and `iap_authorized_groups`: **100% of requests return HTTP 403** — including your own. The service is deployed but completely inaccessible until IAM is updated. Always add at least your own email to `iap_authorized_users` before enabling. |
...
| `secret_rotation_period` | `"2592000s"` (30 days) — must include the `s` suffix | **High** | Omitting the `s` suffix (e.g. `"2592000"` instead of `"2592000s"`): Secret Manager rejects the rotation configuration. The plan may succeed but the rotation Pub/Sub notification is never registered. Rotation never fires. |
