---
title: "Umami on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Umami on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Umami on GKE Autopilot — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Umami_GKE)**
...
takes you through the full operational lifecycle of the **Umami on GKE Autopilot** module on
...
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Umami_GKE) — this lab
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Umami (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs. Configure
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a Cloud SQL
   (PostgreSQL) database with its Secret Manager secrets, builds the container image, and
   runs a one-shot database-initialisation job. First deploys take roughly **20–35 minutes**
...
2. Open `http://${EXTERNAL_IP}` in a browser. Sign in with the default credentials
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. umamidemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^umami" --limit=1)
...
2. **Monitoring** — open the GKE / Kubernetes dashboards and review pod CPU and memory
   ...
   **uptime check** targeting `/api/heartbeat` (when enabled); review Monitoring → Uptime
...
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the DB
  password secret materialised into the namespace, and the init job completed. Note that
  Umami runs its own Prisma migrations on first startup — the startup probe allows up to
  5 minutes for this; a low `failure_threshold` can cause a restart loop on fresh
  deployments.
