---
title: "Activepieces on GKE Autopilot \u2014 Lab Guide"
description: "Hands-on lab: deploy Activepieces on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
the full operational lifecycle of the **Activepieces on GKE Autopilot** module on
...
on Activepieces product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Activepieces (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   ...
   (`AP_ENCRYPTION_KEY`, `AP_JWT_SECRET`, and the database password), a Cloud
   Storage data bucket, builds the container image, and runs a one-shot
   database-initialisation job (including `pgvector` extension setup). First deploys
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep activepieces | head -1 | cut -d/ -f2)
...
2. Confirm the service is healthy. Activepieces exposes a flags endpoint that
   responds only when the server is fully initialised and PostgreSQL is reachable:

   ```bash
   curl -s "http://${EXTERNAL_IP}/api/v1/flags"   # expect a JSON object with flag values
   ```

3. Open `http://${EXTERNAL_IP}` in a browser. On first visit Activepieces prompts
   you to create the initial administrator account. Fill in your name, email, and a
...
2. **Scale** by changing the min/max instance inputs and clicking **Update** on the deployment details page —
   ...
   that scaling beyond one replica requires Redis (`enable_redis = true`); without
   it each pod maintains its own in-memory queue. Session affinity (`ClientIP`) is
   set by default to keep WebSocket connections stable.
...
5. **Open a database session** for inspection or maintenance:
   ...
   # Role and database are tenant-prefixed (e.g. activepiecesdemo426161cf) — not the bare app name.
   DB_USER=$(gcloud sql users list --instance="$INSTANCE" --project="$PROJECT" \
     --format="value(name)" --filter="name~^activepieces" --limit=1)
