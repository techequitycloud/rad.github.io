...
2. The `Services GCP` module deployed in the same project (provides VPC, Cloud SQL, NFS server, and Redis).
...
Resources provisioned by `Services GCP` (VPC, Cloud SQL instance, GKE cluster, NFS server) are managed separately and must be undeployed via their own RAD UI deployment entry.
