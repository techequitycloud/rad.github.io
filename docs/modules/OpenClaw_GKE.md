...
| Ingress | Cloud Load Balancing | `LoadBalancer` by default (public external IP); `ClusterIP` or a custom domain available |
...
By default the workload is exposed as a `LoadBalancer` service with a public external IP.
Set `service_type = "ClusterIP"` for internal-only access (typically behind an OpenClaw
router service), or enable a custom domain via the Kubernetes Gateway API for direct
external access on a stable hostname.
