...
| Auto-generated secrets | Creates `CRAWL4AI_API_TOKEN` and `SECRET_KEY` in Secret Manager and returns them in the `secret_ids` map | Inject LLM keys via `secret_environment_variables` |
...
| `CRAWL4AI_API_TOKEN` | auto-generated | Bearer token for API calls. Required — with it unset the image binds Gunicorn to loopback only, so the health probe never succeeds |
| `SECRET_KEY` | auto-generated | JWT signing secret, pinned so issued tokens survive restarts |
