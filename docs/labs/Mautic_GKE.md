...
description: "Hands-on lab: deploy Mautic on GKE Autopilot in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---

# Mautic on GKE Autopilot — Lab Guide

📖 **[Configuration Guide](https://docs.radmodules.dev/docs/modules/Mautic_GKE)**
...
operational lifecycle of the **Mautic on GKE Autopilot** module on Google Cloud:
...
Mautic product features. For the complete list of provisioned services and every
configuration input (organised by group), see the
[Configuration Guide](https://docs.radmodules.dev/docs/modules/Mautic_GKE) — this
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Mautic (GKE)** from the **Platform Modules** list to start configuration, set `project_id`, and review the inputs.
...
2. The platform deploys the workload into the GKE Autopilot cluster, provisions a
   Cloud SQL (MySQL 8.0) database with its Secret Manager secrets, optional
   NFS/Redis, builds the container image, and runs a one-shot installation job that
   creates the Mautic schema and admin user. First deploys take roughly
...
3. Connect to the cluster and discover the namespace with name-agnostic filters:
   ...
   NS=$(kubectl get ns -o name | grep mautic | head -1 | cut -d/ -f2)
...
1. Confirm the workload is running and find its external address:
   ...
   curl -s "http://${EXTERNAL_IP}/healthz"   # expect a 200 response
   ```

2. Retrieve the admin password from Secret Manager and sign in at
   `http://${EXTERNAL_IP}/s/login`:
...
4. **Manage secrets, storage, and jobs:**
   ...
   kubectl get jobs -n "$NS"          # install job and any scheduled jobs
   ```
...
- **Pod not Ready / CrashLoopBackOff:** inspect events and logs. Note that Mautic
  runs database migrations on first boot — allow additional time before declaring a
  failure.
  ```bash
...
- **Installation job failed:** inspect the job and its pod logs:
  ```bash
...
| Task | Type | Outcome |
|---|---|---|
| 1 — Deploy | Automated | Module deploys the GKE workload, Cloud SQL (MySQL 8.0), secrets, and runs the install job |
...
| 5 — Troubleshoot | Manual | Diagnose pod, database, install-job, scheduling, and image-pull issues |
