...
7.  **Mail Server Email / Mail Server Password:** Enter your SMTP credentials so the system can send emails. Also check **Email Notifications** to activate them.
...
3.  **Platform GitHub Token:** Enter a GitHub Personal Access Token that has access to your modules repository. Click **Save Token**.
4.  **Platform GitHub Repository:** Once the token is saved, select your repository from the dropdown list. Click **Update Repo**.
...
4.  **Credits Per Hour:** Enter `100` (cost per hour for build time). This ensures users are charged for the duration of their deployment builds. Click **Save**.
5.  **Low Credit Notification:** Enter a value (e.g., `50`) to notify users when their balance is low. Click **Save**.
6.  **Monthly Credits:** Enable this feature and set an amount (e.g., `200`) to give users recurring monthly credits. Click **Save**.
