...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Passbolt
   ...
   the two variants don't collide on shared resource names. Click **Deploy
   Module**, review the estimated cost in the confirmation dialog (if credits are enabled) and click **Confirm**, which opens
