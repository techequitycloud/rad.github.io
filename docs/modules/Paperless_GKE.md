...
| Initialization Jobs & CronJobs | §3.E Initialization Jobs & CronJobs | No default `db-init` job — Paperless-ngx runs migrations automatically on startup; see [Group 8: Jobs & Scheduled Tasks](#group-8-jobs--scheduled-tasks). |
...
5. **No default `db-init` job is supplied.** Paperless-ngx runs Django database migrations automatically on container startup via the entrypoint script. There is no equivalent of Ghost's `db-init` Kubernetes Job.
...
8. **`timeout_seconds` defaults to 300 seconds.** OCR processing on large documents can be slow. The higher timeout prevents Kubernetes from terminating slow ingestion requests prematurely.
...
| `timeout_seconds` | `300` | `30` | Request timeout in seconds. OCR on large, multi-page PDFs can take several minutes. 300 seconds is the recommended minimum. |
...
| `enable_image_mirroring` | `true` | `false` | Paperless-ngx images are hosted on GHCR. Mirroring to Artifact Registry avoids rate limits and satisfies Binary Authorization requirements. |
...
| `PAPERLESS_SECRET_KEY` | Django application secret key. Used for session signing and CSRF protection. Auto-generated and rotated on the schedule defined by `secret_rotation_period`. |
...
The standard secrets variables (`environment_variables`, `secret_environment_variables`, `secret_rotation_period`, `secret_propagation_delay`, `manage_storage_kms_iam`) behave as described in [App_GKE](App_GKE.md).
...
**No default `db-init` job:** Unlike Ghost GKE, `Paperless GKE` does not supply a default `db-init` Kubernetes Job. Paperless-ngx runs Django database migrations automatically as part of the container entrypoint on first boot. The `initialization_jobs` list is empty by default.
...
The `cron_jobs` variable is available for tasks such as periodic document re-classification or index rebuilds. See [App_GKE](App_GKE.md) for full schema documentation.
...
gcloud secrets versions access latest \
  --secret="PAPERLESS_ADMIN_PASSWORD" \
  --project=PROJECT_ID
...
These variables configure Paperless-ngx's Redis integration. The underlying Redis infrastructure support is provided by `App_GKE` (see [App_GKE](App_GKE.md)). Redis is **not optional** for Paperless-ngx — it is the Celery message broker that drives all background document processing, OCR tasks, and the consumption pipeline.
...
| `storage_buckets` | Created GCS storage buckets (includes the auto-provisioned `paperless-media` bucket) |
