...
| GKE Backend Configuration | [§3.A Compute (GKE Autopilot)](../App_GKE/App_GKE.md#a-compute-gke-autopilot) | Refer to base App GKE module documentation. `session_affinity` defaults to `"ClientIP"`. `deployment_timeout` defaults to `1200`. |
| Jobs & Scheduled Tasks | [§3.E Initialization Jobs & CronJobs](../App_GKE/App_GKE.md#e-initialization-jobs--cronjobs) | Refer to base App GKE module documentation. The module injects a platform-managed `nfs-init` initialisation job — see [Platform-Managed Behaviours](#platform-managed-behaviours). |
...
| Reliability Policies | [§7 Reliability & Scheduling](../App_GKE/App_GKE.md#7-reliability--scheduling) | Refer to base App GKE module documentation. `enable_pod_disruption_budget` defaults to `false` (because the default `max_instance_count = 1` makes a PDB with `min_available = 1` block node drains). |
...
| **OE_PASS secret** | An OpenEMR admin password is auto-generated by `OpenEMR Common` and stored in Secret Manager. The plaintext value is passed to `App GKE` via `explicit_secret_values`, which creates a Kubernetes Secret and injects it as the `OE_PASS` environment variable. OpenEMR uses this on first boot to set the administrator account password. |
| **MYSQL_PASS secret** | The MySQL database password generated by `App GKE` is automatically injected as the `MYSQL_PASS` environment variable via the `db_password_env_var_name = "MYSQL_PASS"` parameter passed to `App GKE`. Do not define this manually in `secret_environment_variables`. |
...
All other `environment_variables` and `secret_environment_variables` behaviour is identical to App_GKE — refer to [App_GKE §3.A](../App_GKE/App_GKE.md#a-compute-gke-autopilot).
...
For the full NFS variable reference (Filestore instance sizing, capacity, etc.), refer to [App_GKE §3.C](../App_GKE/App_GKE.md#c-storage-nfs--gcs--gcs-fuse).
...
For the full variable reference, refer to [App_GKE §8.B](../App_GKE/App_GKE.md#b-backup-import).
