...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Headscale
   ...
   file-locking support. Click **Deploy Module**, review the estimated cost in
   the confirmation dialog (if credits are enabled) and click **Confirm**, which opens the deployment status page with
   real-time logs.
