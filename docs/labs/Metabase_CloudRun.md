---
title: "Metabase on Cloud Run \u2014 Lab Guide"
description: "Hands-on lab: deploy Metabase on Cloud Run in your own Google Cloud project — guided setup, verification, operations, observability, and teardown."
---
...
full operational lifecycle of the **Metabase on Cloud Run** module on Google Cloud:
...
not on Metabase product features. For the complete list of provisioned services and
...
1. Open **Solutions → Solution Modules** in the RAD platform top navigation, open **Metabase (Cloud Run)** from the **Platform Modules** list to start configuration, set `project_id`, and review the
   inputs. Configure only what you need — the
   [Configuration Guide](https://docs.radmodules.dev/docs/modules/Metabase_CloudRun)
   documents every input by group, with defaults. Review the estimated cost (if credits are enabled) and click **Deploy**, which opens the deployment status page with real-time logs.

2. The platform provisions the Cloud Run service, a Cloud SQL (PostgreSQL) database
   with its Secret Manager secrets, builds the container image, and runs a one-shot
   database-initialisation job that creates the Metabase schema before first boot.
...
3. When it completes, discover the resources with name-agnostic filters (so the
   ...
   SERVICE=$(gcloud run services list --project="$PROJECT" --region="$REGION" \
     --filter="metadata.name~metabase" --format="value(metadata.name)" --limit=1)
...
1. Metabase's JVM takes 60–120 seconds to fully start after the container is ready.
...
2. Open `$SERVICE_URL` in a browser. On first boot Metabase presents a **setup
   wizard** — create your admin account, set your language, and optionally add a
   data source. Metabase manages its own admin credentials internally; no admin
...
- **Revision unhealthy / service won't serve:** inspect the latest revision and its
  logs for startup errors. Metabase's JVM requires up to 120 seconds; if the startup
  probe threshold is too tight the revision will be marked unhealthy prematurely.
  ```bash
  gcloud run revisions list --service="$SERVICE" --project="$PROJECT" --region="$REGION"
  gcloud run services logs read "$SERVICE" --project="$PROJECT" --region="$REGION" --limit=100
  ```
- **Database connection errors:** confirm the Cloud SQL instance is `RUNNABLE`, the
  DB password secret exists, and the initialisation job completed.
...
| 2 — Access & verify | Manual | Health check passes; complete the first-boot setup wizard |
